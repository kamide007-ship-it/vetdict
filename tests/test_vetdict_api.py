...
class TestErrorHandlers:
    ...
    def test_500_handler_returns_json(self, client, monkeypatch):
        ...
        monkeypatch.setattr(vetdict_api, "reco2_get_status", MagicMock(side_effect=RuntimeError("forced crash")))
...
class TestAnalyzeSymptomsValidation:
    def test_missing_body_returns_400(self, client):
        resp = client.post("/api/analyze-symptoms", data="not json", content_type="text/plain")
    ...
    def test_empty_symptoms_list_returns_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": [], "species": "dog"})
        assert resp.status_code == 400

    def test_symptoms_not_a_list_returns_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": "vomiting", "species": "dog"})
        assert resp.status_code == 400

    def test_non_string_symptoms_return_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting", 1], "species": "dog"})
        assert resp.status_code == 400
    ...
    def test_blank_symptom_items_are_filtered_before_forwarding(self, monkeypatch, client):
        ...
        resp = client.post(
            "/api/analyze-symptoms", json={"symptoms": [" vomiting ", "   ", "diarrhea"], "species": "dog"}
        )
    ...
    def test_invalid_onset_returns_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "onset": "immediate"})
        assert resp.status_code == 400
    ...
    def test_valid_onset_values_accepted(self, monkeypatch, client):
        ...
        for onset in ("acute", "subacute", "chronic"):
            resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "onset": onset})
            assert resp.status_code == 200, f"onset={onset!r} rejected unexpectedly"

    def test_invalid_gender_returns_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "gender": "unknown"})
        assert resp.status_code == 400
    ...
    def test_valid_gender_values_accepted(self, monkeypatch, client):
        ...
        for gender in ("male", "female"):
            resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "gender": gender})
    ...
    def test_invalid_vaccination_status_returns_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "vaccination_status": "partial"})
        assert resp.status_code == 400
    ...
    def test_valid_vaccination_status_values_accepted(self, monkeypatch, client):
        ...
        for vs in ("current", "outdated", "none"):
            resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "vaccination_status": vs})
    ...
    def test_non_numeric_age_years_returns_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "age_years": "old"})
        assert resp.status_code == 400
    ...
    def test_numeric_age_years_coerced_to_float(self, monkeypatch, client):
        ...
        client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "age_years": 5})
    ...
    def test_string_numeric_age_years_coerced(self, monkeypatch, client):
        ...
        client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "age_years": "2.5"})
    ...
    def test_lab_values_coerced_to_float(self, monkeypatch, client):
        ...
        client.post(
            "/api/analyze-symptoms", json={"symptoms": ["vomiting"], "lab_values": {"bun": "25", "creatinine": 1.2}}
        )
    ...
    def test_invalid_lab_values_entries_skipped(self, monkeypatch, client):
        ...
        client.post(
            "/api/analyze-symptoms",
            json={"symptoms": ["vomiting"], "lab_values": {"bun": "not_a_number", "creatinine": 1.2}},
        )
    ...
    def test_all_invalid_lab_values_become_none(self, monkeypatch, client):
        ...
        client.post(
            "/api/analyze-symptoms", json={"symptoms": ["vomiting"], "lab_values": {"bun": "bad", "alt": "worse"}}
        )
    ...
    def test_valid_vaccines_forwarded(self, monkeypatch, client):
        ...
        client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "vaccines": ["core_5in1", "rabies"]})
    ...
    def test_blank_vaccine_ids_are_filtered_out(self, monkeypatch, client):
        ...
        client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "vaccines": [" rabies ", "   "]})
        assert captured["vaccines"] == ["rabies"]

    def test_vaccines_not_a_list_returns_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "vaccines": "rabies"})
        assert resp.status_code == 400
        assert "vaccines" in resp.get_json()["error"]

    def test_non_string_vaccines_return_400(self, client):
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting"], "vaccines": ["rabies", 1]})
        assert resp.status_code == 400
        assert "strings" in resp.get_json()["error"]
...
class TestAnalyzeSymptomsSuccess:
    ...
    def test_dog_path_calls_analyze_symptoms(self, monkeypatch, client):
        ...
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["vomiting", "diarrhea"], "species": "dog"})
        assert resp.status_code == 200
    ...
    def test_none_species_defaults_to_dog_path(self, monkeypatch, client):
        ...
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["lethargy"]})
        assert resp.status_code == 200
        mock.assert_called_once()
    ...
    def test_non_dog_species_calls_species_analyzer(self, monkeypatch, client):
        ...
        resp = client.post("/api/analyze-symptoms", json={"symptoms": ["lethargy"], "species": "cat"})
        assert resp.status_code == 200
        mock.assert_called_once()
    ...
    def test_value_error_from_analyzer_returns_400(self, monkeypatch, client):
        monkeypatch.setattr(vetdict_api, "SYMPTOM_CHECKER_AVAILABLE", True)
        monkeypatch.setattr(vetdict_api, "analyze_symptoms", MagicMock(side_effect=ValueError("bad symptoms")))
    ...
    def test_unexpected_exception_from_analyzer_returns_500(self, monkeypatch, client):
        monkeypatch.setattr(vetdict_api, "SYMPTOM_CHECKER_AVAILABLE", True)
        monkeypatch.setattr(vetdict_api, "analyze_symptoms", MagicMock(side_effect=RuntimeError("db crash")))
...
class TestProtectedRoutes:
    ...
    def test_status_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
    ...
    def test_logs_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
        self._disable_token(monkeypatch)
        monkeypatch.setattr(vetdict_api, "RECO2_AVAILABLE", False)
    ...
    def test_evaluate_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
        self._disable_token(monkeypatch)
        monkeypatch.setattr(vetdict_api, "RECO2_AVAILABLE", False)
    ...
    def test_feedback_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
        self._disable_token(monkeypatch)
        monkeypatch.setattr(vetdict_api, "RECO2_AVAILABLE", False)
    ...
    def test_patrol_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
        self._disable_token(monkeypatch)
        monkeypatch.setattr(vetdict_api, "RECO2_AVAILABLE", False)
    ...
    def test_r3_config_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
        self._disable_token(monkeypatch)
        monkeypatch.setattr(vetdict_api, "RECO2_AVAILABLE", False)
    ...
    def test_r3_analyze_input_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
        self._disable_token(monkeypatch)
        monkeypatch.setattr(vetdict_api, "RECO2_AVAILABLE", False)
    ...
    def test_r3_analyze_output_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
        self._disable_token(monkeypatch)
        monkeypatch.setattr(vetdict_api, "RECO2_AVAILABLE", False)
    ...
    def test_r3_chat_without_token_config_returns_503_when_reco2_unavailable(self, monkeypatch, client):
        self._disable_token(monkeypatch)
        monkeypatch.setattr(vetdict_api, "RECO2_AVAILABLE", False)
    ...
    def test_wrong_token_returns_401(self, monkeypatch, client):
        ...
        resp = client.get("/api/status", headers=_auth_header("wrong-token"))
...
class TestReco2RoutesWithMocks:
    ...
    def test_r3_config_returns_public_config(self, monkeypatch, client):
        ...
        monkeypatch.setattr(vetdict_api, "public_reco2_config", MagicMock(return_value={"public_key": "value"}))
    ...
    def test_r3_analyze_input_calls_input_gate(self, monkeypatch, client):
        ...
        monkeypatch.setattr(
            ...
            MagicMock(
                return_value={
                    "input_w_ambiguity": 0.2,
                    "input_w_assertion": 0.25,
                    "input_w_emotion": 0.3,
                    "input_w_unrealistic": 0.25,
                }
            ),
        )
    ...
    def test_r3_analyze_output_calls_output_gate(self, monkeypatch, client):
        ...
        monkeypatch.setattr(
            ...
            MagicMock(
                return_value={
                    "output_w_assertion": 0.3,
                    "output_w_evidence": 0.3,
    ...
    def test_r3_chat_calls_orchestrator_process(self, monkeypatch, client):
        ...
        monkeypatch.setattr(vetdict_api, "reco2_get_orchestrator", MagicMock(return_value=mock_orch))
        ...
        assert resp.status_code == 200
        mock_orch.process.assert_called_once_with("what ails my cat?", domain="veterinary", context={}, max_tokens=512)
