...
def test_sweep16_brand_paren_skip_fix_and_variant_aliases():
...
class TestBatch50AndSweep16Parallel:
    """2026-08 audit (16th sweep, parallel session): referenced-but-absent drugs
    (shipped as batch 50 — sibling sessions claimed the 47/48/49 slots first) +
    duplicate-card consolidation + katakana variant aliases."""

    def test_batch50_drugs_present_with_bilingual_dosing(self):
...
class TestBatch49FluorouracilAndIronAliases:
...
class TestBatch59Methazolamide:
    """2026-09 audit (25th sweep): methazolamide was dosed by name in the
