...
DISEASES: List[Dict[str, Any]] = [
    # ── Endocrine / Metabolic ──────────────────────────────────────────────
    {
        ...
        "description_ja": "デグーの糖尿病は非常に高い有病率（デグーの30-50%が生涯中に臨床糖尿病発展）の深刻な代謝疾患。重大：デグーは食事糖に対する貧弱な耐性—ほとんどげっ歯類と異なり、デグーは限定的な果糖吸収能と糖摂取応答における最小限のグルコース恒常性調節能。わずかな量の果物または高炭水化物食でも持続的高血糖を誘発。ほとんどの症例でtype 2糖尿病（インスリン抵抗、続発性ベータ細胞不全）として提示、遺伝的素因と食事炭水化物曝露による。早期検出を通じた検査と厳格食事管理は易感受性動物の発症を防止または遅延可能。",
        "urgency": "high",
        ...
        "prevention_ja": "デグーの重大予防（積極的食事管理で遺伝的易感受性動物の50-70%で糖尿病を防止可能）：(1) デグー特異的食事—離乳から厳密な炭水化物制限食を実装。(2) 体重管理。(3) ストレス低減。(4) 遺伝学的スクリーニング。(5) 日常的健康スクリーニング。(6) 砂糖摂取を毒性として扱う。(7) 水質。(8) 生殖管理。(9) 併発疾患予防。(10) 診断後予防。",
