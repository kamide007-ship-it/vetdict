...
"urgency": "normal",
    },
    {
        ...
        "treatment": "IMMEDIATE INTERVENTION: 1) ANTIBIOTICS empirical (culture pending, do NOT delay for results): First-line—enrofloxacin 5-10 mg/kg IV q12h OR amoxicillin-clavulanate 22 mg/kg IV/SC q8h (excellent penetration into mastoid/abscess; Staph coverage) × 14-21 days. Alternative—cephalexin 25-30 mg/kg PO q6-8h (oral only if mild, stable case). Culture and sensitivity should guide switch if resistance noted. 2) SC/IV FLUIDS: LRS 20 mL/kg/day × 48-72h (support perfusion, assist toxin clearance); more aggressive (40 mL/kg/day) if signs of septic shock (tachycardia >150, MM pale/muddy, CRT >2 sec). 3) ANALGESIA + NSAID: Meloxicam 0.2 mg/kg PO/IV q24h × 7 days (reduces local inflammation, improves comfort, allows nursing) OR carprofen 4-6 mg/kg q8-12h PO (alternatives if meloxicam contraindicated). Pain relief is CRITICAL—relieves stress, improves dam appetite (essential for continued lactation), allows puppies to nurse (milk removal aids resolution). 4) LOCAL MANAGEMENT: Warm compresses q6-8h × 10-15 min to affected mammae (increases blood flow, promotes drainage, comfort). Gentle expression of milk (if possible without dam distress) q4-6h to relieve engorgement + prevent stasis. NOTE: NEVER aggressively milk out infected gland (can spread infection systemically); gentle expression or express into container (discard—do not allow puppies to consume infected milk if possible, but nursing from affected gland is usually safe once antibiotics given 24h+). 5) NUTRITIONAL SUPPORT: Ensure dam receives HIGH-QUALITY, high-protein diet (30%+ protein, 15-20% fat); increase food volume 50% above maintenance (lactation demands 2-4× normal calories). Offer frequent small meals, ensure adequate fresh water. Hypoglycemia + poor nutrition perpetuate infection. 6) NURSING MANAGEMENT: CONTINUE NURSING if possible (milk removal is therapeutic; antibiotics rapidly achieve milk concentrations safe for puppies after 24h). Monitor puppies q4-6h for diarrhea or lethargy (signs of infected milk); if dam cannot nurse effectively, supplement formula (bottle/syringe) and consider early weaning (day 24-28 vs normal 28-35 days). If mastitis severely affects dam's condition (fever >40°C, signs of septic shock), temporarily separate puppies (24-48h) and hand-rear with formula, reunite once dam is stabilized + antibiotics >24h aboard + fever resolved. 7) IMAGING if abscess suspected (fluctuant mass, nonresponsive to antibiotics >7 days): Abdominal ultrasound to assess abscess size, guide drainage. 8) SURGICAL DRAINAGE if abscess large (>3 cm) or fluctuant: under local anesthesia (procaine 2% infiltration), incise skin + capsule, perform digital evacuation of pus, culture, irrigate with saline, consider penrose drain × 24-48h (monitor for infection). Avoid removal of entire gland (rarely necessary; even necrotic tissue may regain function). 9) SEPSIS MONITORING: If high fever (>40°C) + lethargy/anorexia/shock signs: hospitalize, IV fluids, consider ICU care (vasopressor support if hypotensive).",
        "treatment_ja": "迅速介入：1) 抗菌薬経験的（培養待機、結果待たず直ちに開始）：第一選択—エンロフロキサシン5-10 mg/kg IV q12h OR アモキシシリン・クラブラン酸 22 mg/kg IV/SC q8h（膿瘍への優れた浸透；スタフィロコッカス被覆）× 14-21日。代替—セファレキシン25-30 mg/kg PO q6-8h（軽症で安定している場合のみ経口）。培養感受性で抵抗ありなら変更。2) SC/IV輸液：LRS 20 mL/kg/日 × 48-72h（灌流支持、毒素クリアランス支援）；敗血性ショック兆候（頻脈 >150、MM蒼白/濁色、CRT >2秒）なら攻撃的（40 mL/kg/日）。3) 鎮痛+NSAID：メロキシカム0.2 mg/kg PO/IV q24h × 7日（局所炎症軽減、快適性改善、授乳許可）OR カルプロフェン4-6 mg/kg q8-12h PO（メロキシカム禁忌時の代替）。疼痛緩和は重大—ストレス軽減、母体食欲改善（継続授乳必須）、仔犬が授乳可能（乳搾乳が解決を促進）。4) 局所管理：温罨布 q6-8h × 10-15分患側乳腺（血流増加、排液促進、快適）。温和な搾乳（母体苦痛なしで可能）q4-6h で硬化軽減+うっ滞防止。重要：感染乳腺を積極的に搾乳しない（全身感染拡散リスク）；温和な搾乳または容器に搾乳（廃棄—患側から仔が吸吮できれば許可、抗菌薬24h以上で安全）。5) 栄養補給：高品質、高タンパク食（30%以上タンパク、15-20%脂肪）；食事量を維持量の50%増加（授乳は通常の2-4倍カロリー要求）。頻回少量食、十分な新鮮水。低血糖+栄養不良が感染遷延。6) 授乳管理：可能な限り授乳継続（乳搾乳は治療的；抗菌薬が24h後に仔安全な乳濃度に急速到達）。仔 q4-6h を下痢/沈鬱で監視（感染乳徴候）；母体が授乳不能なら補足式（ボトル/シリンジ）考慮+早期離乳（日24-28 vs 正常28-35日）。マスティティスが母体状態に重度影響（発熱 >40°C、敗血性ショック兆候）なら一時的に仔分離（24-48h）と人工飼育フォーミュラ、母体安定化+抗菌薬24h以上+発熱解決で再合流。7) 膿瘍疑い時の画像検査（波動性腫瘤、抗菌薬>7日で無反応）：腹部超音波で膿瘍サイズ評価、排液ガイダンス。8) 膿瘍大きい（>3 cm）または波動性の場合の外科的排液：局所麻酔（プロカイン2%浸潤）で皮膚+被膜切開、膿を指で除去、培養、生食灌流、ペンローズドレーン × 24-48h（感染を監視）。乳腺全体除去は稀に必要（壊死組織でも機能回復可能）。9) 敗血症監視：高熱（>40°C）+沈鬱/食欲不振/ショック兆候：入院、IV輸液、ICU管理考慮（低血圧時のバソプレッサー支持）。",
