...
# 助詞直後の純粋な強調・時間副詞の除去（2026-09 第35回監査）。
# 「後ろ足を急に痛がって」「お腹が突然パンパンに膨れて」のような副詞挿入形が
# キュレート済みキー（後ろ足を痛が / お腹が膨れて）に不一致で抽出ゼロになる
# 系統的故障クラスへの根本対応。日本語で助詞（が/を/も/は）直後の
# 「急に」「突然」「パンパンに」は常に述語修飾の副詞であり、除去しても
...
def normalize_chat_text(text: str) -> str:
    ...
    正規化後も保存される。助詞直後の強調・時間副詞（急に/突然/パンパンに）も
    除去し、副詞挿入によるエイリアス不一致を防ぐ。
    """
