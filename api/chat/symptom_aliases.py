...
SYMPTOM_ALIASES = {
    ...
    "口をパクパクさせてる": "vomiting",
    "泡が出てる": "vomiting",
    # 「口から泡」は嘔吐ではなく泡沫状流涎（発作・中毒・口腔痛・爬虫類呼吸器）
    # — 既存の「泡を吹く」→drooling と整合（2026-09 第23弾で vomiting から是正）
    "口から泡": "drooling",
    # フェレット
    "まだらに抜ける": "hair_loss",
    "腫瘤": "lumps_and_bumps",
    ...
    "目が黄色い": "jaundice",
    "皮膚が黄色い": "jaundice",
    # 頻呼吸: 「息が速い」表記（呼吸が速い/速く は上のブロックで収載済み）
    ...
    # --- 2026-08 第14回精度スイープ ---
    # 多飲の連用形（「水をたくさん飲んで」はmainの並行第29弾が独立に追加済みの
    # ため再定義しない: ruff F601。「水をよく飲んで」形のみ本セッションで補完）
    "水をよく飲んで": "excessive_thirst",
    # 「お腹だけ膨れてきた」（クッシングのポットベリー — 「お腹が膨れ」系は
    # 収載済みだが助詞「だけ」で不一致だった）
    "お腹だけ膨れ": "bloating",
    # 眼球突出の連用形（「目が飛び出してきた」のみで「〜てきて」を取りこぼし）
    "目が飛び出してきて": "eye_bulging",
    # 色素尿の別表現（「おしっこが茶色い」「尿の色が濃い」に加え「おしっこの
    # 色が濃い」形 — 尿の色が濃い は既収載のため再定義しない: ruff F601）
    "おしっこの色が濃い": "dark_urine",
    # 口腔内腫瘤（従来は「できものがある」→lumps_and_bumps に消費され、
    # 口腔特異的な鑑別（口腔内腫瘍・マウスロット）に到達しなかった）
    "口の中にできもの": "oral_mass",
    "口の中にしこり": "oral_mass",
    "口の中に腫瘤": "oral_mass",
    "歯茎にできもの": "oral_mass",
    "歯ぐきにできもの": "oral_mass",
    # 鳥の疥癬（クヌドコプテス）: 脚の痂皮性病変の口語が皆無だった
    "脚に白いかさぶた": "crusty_lesions_on_legs",
    "足に白いかさぶた": "crusty_lesions_on_legs",
    "脚がガサガサ": "crusty_lesions_on_legs",
    "足がガサガサ": "crusty_lesions_on_legs",
    "脚にかさぶた": "crusty_lesions_on_legs",
    # --- 2026-08 精度スイープ第14弾: 辞書形・連用形・語順ゆれの取りこぼし ---
    # 「関節が腫れてる/腫れて」のみ収載で辞書形「関節が腫れる」が欠落
    # （モルモット壊血病の教科書的主訴が抽出不能だった）
    "関節が腫れる": "swollen_joints",
    # 血便の口語: 「うんちに血」はあるが「便に血」系が欠落
    "便に血が混じ": "blood_in_stool",
    "便に血": "blood_in_stool",
    "お尻から血": "blood_in_stool",
    # 鳥の気嚢炎・呼吸器疾患: クリック音の擬音語（「呼吸のたびに音」では
    # 「呼吸のたびにプチプチ音」を取りこぼす）
    "プチプチ音": "clicking_breathing_sounds",
    ...
    "骨が弱い": "soft_bones",
    "骨がもろい": "soft_bones",
    # サワークロップ（そのうカンジダ・そのう炎）の定義的主訴
    "そのうから酸っぱい": "sour_crop_odor",
    "そのうが酸っぱい": "sour_crop_odor",
    "酸っぱい臭い": "sour_crop_odor",
    "酸っぱいにおい": "sour_crop_odor",
    # 犬の整形外科主訴: 「階段を上れない」「後ろ足が震える」が抽出不能で
    # てんかん/甲状腺が上位だった（変形性関節症の代表的主訴）
    "階段を上れない": "stiffness",
    "階段を嫌がる": "stiffness",
    "階段を上りたがらない": "stiffness",
    # 「足が震え」→tremors より長い最長一致で後肢弱化に正しく解決
    ...
    "皮膚が見えて": "feather_loss",
    # --- 2026-09 Round 19 ---
    # 失神・虚脱: 「倒れた」のみで連用形「倒れて」と「意識を失った」が欠落
    # （散歩中の失神は僧帽弁閉鎖不全症/不整脈の代表的主訴 — Ettinger 8th ed）
    "倒れて": "collapse",
    "意識を失っ": "collapse",
    "意識がなくなっ": "collapse",
    "気を失っ": "collapse",
    # GDV: 「お腹がパンパンに膨れて」に副詞（突然/急に）が挟まると
    # 「パンパンに膨れて」→edema の最長一致に負けて腹部文脈が消えていた
    "お腹が突然パンパン": "bloating",
    "お腹が急にパンパン": "bloating",
    # 慢性アトピーの苔癬化・色素沈着（掻きむしる/黒ずみ/ゴワゴワが全て未収載だった）
    "掻きむしっ": "itching",
    "かきむしっ": "itching",
    "皮膚が黒ずん": "skin_thickening",
    "皮膚がゴワゴワ": "skin_thickening",
    "皮膚が厚く": "skin_thickening",
    # 多飲の飼い主表現: 「水をよく飲む/たくさん飲んで」のみで「量が増えた」形が欠落
    "水を飲む量が増え": "excessive_thirst",
    "飲水量が増え": "excessive_thirst",
    # 被毛粗剛（慢性疾患の非特異的サイン: CKD/甲状腺/寄生虫）
    "毛づやが悪": "poor_coat",
    "毛艶が悪": "poor_coat",
    "毛並みが悪": "poor_coat",
    # 前庭疾患: 「首が傾いて」のみで「頭が傾いて」「目が回って」が欠落
    "頭が傾い": "head_tilt",
    "目が回っ": "nystagmus",
    "グルグル回る": "circling",
    "グルグル回っ": "circling",
    "ぐるぐる回っ": "circling",
    # カメの甲羅軟化（MBD）: 「甲羅がやわらかい」（かな）のみで漢字形と「凹む」が欠落
    "甲羅が柔らか": "soft_shell",
    "甲羅が凹": "shell_deformity",
    "甲羅がへこ": "shell_deformity",
    # 腹腔内腫瘤の触知（フェレット・リンパ腫等）と体重減少の連用形
    "痩せてきて": "weight_loss",
    "お腹にしこり": "abdominal_masses",
    "お腹を触るとしこり": "abdominal_masses",
    "お腹の中にしこり": "abdominal_masses",
    # 鳥の水様便（「便が緑/水っぽいうんち」のみで「便が水っぽい」形が欠落）
    "便が水っぽ": "diarrhea",
    "水っぽい便": "diarrhea",
    # --- 2026-09 第23弾スイープ ---
    # 犬膵炎の祈りのポーズ主訴: 「お腹を触ると痛がる」のみで「お腹が痛そう」形が欠落
    "お腹が痛そう": "abdominal_pain",
    "おなかが痛そう": "abdominal_pain",
    "お腹を痛そうに": "abdominal_pain",
    # 心疾患の運動不耐性: 「疲れやすい/すぐ疲れる」のみで連用形「すぐ疲れて」が欠落
    "すぐ疲れ": "exercise_intolerance",
    # 趾間皮膚炎/アトピーの足先咬み（掻痒行動）
    "足先を噛": "itching",
    "足の先を噛": "itching",
    "足先をかじ": "itching",
    # チンチラ毛噛み（バーバリング）: 疾患名は収載済みなのに飼い主表現が皆無だった
    "毛をかじ": "fur_chewing",
    "自分の毛を噛": "fur_chewing",
    "毛が短くなっ": "patchy_short_fur",
    # 爬虫類肺炎の開口呼吸: 「口を開けて呼吸」のみで語順逆転形が欠落
    "呼吸のたびに口を開": "open_mouth_breathing",
    "口を開けたまま呼吸": "open_mouth_breathing",
    # --- 2026-09 第24弾スイープ ---
    # 血尿の口語形（「尿が赤い」「おしっこに血」のみで「おしっこが赤い」系が欠落 —
    # 血尿+頻尿の膀胱炎/尿石症主訴が多飲多尿疾患に誤誘導されていた）
    "おしっこが赤": "blood_in_urine",
    "おしっこの色が赤": "blood_in_urine",
    # 眼瞼痙攣（目が開かない）: 既存「目が開かない」→eye_swelling は温存しつつ、
    # 「開かなくなった」変化形は羞明/疼痛の squinting に解決（猫の角膜潰瘍/FHV-1）
    "目が開かなく": "squinting",
    "片目が開かな": "squinting",
    "目が開けられない": "squinting",
    ...
    # ハムスター等の皮膚腫瘤の口語（「黒いイボのようなもの」が抽出ゼロだった）
    "イボのような": "wart_like_growths",
    # 鳥の慢性産卵症候群（発情過多）: 疾患は収載済みなのに飼い主表現が皆無だった
    "産卵が止まらない": "excessive_egg_laying",
    "産卵が続い": "excessive_egg_laying",
    "卵を産み続け": "excessive_egg_laying",
    # 発情持続（フェレット高エストロジェン血症 / 鳥の慢性発情はチェーンで解決）
    "発情が続い": "prolonged_estrus",
    "発情が止まらない": "prolonged_estrus",
    # チンチラ流涙/歯科疾患の顔こすり主訴
    "目が濡れ": "eye_discharge",
    "顔をこす": "pawing_at_face",
    ...
    "抱くと鳴": "back_pain",
    "抱っこすると鳴": "back_pain",
    # 急性緑内障の角膜浮腫（「目が白く濁って」は副詞挿入「急に」で不一致だった）
    "急に白く濁っ": "cloudy_eye",
    "片目が白く": "cloudy_eye",
    "片目が濁っ": "cloudy_eye",
    # 腹囲膨満の「大きくなって」語幹形（なった/なってきた形のみ収載で て形が欠落）
    "お腹が大きくなっ": "bloating",
    # 「痛がる」の連用形（「痛がって（いる）」が終止形キーに不一致だった）
    "痛がって": "pain",
    # 斜頸の「かしげたまま」形（「首をかしげて」のみ収載で た形が欠落）→ 語幹キー
    "首をかしげ": "head_tilt",
    # ハムスター頬袋脱（「頬袋から何か飛び出している」が抽出ゼロだった）
    "頬袋から何か": "cheek_pouch_prolapse",
    "頬袋が飛び出": "cheek_pouch_prolapse",
    "頬袋から飛び出": "cheek_pouch_prolapse",
    # 爬虫類の開口呼吸「口をずっと開けている」（ずっと挿入形）
    ...
    "糞の数が減": "small_fecal_pellets",
    # 受動的脱羽の語幹形（「羽が抜ける」のみで てきた/て形が欠落。行動学的な
    # 「羽を抜く」（毛引き）とは助詞で区別される）
    "羽が抜け": "hair_loss",
    # モルモット尿石症の排尿時発声（squealing_when_urinating が種語彙に実在）
    "おしっこのときにキーキー": "squealing_when_urinating",
    "おしっこのときに鳴": "squealing_when_urinating",
    "排尿時にキーキー": "squealing_when_urinating",
    ...
    # 2026-09 精度監査 第28弾
    # ---------------------------------------------------------------
    # 多飲の連用形（「水をよく飲む/飲んで」のみ収載で「飲み」を取りこぼし）
    "水をよく飲み": "excessive_thirst",
    # 猫の多尿の飼い主観察（トイレ砂の塊が大きい＝尿量増加の代表的な気づき）
    "トイレの砂の塊が大き": "excessive_urination",
    "砂の塊が大き": "excessive_urination",
    "おしっこの塊が大き": "excessive_urination",
    # ウサギの良性ポルフィリン色素尿はオレンジ〜赤色（赤系のみ収載だった）
    "おしっこがオレンジ": "red_urine",
    "尿がオレンジ": "red_urine",
    ...
    # 裸の「口をパクパク」→vomiting（そのう疾患の吐出動作）は維持し、
    # 呼吸語を含む長い形のみ最長一致で上書きする）
    "口をパクパクさせて呼吸": "open_mouth_breathing",
    "口を開けてパクパク": "open_mouth_breathing",
    # 腹部膨満の副詞挿入形（「お腹が膨らんで」に「大きく」が割り込むと不一致）
    "お腹が大きく膨らん": "bloating",
    # ヘビの腹側発赤（スケールロット）の連用形・体側一般形
    "鱗が赤く": "skin_redness",
    "うろこが赤く": "skin_redness",
    # 爬虫類ラバージョーの複合形（「あごが腫れて柔らかい」は既存の
    # 「あごが柔らか」語幹が非連続で不一致だった）
    "あごが腫れて柔らか": "jaw_softening",
    # カメの伸頸呼吸（チェロニアン肺炎の教科書的姿勢 — Mader 3rd ed）
    "首を伸ばして呼吸": "respiratory_distress",
    "首を伸ばして息": "respiratory_distress",
    # 骨折疑いの飼い主申告（観察可能な実体は跛行・負重不能）
    "骨が折れたかも": "limping",
    "骨が折れてるかも": "limping",
    "骨折したかも": "limping",
    "骨が折れた": "limping",
    # 毛でつながった便（ウサギ等の毛球・GIうっ滞の古典的な飼い主観察 —
    # Oglesbee, Blackwell's 5-Minute: Small Mammal）
    ...
    # 2026-09 精度監査 第34弾
    # ---------------------------------------------------------------
    # 尿閉の「ほとんど出ない」形（既存「おしっこが出ない」に ほとんど が
    # 割り込む — ブロック猫の代表的主訴。キーに否定を含めることで
    # 否定ガードの自己遮断を回避する既存慣行に従う）
    "おしっこがほとんど出ない": "decreased_urination",
    "おしっこがほとんど出ていない": "decreased_urination",
    "おしっこがほとんど出てない": "decreased_urination",
    "尿がほとんど出ない": "decreased_urination",
    # 後肢脱力の ず形・足表記（既存は「後ろ足に力が入らない」のみ —
    # フェレット・インスリノーマの代表的主訴が ず形で不達だった）
    "後ろ足に力が入らず": "hind_limb_weakness",
    "後ろ足に力が入らなく": "hind_limb_weakness",
    "足に力が入らず": "leg_weakness",
    "脚に力が入らず": "leg_weakness",
    # デグー尾剥脱の語幹形（既存「尻尾の皮がむけた」は
    # 「むけてしまいました」形に不一致だった）
    "尻尾の皮がむけ": "tail_injury",
    "しっぽの皮がむけ": "tail_injury",
    "尾の皮がむけ": "tail_injury",
    # うずくまり姿勢の促音便語幹（既存「うずくまる/うずくまっている」は
    # 「うずくまっていて」形に不一致 — 自動て語幹展開は漢字必須ガードで
    # かな語幹を生成しないため手動キュレート）
    "うずくまっ": "hunched_posture",
    # 飲水拒否（adipsia — 急性腹症・重度全身疾患の随伴。食欲不振の
    # プロキシとして解決する: 骨が折れたかも→limping と同型の観察実体マップ）
    "水も飲まず": "loss_of_appetite",
    "水も飲まない": "loss_of_appetite",
    "水を飲まず": "loss_of_appetite",
    "水を飲まなくなっ": "loss_of_appetite",
    # 耳基部の掻痒（既存「耳を掻く」に 付け根 が割り込む形 — 猫耳ダニの
    # 代表的主訴）+ 耳ダニの黒色耳垢の「出ます」形
    "耳の付け根を掻": "ear_scratching",
    "耳の付け根をか": "ear_scratching",
    "黒いカスが出": "ear_discharge",
    # ウサギの盲腸便付着（uneaten cecotropes / sticky bottom — 最頻の
    # 飼い主相談の一つなのに付着表現が皆無だった。Harcourt-Brown;
    # 非保有種は ID_SYNONYMS で soft_stool/diarrhea へフォールバック）
    "お尻の周りに軟らかい便": "abnormal_cecotropes",
    "お尻の周りに柔らかい便": "abnormal_cecotropes",
    "お尻に軟らかい便": "abnormal_cecotropes",
    "軟らかい便がつ": "abnormal_cecotropes",
    "柔らかい便がつ": "abnormal_cecotropes",
    "盲腸便がつ": "abnormal_cecotropes",
    "盲腸便が残": "abnormal_cecotropes",
    # ヘビ・スペクタクル残留の位置修飾形（既存「脱皮した皮が目に残って」は
    # 「目のところだけ残って」形に不一致）
    "脱皮した皮が目": "retained_spectacle",
    # 白内障の「白目のあたり」形（飼い主は強膜と水晶体を区別しない —
    # 既存キーは全て「目が白く濁」直結形のみだった）
    "白目のあたりが白く": "cloudy_eye",
    "白目が白く濁": "cloudy_eye",
    "白目が濁っ": "cloudy_eye",
    "目のあたりが白く濁": "cloudy_eye",
    # 悪臭のある滲出液（リクガメ甲羅腐敗の定義的所見 — Mader 3rd ed。
    # foul_odor 非保有種は抽出段階で安全に脱落）
    "臭い液が出": "foul_odor",
    "悪臭のする液": "foul_odor",
    "悪臭のある液": "foul_odor",
}
