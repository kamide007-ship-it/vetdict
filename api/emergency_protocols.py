...
EMERGENCY_PROTOCOLS: list[dict[str, Any]] = [
    ...
    # ============================================================
    {
        ...
        "species": ["dog", "cat"],
        "trigger_signs_ja": ["体温>41°C", "パンティング/開口呼吸", "粘膜充血→チアノーゼ", "虚脱", "嘔吐/下痢（血性）", "DIC徴候"],
        "trigger_signs_en": ["BT >41°C (106°F)", "Excessive panting", "Brick-red→cyanotic MM", "Collapse", "Vomiting/bloody diarrhea", "DIC signs"],
        "steps": [
            {"order": 1, "phase": "Active cooling", "ja": "冷水スプレー + 扇風機。⚠氷水浴は末梢血管収縮で逆効果。大血管走行部（頸・腋窩・鼠径）にクーリングパック。", "en": "Cool water spray + fan. ⚠ Ice bath causes vasoconstriction (counterproductive). Apply cooling packs to large vessels (neck, axilla, groin).", "time_target": "即時"},
            {"order": 2, "phase": "Target temp", "ja": "39.5°Cで積極的冷却中止（過冷却防止）。体温q5分モニタリング。", "en": "Stop active cooling at 39.5°C (103°F) to prevent overshoot. Monitor BT q5min.", "time_target": "10-30分"},
            {"order": 3, "phase": "IV fluids", "ja": "LRS 10-20 mL/kg IV ボーラス（室温液）。⚠冷却液は不要。", "en": "Room temp LRS 10-20 mL/kg IV bolus. ⚠ Chilled fluids not needed.", "time_target": "並行"},
            {"order": 4, "phase": "Coagulopathy", "ja": "PT/aPTT/フィブリノゲン/D-dimer/血小板。DIC → FFP 10-15 mL/kg IV。", "en": "Check PT/aPTT/fibrinogen/D-dimer/platelets. DIC → FFP 10-15 mL/kg IV.", "time_target": "30分以内"},
            {"order": 5, "phase": "Organ protection", "ja": "腎：輸液+尿量モニタリング。肝：ALT/biliモニタリング。腸：マロピタント+制酸薬。脳：痙攣→ジアゼパム。", "en": "Renal: fluids + UO monitoring. Hepatic: ALT/bili. GI: maropitant + PPI. CNS: diazepam if seizures.", "time_target": "持続"},
            {"order": 6, "phase": "⚠ Do NOT", "ja": "解熱薬（NSAIDs/ステロイド）禁忌：セットポイントは正常→薬理学的解熱は無意味かつ有害。", "en": "⚠ Do NOT give antipyretics (NSAIDs/steroids) — thermostat set-point is normal; chemical antipyresis is useless and harmful.", "time_target": "常に"},
        ],
        "key_drugs": [
        ...
        "monitoring": ["Body temperature q5min until 39.5°C", "PT/aPTT/platelets", "BUN/Cr/ALT/bilirubin", "Urine output", "Blood glucose", "ECG"],
    ...
    # ============================================================
    {
        ...
        "species": ["cat"],
        "trigger_signs_ja": ["排尿努責（空振り）", "嘔吐", "元気消失", "膀胱過膨張（硬い大きな膀胱触知）", "徐脈（高K）", "低体温"],
        "trigger_signs_en": ["Stranguria (non-productive)", "Vomiting", "Lethargy", "Large firm bladder", "Bradycardia (hyperK)", "Hypothermia"],
        "steps": [
            {"order": 1, "phase": "Stabilize hyperK", "ja": "ECG: テント状T波/QRS幅広/徐脈 → Ca gluconate 10% 0.5-1 mL/kg IV slow（心筋安定化）。", "en": "ECG: peaked T/wide QRS/bradycardia → Ca gluconate 10% 0.5-1 mL/kg IV slow (cardioprotection).", "time_target": "即時"},
            {"order": 2, "phase": "IV fluids", "ja": "0.9% NaCl IV（⚠LRSはK含有→高K時回避）。", "en": "0.9% NaCl IV (⚠ avoid LRS — contains K+).", "time_target": "即時"},
            {"order": 3, "phase": "Relieve obstruction", "ja": "鎮静（ブトルファノール0.2 mg/kg + ミダゾラム0.2 mg/kg IM/IV）→ 3.5Fr尿道カテーテル挿入+フラッシュ。", "en": "Sedate (butorphanol 0.2 mg/kg + midazolam 0.2 mg/kg IM/IV) → insert 3.5Fr urinary catheter, flush with saline.", "time_target": "安定化後"},
            {"order": 4, "phase": "Post-obstructive diuresis", "ja": "閉塞解除後は大量利尿あり → IV fluids 2-3×維持量 × 24-48h。尿量モニタリング。", "en": "Expect post-obstructive diuresis → IV fluids 2-3× maintenance × 24-48h. Monitor UO.", "time_target": "解除後24-48h"},
            {"order": 5, "phase": "Monitor", "ja": "電解質（K+, Ca, P）q6-12h。BUN/Cr q12-24h。ECG。", "en": "Electrolytes (K+, Ca, P) q6-12h. BUN/Cr q12-24h. ECG.", "time_target": "入院中"},
            {"order": 6, "phase": "Long-term", "ja": "再発予防: 処方食（S/O等）、飲水促進、ストレス軽減。再発3回以上 → 会陰尿道瘻術（PU）。", "en": "Prevention: prescription diet, increase water intake, reduce stress. Recurrence ≥3 → perineal urethrostomy (PU).", "time_target": "退院後"},
        ],
        "key_drugs": [
            {"name": "Calcium gluconate 10%", "name_ja": "グルコン酸カルシウム10%", "dose": "0.5-1 mL/kg IV slow over 5-10 min (hyperK)"},
            ...
            {"name": "Buprenorphine", "name_ja": "ブプレノルフィン", "dose": "0.02 mg/kg IV/buccal q6-8h (post-op analgesia)"},
    ...
    # ============================================================
    {
        ...
        "species": ["dog", "cat"],
        "trigger_signs_ja": ["陰部膣分泌物（開放型）", "腹部膨満（閉鎖型）", "多飲多尿", "嘔吐/食欲廃絶", "発熱→低体温（敗血症）"],
        "trigger_signs_en": ["Vaginal discharge (open)", "Abdominal distension (closed)", "PU/PD", "Vomiting/anorexia", "Fever→hypothermia (sepsis)"],
        "steps": [
            {"order": 1, "phase": "Stabilize", "ja": "輸液: LRS 10-20 mL/kg ボーラス（敗血症ならショック輸液）。広域抗菌薬開始。", "en": "Fluids: LRS 10-20 mL/kg bolus (shock dose if sepsis). Start broad-spectrum antibiotics.", "time_target": "0-30分"},
            {"order": 2, "phase": "Antibiotics", "ja": "アンピシリン/スルバクタム 20 mg/kg IV q8h + エンロフロキサシン 5 mg/kg IV q24h。", "en": "Ampicillin/sulbactam 20 mg/kg IV q8h + Enrofloxacin 5 mg/kg IV q24h.", "time_target": "即時"},
            {"order": 3, "phase": "Surgery (definitive)", "ja": "OVH: 全身麻酔下卵巣子宮摘出術（可能な限り早期 — 閉鎖型は緊急度高）。", "en": "OVH: ovariohysterectomy under GA (ASAP — closed pyometra is more urgent).", "time_target": "安定化後6-12h以内"},
            {"order": 4, "phase": "Medical (breeding)", "ja": "繁殖希望: PGF2α（ジノプロスト）0.1-0.25 mg/kg SC q12-24h ×3-7d + 抗菌薬。⚠閉鎖型は子宮破裂リスク。", "en": "Breeding value: PGF2α (dinoprost) 0.1-0.25 mg/kg SC q12-24h ×3-7d + antibiotics. ⚠ Closed pyometra = uterine rupture risk.", "time_target": "OVH不可の場合のみ"},
            {"order": 5, "phase": "Post-op", "ja": "輸液維持、抗菌薬継続7-10日、メロキシカム0.1-0.2 mg/kg q24h。", "en": "Maintenance fluids, continue antibiotics 7-10d, meloxicam 0.1-0.2 mg/kg q24h.", "time_target": "術後"},
        ],
        "key_drugs": [
            ...
            {"name": "PGF2α (dinoprost)", "name_ja": "ジノプロスト", "dose": "0.1-0.25 mg/kg SC (medical management only)"},
        ],
        "monitoring": ["Temperature q4h", "WBC/band neutrophils", "Blood glucose (hypoglycemia in sepsis)", "Blood pressure", "Urine output"],
    ...
    # ============================================================
    {
        ...
        "trigger_signs_en": ["Acute dyspnea", "Tachypnea", "Shallow rapid breathing", "Cyanosis", "Absent lung sounds dorsally", "Trauma history"],
        "steps": [
            {"order": 1, "phase": "Thoracocentesis", "ja": "即時胸腔穿刺: 背側7-9肋間、バタフライカテーテルまたは18-20G針+三方活栓+シリンジ。", "en": "Immediate thoracocentesis: dorsal 7-9th ICS, butterfly catheter or 18-20G needle + 3-way stopcock + syringe.", "time_target": "即時"},
            {"order": 2, "phase": "Oxygen", "ja": "100%酸素フローバイまたはケージ。", "en": "100% O2 flow-by or cage.", "time_target": "並行"},
            {"order": 3, "phase": "Tension pneumo", "ja": "緊張性: 縦隔偏位 → 即時大量脱気（生命を脅かす）。胸腔ドレーン留置。", "en": "Tension: mediastinal shift → immediate large-volume evacuation (life-threatening). Place chest drain.", "time_target": "緊急"},
            {"order": 4, "phase": "Chest drain", "ja": "持続/反復性 → 胸腔ドレーン（トロカール）留置。間欠的吸引または水封ドレナージ。", "en": "Persistent/recurrent → thoracostomy tube (trocar). Intermittent aspiration or underwater seal drainage.", "time_target": "穿刺で再貯留時"},
            {"order": 5, "phase": "Imaging", "ja": "安定後: 胸部X線。外傷性: 肺/大血管損傷評価。自然気胸: 肺ブラ/ブレブ（CT推奨）。", "en": "After stabilization: thoracic radiography. Traumatic: assess lung/vessel injury. Spontaneous: bullae/blebs (CT recommended).", "time_target": "安定後"},
            {"order": 6, "phase": "Surgery", "ja": "持続空気漏出/自然気胸 → 探索的開胸（肺葉切除）。外傷性大半は保存的管理で改善。", "en": "Persistent air leak / spontaneous → exploratory thoracotomy (lung lobectomy). Most traumatic cases resolve conservatively.", "time_target": "48-72h改善なし"},
        ],
        "key_drugs": [
            {"name": "Butorphanol", "name_ja": "ブトルファノール", "dose": "0.2-0.4 mg/kg IM/IV (analgesia/mild sedation)"},
        ...
        "monitoring": ["Respiratory rate/effort", "SpO2", "Chest drain output (volume + air)", "Thoracic radiography q12-24h"],
    ...
    # ============================================================
    {
        ...
        "trigger_signs_en": ["Vomiting/diarrhea", "Polyuria", "Hyperactivity/restlessness", "Tachycardia", "Ventricular arrhythmias", "Seizures (severe)"],
        "steps": [
            {"order": 1, "phase": "Assess dose", "ja": "テオブロミン有毒量: 犬20 mg/kg（軽度）→40-50 mg/kg（心毒性）→60+ mg/kg（痙攣/死亡）。チョコレート種別で含有量大幅差。", "en": "Toxic dose theobromine: 20 mg/kg (mild) → 40-50 mg/kg (cardiac) → 60+ mg/kg (seizures/death). Varies greatly by chocolate type.", "time_target": "即時評価"},
            {"order": 2, "phase": "Decontamination", "ja": "摂取2h以内: 催吐（アポモルフィン0.03 mg/kg IV/結膜嚢）。活性炭1-2 g/kg PO（カタルシス剤と）。", "en": "If <2h post-ingestion: emesis (apomorphine 0.03 mg/kg IV/conjunctival). Activated charcoal 1-2 g/kg PO (with cathartic).", "time_target": "摂取2h以内"},
            {"order": 3, "phase": "Cardiac monitoring", "ja": "ECG連続モニタリング。VPC/VT → リドカイン 2 mg/kg IV ボーラス → CRI 25-80 μg/kg/min。", "en": "Continuous ECG. VPCs/VT → Lidocaine 2 mg/kg IV bolus → CRI 25-80 μg/kg/min.", "time_target": "入院中"},
            {"order": 4, "phase": "Seizure control", "ja": "ジアゼパム 0.5-1 mg/kg IV。反復 → フェノバルビタール。", "en": "Diazepam 0.5-1 mg/kg IV. If refractory → phenobarbital.", "time_target": "痙攣時"},
            {"order": 5, "phase": "Supportive", "ja": "輸液: 排泄促進（テオブロミンは膀胱再吸収あり → 尿カテーテル留置推奨）。制吐薬。", "en": "IV fluids to promote excretion (theobromine undergoes enterohepatic recirculation → urinary catheter recommended). Antiemetics.", "time_target": "持続"},
        ],
        "key_drugs": [
            {"name": "Apomorphine", "name_ja": "アポモルフィン", "dose": "0.03 mg/kg IV or conjunctival (emesis)"},
            {"name": "Activated charcoal", "name_ja": "活性炭", "dose": "1-2 g/kg PO (repeat q6-8h for enterohepatic recirculation)"},
            {"name": "Lidocaine", "name_ja": "リドカイン", "dose": "2 mg/kg IV bolus → CRI 25-80 μg/kg/min (arrhythmias)"},
            {"name": "Diazepam", "name_ja": "ジアゼパム", "dose": "0.5-1 mg/kg IV (seizures)"},
    ...
    # ============================================================
    {
        ...
        "trigger_signs_en": ["Vomiting", "Weakness/collapse", "Hypoglycemia (30min-12h)", "Hepatic failure (24-72h, high dose)", "Coagulopathy"],
        "steps": [
            {"order": 1, "phase": "Assess dose", "ja": "低血糖: >0.1 g/kg。肝不全: >0.5 g/kg。⚠ガム1枚≈0.3-1g。", "en": "Hypoglycemia: >0.1 g/kg. Hepatic failure: >0.5 g/kg. ⚠ One gum piece ≈ 0.3-1g.", "time_target": "即時"},
            {"order": 2, "phase": "Decontamination", "ja": "摂取30分以内かつ無症状: 催吐。⚠既に低血糖なら催吐禁忌（痙攣リスク）。", "en": "If <30min and asymptomatic: induce emesis. ⚠ If already hypoglycemic, do NOT induce vomiting (seizure risk).", "time_target": "30分以内"},
            {"order": 3, "phase": "Dextrose", "ja": "血糖確認 → 50%デキストロース 1-2 mL/kg 希釈IV → 2.5-5%デキストロースCRI維持。", "en": "Check BG → 50% dextrose 1-2 mL/kg diluted IV → maintain with 2.5-5% dextrose CRI.", "time_target": "即時"},
            {"order": 4, "phase": "Hepatoprotection", "ja": "高用量摂取 → SAMe 20 mg/kg PO q24h + シリマリン。肝酵素モニタリング72h。", "en": "High dose → SAMe 20 mg/kg PO q24h + silymarin. Monitor liver enzymes for 72h.", "time_target": ">0.5 g/kg時"},
            {"order": 5, "phase": "Monitor", "ja": "血糖q1-2h×12h。肝酵素/PT q12-24h×72h。FFP if凝固障害。", "en": "Blood glucose q1-2h ×12h. Liver enzymes/PT q12-24h ×72h. FFP if coagulopathy.", "time_target": "入院中"},
        ],
        "key_drugs": [
            {"name": "50% Dextrose", "name_ja": "50%デキストロース", "dose": "1-2 mL/kg diluted (≥1:4) IV slow → CRI 2.5-5%"},
    ...
    # ============================================================
    {
        ...
        "species": ["dog", "cat"],
        "trigger_signs_ja": ["出血（摂取2-5日後）", "紫斑/点状出血", "血尿/メレナ/喀血", "虚脱/蒼白", "呼吸困難（胸腔内出血）"],
        "trigger_signs_en": ["Hemorrhage (2-5d post-ingestion)", "Bruising/petechiae", "Hematuria/melena/hemoptysis", "Collapse/pallor", "Dyspnea (pleural hemorrhage)"],
        "steps": [
            {"order": 1, "phase": "Recent ingestion", "ja": "摂取2-4h以内: 催吐 + 活性炭。⚠既に出血中なら催吐禁忌。", "en": "If <2-4h: induce emesis + activated charcoal. ⚠ If already bleeding, do NOT induce vomiting.", "time_target": "4h以内"},
            {"order": 2, "phase": "Vitamin K1", "ja": "ビタミンK1 2.5-5 mg/kg PO q12h（食事と — 脂肪で吸収↑）。第1世代: 14日。第2世代: 4-6週。", "en": "Vitamin K1 2.5-5 mg/kg PO q12h (with fatty food for absorption). 1st gen: 14d. 2nd gen: 4-6 weeks.", "time_target": "即時開始"},
            {"order": 3, "phase": "Active bleeding", "ja": "大量出血 → FFP 10-15 mL/kg IV + 全血輸血（PCV<15%）。⚠ 輸血はK1効果待ち（6-12h）のブリッジ。", "en": "Severe hemorrhage → FFP 10-15 mL/kg IV + whole blood transfusion (PCV <15%). Transfusion bridges until K1 takes effect (6-12h).", "time_target": "出血時即時"},
            {"order": 4, "phase": "Identify product", "ja": "殺鼠剤パッケージ確認: 第1世代（ワルファリン）vs 第2世代（ブロジファクム/ブロマジオロン）。世代で治療期間が異なる。", "en": "Identify product: 1st gen (warfarin) vs 2nd gen (brodifacoum/bromadiolone). Treatment duration differs.", "time_target": "並行"},
            {"order": 5, "phase": "PT test protocol", "ja": "K1投与終了48-72h後にPT再検 → 延長していれば継続。正常なら終了。", "en": "Recheck PT 48-72h after last K1 dose → if prolonged, continue. If normal, stop.", "time_target": "K1終了後"},
        ],
        "key_drugs": [
            {"name": "Vitamin K1 (phytonadione)", "name_ja": "ビタミンK1", "dose": "2.5-5 mg/kg PO q12h with fatty food (4-6 weeks for 2nd gen)"},
            ...
            {"name": "Apomorphine", "name_ja": "アポモルフィン", "dose": "0.03 mg/kg IV (emesis if <2-4h post ingestion)"},
        ],
        "monitoring": ["PT/PTT (baseline, 48-72h after K1 stops)", "PCV/TP", "Chest/abdominal radiographs (if hemorrhage suspected)", "Platelet count"],
    ...
    # ============================================================
    {
        ...
        "trigger_signs_en": ["Vomiting/anorexia", "PU/PD", "Dehydration", "Acetone breath", "Kussmaul breathing", "Collapse"],
        "steps": [
            {"order": 1, "phase": "Fluids first", "ja": "0.9% NaCl IV: 脱水欠乏量を6-12hで補正 + 維持量。⚠脳浮腫防止のため急速補正回避。", "en": "0.9% NaCl IV: replace deficit over 6-12h + maintenance. ⚠ Avoid too-rapid correction (cerebral edema risk).", "time_target": "0-6h"},
            {"order": 2, "phase": "Insulin CRI", "ja": "レギュラーインスリン CRI: 犬 0.05-0.1 U/kg/hr IV、猫 0.05 U/kg/hr。⚠IM低用量法も可。", "en": "Regular insulin CRI: dog 0.05-0.1 U/kg/hr IV; cat 0.05 U/kg/hr. IM low-dose protocol alternative.", "time_target": "輸液開始1-2h後"},
            {"order": 3, "phase": "Glucose monitoring", "ja": "血糖q1-2h。BG 200-250 mg/dL到達 → 輸液に2.5-5%デキストロース追加（インスリン継続）。", "en": "BG q1-2h. When BG reaches 200-250 → add 2.5-5% dextrose to fluids (continue insulin).", "time_target": "q1-2h"},
            {"order": 4, "phase": "Potassium", "ja": "K+補正（ほぼ全例で必要）: インスリンがKを細胞内移行させるため。20-40 mEq/L添加。", "en": "K+ supplementation (needed in virtually all cases): insulin shifts K+ intracellular. Add 20-40 mEq/L to fluids.", "time_target": "K+結果後"},
            {"order": 5, "phase": "Phosphate", "ja": "重度低P（<1.5 mg/dL）→ KPO4 0.01-0.03 mmol/kg/hr IV。", "en": "Severe hypophosphatemia (<1.5 mg/dL) → KPO4 0.01-0.03 mmol/kg/hr IV.", "time_target": "P測定後"},
            {"order": 6, "phase": "Transition", "ja": "食欲回復+ケトン消失 → 皮下インスリン（NPH/グラルギン）に移行。CRI漸減。", "en": "When eating + ketones cleared → transition to SC insulin (NPH/glargine). Taper CRI.", "time_target": "臨床的改善後"},
        ],
        "key_drugs": [
            {"name": "Regular insulin", "name_ja": "レギュラーインスリン", "dose": "CRI 0.05-0.1 U/kg/hr IV (dog); 0.05 U/kg/hr (cat)"},
            {"name": "0.9% NaCl", "name_ja": "生理食塩水", "dose": "Deficit replacement + maintenance"},
            {"name": "KCl", "name_ja": "塩化カリウム", "dose": "20-40 mEq/L added to fluids (adjust based on serum K+)"},
            {"name": "Maropitant", "name_ja": "マロピタント", "dose": "1 mg/kg SC/IV q24h (nausea)"},
            {"name": "NaHCO3", "name_ja": "炭酸水素ナトリウム", "dose": "1-2 mEq/kg IV slow ONLY if pH <7.0 (controversial)"},
        ],
        "monitoring": ["Blood glucose q1-2h", "Serum K+ q4-6h", "Blood gas (pH, HCO3-) q6-12h", "Urine ketones", "Phosphorus q12h"],
    ...
    # ============================================================
    {
        ...
        "trigger_signs_en": ["Collapse/shock", "Bradycardia (hyperK)", "Vomiting/diarrhea", "Hypothermia", "Hypotension", "Na:K ratio <27"],
        "steps": [
            {"order": 1, "phase": "Aggressive fluids", "ja": "0.9% NaCl 20-30 mL/kg IV ボーラス（ショック輸液）。⚠LRS回避（K含有）。", "en": "0.9% NaCl 20-30 mL/kg IV bolus (shock dose). ⚠ Avoid LRS (contains K+).", "time_target": "即時"},
            {"order": 2, "phase": "Treat hyperK", "ja": "K+>7 → Ca gluconate 10% 0.5-1 mL/kg IV slow。⚠輸液のみでK+は希釈・改善されることが多い。", "en": "K+ >7 → Ca gluconate 10% 0.5-1 mL/kg IV slow. ⚠ Fluids alone often dilute K+ sufficiently.", "time_target": "ECG異常時"},
            {"order": 3, "phase": "Glucocorticoid", "ja": "デキサメタゾン 0.1-0.2 mg/kg IV（ACTH刺激試験に干渉しない唯一のGC）。", "en": "Dexamethasone 0.1-0.2 mg/kg IV (only GC that does NOT interfere with ACTH stim test).", "time_target": "輸液と同時"},
            {"order": 4, "phase": "ACTH test", "ja": "安定後: ACTH刺激試験（cortisol pre/post）で確定診断。⚠危機的状況ではテスト前に治療開始。", "en": "After stabilization: ACTH stimulation test (confirm). ⚠ In crisis, treat BEFORE testing.", "time_target": "安定後"},
            {"order": 5, "phase": "Maintenance", "ja": "確定後: DOCP (desoxycorticosterone) 2.2 mg/kg IM q25d + プレドニゾロン 0.2 mg/kg PO q24h。", "en": "After diagnosis: DOCP 2.2 mg/kg IM q25d + prednisolone 0.2 mg/kg PO q24h.", "time_target": "確定後"},
        ],
        "key_drugs": [
            {"name": "0.9% NaCl", "name_ja": "生理食塩水", "dose": "20-30 mL/kg IV bolus (shock)"},
            {"name": "Dexamethasone", "name_ja": "デキサメタゾン", "dose": "0.1-0.2 mg/kg IV (doesn't interfere with ACTH test)"},
            {"name": "Ca gluconate 10%", "name_ja": "グルコン酸Ca10%", "dose": "0.5-1 mL/kg IV slow (hyperK cardioprotection)"},
            {"name": "DOCP", "name_ja": "デスオキシコルチコステロン", "dose": "2.2 mg/kg IM q25d (long-term mineralocorticoid)"},
            {"name": "Fludrocortisone", "name_ja": "フルドロコルチゾン", "dose": "0.02 mg/kg PO q12h (alternative to DOCP)"},
        ],
        "monitoring": ["Serum electrolytes (Na+, K+, Na:K ratio) q6-12h", "Blood pressure", "ECG", "BUN/Cr", "Cortisol (ACTH stim)"],
    ...
    # ============================================================
    {
        ...
        "species": ["dog", "cat"],
        "trigger_signs_ja": ["強い陣痛30分以上で胎児排出なし", "緑色分泌物（胎盤剥離）で分娩なし", "陣痛停止>2時間", "全身状態悪化", "異常胎位触知"],
        "trigger_signs_en": [">30min strong contractions without delivery", "Green discharge (placental separation) without delivery", "No labor >2h between pups", "Deteriorating condition", "Abnormal fetal presentation"],
        "steps": [
            {"order": 1, "phase": "Medical trial", "ja": "子宮無力症: オキシトシン 0.5-2 IU IV/IM（最大3回、20分間隔）。Ca gluconate 10% 0.5-1 mL/kg IV slow（子宮収縮力増強）。", "en": "Uterine inertia: Oxytocin 0.5-2 IU IV/IM (max 3 doses, 20min apart). Ca gluconate 10% 0.5-1 mL/kg IV slow (augment contraction).", "time_target": "0-60分"},
            {"order": 2, "phase": "Assess response", "ja": "20-30分で胎児排出なし → 帝王切開。⚠閉塞性難産はオキシトシン禁忌（子宮破裂）。", "en": "No delivery within 20-30min → C-section. ⚠ Obstructive dystocia = oxytocin contraindicated (uterine rupture).", "time_target": "30分後"},
            {"order": 3, "phase": "C-section", "ja": "全身麻酔（プロポフォール導入→イソフルラン）。⚠α2/高用量オピオイドは新生児抑制。胎児取り出し後に子宮を体外に出してから閉鎖。", "en": "GA (propofol induction → isoflurane). ⚠ Avoid α2/high-dose opioids (neonatal depression). Exteriorize uterus, extract pups, then close.", "time_target": "決断後即時"},
            {"order": 4, "phase": "Neonatal resuscitation", "ja": "胎膜除去→鼻腔吸引→擦拭（タオルで刺激）→2-3回振り→ドキサプラム1-2滴舌下（最終手段）。", "en": "Remove membranes → suction nose → rub vigorously (towel stimulation) → gentle swing 2-3x → doxapram 1-2 drops sublingual (last resort).", "time_target": "取り出し後30秒以内"},
            {"order": 5, "phase": "Post-op dam", "ja": "オキシトシン 0.5-2 IU IM（子宮内容排出）。抗菌薬。メロキシカム0.1 mg/kg（授乳への影響最小）。", "en": "Oxytocin 0.5-2 IU IM (uterine involution). Antibiotics. Meloxicam 0.1 mg/kg (minimal lactation impact).", "time_target": "術後"},
        ],
        "key_drugs": [
            {"name": "Oxytocin", "name_ja": "オキシトシン", "dose": "0.5-2 IU IV/IM q20min (max 3 doses; NOT for obstructive)"},
            {"name": "Ca gluconate 10%", "name_ja": "グルコン酸Ca 10%", "dose": "0.5-1 mL/kg IV slow (augment contractions)"},
            {"name": "Propofol", "name_ja": "プロポフォール", "dose": "4-6 mg/kg IV (induction for C-section)"},
            {"name": "Doxapram", "name_ja": "ドキサプラム", "dose": "1-2 drops sublingual to neonates (respiratory stimulant)"},
        ],
        "monitoring": ["Fetal heart rates (US: >180 bpm normal; <150 distress)", "Maternal temperature/HR/BP", "Time between pups (<2h)", "Number of pups delivered vs expected (radiograph)"],
