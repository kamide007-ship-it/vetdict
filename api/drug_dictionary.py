...
DRUGS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "0.01-0.02 mg/kg IV/IM/SC q6-8h for mild-to-moderate perioperative pain; sustained release: 0.12 mg/kg SC q72h. Note: unlike cats, OTM (oral transmucosal) absorption in dogs is poor - buccal dosing needs approx. 0.12 mg/kg to reach effective levels (Ko 2011 JAVMA), so parenteral routes are preferred.",
                "dosage_ja": "軽度～中等度の周術期疼痛に 0.01-0.02 mg/kg 静注/筋注/皮下 6-8時間毎。持続型: 0.12 mg/kg 皮下 72時間毎。注意: 猫と異なり犬の口腔粘膜（OTM）吸収は不良 — 頬粘膜投与は約0.12 mg/kgでようやく有効域（Ko 2011 JAVMA）のため、犬では注射経路を優先する。",
                "notes": "Partial mu-agonist with a ceiling effect - escalate to a full mu-agonist (methadone, fentanyl) for severe pain rather than increasing the dose.",
                "notes_ja": "天井効果のある部分μ作動薬 — 重度疼痛では増量ではなく完全μ作動薬（メサドン・フェンタニル）へ切り替える。",
            },
            "cat": {
                "safe": True,
                "dosage": "Injectable: 0.02-0.04 mg/kg IV/IM q6-8h (0.02 mg/kg q6-8h is the common clinical standard). OTM (oral transmucosal): 0.02-0.04 mg/kg dripped onto the buccal mucosa q6-8h - the alkaline feline saliva gives near-parenteral bioavailability (Robertson 2005), making OTM ideal for owner-administered continuation of postoperative analgesia at home. High-concentration 24-hour formulation (Simbadol 1.8 mg/mL): 0.24 mg/kg SC q24h, first dose approx. 1 h before surgery, for up to 3 days (FDA-approved for feline postoperative pain). Simbadol is 6x the concentration of standard 0.3 mg/mL buprenorphine - never interchange volumes between products.",
                "dosage_ja": "注射: 0.02-0.04 mg/kg 静注/筋注 6-8時間毎（臨床標準は0.02 mg/kg q6-8h）。口腔粘膜（OTM）: 0.02-0.04 mg/kg を頬粘膜に滴下 6-8時間毎 — 猫はアルカリ性唾液のため口腔粘膜バイオアベイラビリティが注射に近く（Robertson 2005）、自宅での術後鎮痛継続（飼い主投与）に最適。高濃度24時間持続製剤（Simbadol 1.8 mg/mL）: 0.24 mg/kg 皮下 24時間毎（初回は手術の約1時間前、最長3日間 — 猫の術後疼痛でFDA承認）。Simbadolは通常製剤（0.3 mg/mL）の6倍濃度 — 製剤間で液量を流用しない。",
                "notes": "Cornerstone feline analgesic (mild-to-moderate pain); ceiling effect means severe pain needs a full mu-agonist (methadone, fentanyl CRI) instead of dose escalation. Opioids can cause post-anesthetic hyperthermia (>40C) in cats - monitor temperature after Simbadol or repeated dosing. Mydriasis and euphoria (kneading, rolling) are normal effects, not toxicity.",
                "notes_ja": "猫鎮痛の中核薬（軽度～中等度疼痛）。天井効果があるため重度疼痛は増量ではなく完全μ作動薬（メサドン・フェンタニルCRI）へ。猫はオピオイドで麻酔後高体温（>40℃）を起こしうる — Simbadol投与後・反復投与時は体温を監視。散瞳と多幸感（ふみふみ・ゴロゴロ転がる）は正常反応であり中毒ではない。",
            },
            "horse": {
                "safe": True,
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "0.25-0.3 mg/kg PO q12h (total 0.5-0.6 mg/kg/day), given approx. 1 h before food (fasted absorption is better). Start in preclinical MMVD at stage B2 = murmur >=3/6 PLUS echocardiographic cardiomegaly (LA/Ao >=1.6, LVIDDN >=1.7, VHS >10.5) - EPIC trial (Boswood 2016 JVIM): prolonged the preclinical period to CHF/cardiac death by approx. 15 months (median 1228 vs 766 days). NOT indicated at stage B1 (murmur without cardiomegaly). Preclinical DCM (Dobermans): prolongs time to CHF/sudden death and survival (PROTECT, Summerfield 2012 JVIM). In acute CHF, q8h dosing (off-label) is often used during hospitalization.",
                "dosage_ja": "0.25-0.3 mg/kg 経口 12時間毎（合計0.5-0.6 mg/kg/日）を食事の約1時間前（空腹時の方が吸収良好）。前臨床MMVDでは【ステージB2＝心雑音3/6以上＋心エコーでの心拡大（LA/Ao ≥1.6・LVIDDN ≥1.7・VHS >10.5）】を満たした時点で開始 — EPIC試験（Boswood 2016 JVIM）: CHF発症/心臓死までの前臨床期間を約15ヶ月延長（中央値1228日 vs 766日）。心拡大のないB1（雑音のみ）では適応なし。前臨床DCM（ドーベルマン）: CHF/突然死までの期間と生存を有意に延長（PROTECT試験、Summerfield 2012 JVIM）。急性CHF入院中は8時間毎（適応外）への増強もよく用いられる。",
                "notes": "From CHF stage C onward, combine with furosemide + spironolactone +/- ACE inhibitor as standard quadruple/triple therapy. Diagnose B2 by echocardiography - starting on radiographic VHS alone risks overtreatment of B1 dogs.",
                "notes_ja": "CHFステージC以降はフロセミド＋スピロノラクトン±ACE阻害薬との併用が標準。B2判定は心エコー計測が基本 — X線VHSのみでの開始はB1犬への過剰治療リスク。",
            },
            "cat": {
                "safe": True,
                "dosage": "0.625-1.25 mg/cat PO q12h (off-label). Consider in refractory CHF WITHOUT dynamic left ventricular outflow tract obstruction - retrospective data suggest longer survival in pimobendan-treated cats with CHF (Reina-Doreste 2014 JAVMA). Confirm absence of LVOTO/SAM by echocardiography before starting.",
                "dosage_ja": "0.625-1.25 mg/頭 経口 12時間毎（適応外使用）。【動的左室流出路閉塞（LVOTO）のない】難治性CHFで考慮 — 後ろ向き研究でピモベンダン投与猫のCHF生存期間延長が示唆（Reina-Doreste 2014 JAVMA）。開始前に心エコーでLVOTO/SAMがないことを確認する。",
                "notes": "Avoid in obstructive HCM (HOCM) - positive inotropy can worsen the dynamic obstruction. Use is controversial; reserve for cardiologist-guided refractory CHF cases.",
                "notes_ja": "閉塞性HCM（HOCM）では使用を避ける — 陽性変力作用が動的閉塞を悪化させうる。使用には議論があり、循環器評価下の難治性CHF例に限定する。",
            },
        },
    ...
    },
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "0.05-0.2 mg/kg PO q24h, starting at 0.05-0.1 mg/kg and titrating to blood pressure. In dogs, amlodipine monotherapy activates RAAS - combine with an ACE inhibitor or telmisartan rather than using it alone (ACVIM 2018 hypertension consensus).",
                "dosage_ja": "0.05-0.2 mg/kg 経口 24時間毎（0.05-0.1 mg/kgから開始し血圧で漸増）。犬ではアムロジピン単剤はRAASを活性化するため、単独ではなくACE阻害薬またはテルミサルタンとの併用が推奨（ACVIM 2018 高血圧コンセンサス）。",
                "notes": "Recheck blood pressure 1-2 weeks after each dose change.",
                "notes_ja": "用量変更のたびに1-2週後に血圧を再測定。",
            },
            "cat": {
                "safe": True,
                "dosage": "FIRST-LINE antihypertensive in cats (ACVIM 2018 consensus, Acierno 2018 JVIM). Start 0.625 mg/cat PO q24h; if SBP >=200 mmHg or target-organ damage is present, may start at 1.25 mg. Recheck SBP at 1-2 week intervals and titrate to 1.25 mg (max 2.5 mg/cat q24h), targeting SBP <160 mmHg (ideally <140). Before diagnosing, exclude situational (white-coat) hypertension: acclimatize, average multiple readings in a quiet room, or confirm target-organ damage (hypertensive retinopathy, LVH, proteinuria).",
                "dosage_ja": "猫の全身性高血圧の第一選択薬（ACVIM 2018 コンセンサス、Acierno 2018 JVIM）。開始: 0.625 mg/頭 経口 24時間毎（SBP ≥200 mmHg または標的臓器障害ありは1.25 mg開始可）。1-2週間隔でSBPを再測定し、SBP <160 mmHg（理想 <140）を目標に 1.25 mg → 最大2.5 mg/頭へ漸増。診断時は状況性高血圧（白衣効果）を除外 — 静かな部屋で馴化後に複数回測定の平均、または標的臓器障害（高血圧性網膜症・左室肥大・蛋白尿）で裏付ける。",
                "notes": "Work up and treat the underlying cause in parallel (CKD, hyperthyroidism, primary hyperaldosteronism). Add telmisartan or benazepril if proteinuric. Acute hypertensive retinopathy (sudden blindness, fixed dilated pupils) is an emergency - start antihypertensive therapy the same day; retinal reattachment and some vision can return with prompt control. Gingival hyperplasia is a rare long-term effect.",
                "notes_ja": "基礎疾患（CKD・甲状腺機能亢進症・原発性アルドステロン症）の検索と治療を並行。蛋白尿併発例はテルミサルタン/ベナゼプリルを追加。急性高血圧性網膜症（突然の失明・散瞳固定）は緊急 — 即日降圧を開始（迅速な降圧で網膜復位・一部視力回復の可能性）。歯肉増殖は長期投与でまれに発生。",
            },
        },
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "species_info": {
            "cat": {
                "safe": True,
                "dosage": "Start LOW: 1.25-2.5 mg/cat PO q12h (2.5 mg q12h for markedly elevated T4) - low starting doses reduce adverse events (AAFP 2016 hyperthyroidism guidelines). Recheck T4 plus CBC/platelets, liver enzymes and renal values 2-3 weeks after starting or any dose change; adjust in 1.25-2.5 mg/day increments targeting T4 in the lower half of the reference range; once stable, monitor every 3-6 months. Transdermal PLO gel: 2.5-5 mg applied to the inner ear pinna q12h for cats that cannot be pilled - effective with slower onset and fewer GI effects than oral (Sartor 2004 JVIM); apply with a finger cot/glove, alternate ears, wipe residue before each dose.",
                "dosage_ja": "低用量から開始: 1.25-2.5 mg/頭 経口 12時間毎（T4著明高値例は2.5 mg q12h）— 低用量開始が副作用を減らす（AAFP 2016 甲状腺機能亢進症ガイドライン）。開始・用量変更の2-3週後にT4＋CBC・血小板・肝酵素・腎数値を再検し、T4が基準範囲の下半分に入るよう1.25-2.5 mg/日刻みで調整。安定後は3-6ヶ月毎にモニタリング。経皮PLOゲル: 投薬困難猫の代替として 2.5-5 mg を耳介内側に12時間毎 — 有効だが経口よりやや発現が遅く、消化器副作用は少ない（Sartor 2004 JVIM）。指サック/手袋で塗布し、左右の耳を交互に、次回投与前に残薬を拭き取る。",
                "notes": "Adverse effects cluster in the first 3 months: GI upset (~10%, often transient), facial excoriation (2-3%), hepatopathy (~2%), blood dyscrasias (neutropenia/thrombocytopenia, <5%). For excoriation, hepatopathy or dyscrasia STOP the drug (recurs on rechallenge) and switch to I-131, thyroidectomy or an iodine-restricted diet. Renal masking: hyperthyroidism inflates GFR - azotemia is unmasked in ~15-40% of cats after euthyroidism. Do not stop for azotemia alone; instead avoid iatrogenic hypothyroidism (low T4 + azotemia shortens survival - Williams 2010 JVIM) by reducing the dose. A methimazole trial before I-131 also serves to assess renal reserve.",
                "notes_ja": "副作用は開始後3ヶ月以内に集中: 消化器症状（約10%・多くは一過性）、顔面掻爬（2-3%）、肝障害（約2%）、血球減少（好中球減少・血小板減少 <5%）。顔面掻爬・肝障害・血球減少は【中止】（再投与で再発するため減量では不可）し、I-131・甲状腺摘出・ヨウ素制限食へ切り替える。腎マスキング: 甲状腺機能亢進はGFRを見かけ上増加させるため、正甲状腺化後に約15-40%で高窒素血症が顕在化。高窒素血症のみでは中止せず、医原性甲状腺機能低下（低T4＋高窒素血症は生存を短縮 — Williams 2010 JVIM）を避けて減量で対応。I-131前のメチマゾール試験投与は腎予備能の評価も兼ねる。",
            },
            "dog": {
                "safe": True,
    ...
    },
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "Start 1-2 mg/kg PO q24h WITH FOOD (absorption is markedly better fed). Split the total to q12h if signs recur in the evening or with concurrent diabetes (short duration of effect in some dogs - Vaughan 2008 JAVMA). ACTH STIMULATION MONITORING: perform 10-14 days after starting or any dose change, timed 4-6 h POST-PILL; targets = clinical signs controlled + post-ACTH cortisol 1.45-5.4 ug/dL (40-150 nmol/L). Recheck at 30 days, 90 days, then every 3 months, with electrolytes (Na/K) each time. Alternative where cosyntropin is scarce: pre-pill cortisol - >1.4 ug/dL with good clinical control indicates no over-suppression (Macfarlane 2016 Vet Rec).",
                "dosage_ja": "開始: 1-2 mg/kg 経口 24時間毎・【必ず食事と共に】（空腹時は吸収が大幅に低下）。夕方に症状が再燃する例・糖尿病併発例は総量を分割して12時間毎へ（作用持続が短い犬がいる — Vaughan 2008 JAVMA）。【ACTH刺激試験モニタリング】開始・用量変更の10-14日後に、【投薬後4-6時間】のタイミングで実施。目標＝臨床徴候の改善＋刺激後コルチゾール 1.45-5.4 μg/dL（40-150 nmol/L）。以後30日・90日・その後3ヶ月毎に、毎回電解質（Na/K）とセットで再検。コシントロピン入手困難時の代替: 投薬前コルチゾール（pre-pill）>1.4 μg/dL かつ臨床良好なら過剰抑制なしと判断可（Macfarlane 2016 Vet Rec）。",
                "notes": "STOP the drug and recheck if post-ACTH cortisol falls below 1.45 ug/dL or the dog develops vomiting, diarrhoea, anorexia or collapse (iatrogenic hypocortisolism; rarely adrenal necrosis - Chapman 2004); restart at a lower dose after recovery. Counsel owners to SKIP the dose on any day the dog is vomiting or not eating, and to call rather than push through. Expect insulin requirements to fall in diabetic dogs once cortisol is controlled.",
                "notes_ja": "刺激後コルチゾールが1.45 μg/dL未満、または嘔吐・下痢・食欲不振・虚脱が出たら【投薬を中止して再診】（医原性コルチゾール欠乏。まれに副腎壊死 — Chapman 2004）。回復確認後に減量して再開する。飼い主には「嘔吐・食欲不振の日は投薬をスキップして連絡」を指導（無理に続けない）。糖尿病併発犬はコルチゾール制御後にインスリン要求量が低下する — 低血糖を予期してインスリン減量を計画。",
            },
            "ferret": {
                "safe": True,
                "dosage": "1-2 mg/kg PO q12-24h",
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "First-line maintenance anticonvulsant (ACVIM consensus, Podell 2016 JVIM): start 2.5-3 mg/kg PO q12h. SERUM MONITORING: measure 2 weeks after starting or any dose change (steady state approx. 10-14 days; at steady state sample timing barely matters - Levitski & Trepanier 2000 JAVMA), target 15-35 ug/mL (>35 raises hepatotoxicity risk). Dose adjustment is proportional: new dose = current dose x (target / measured concentration). Once stable, recheck level + CBC + liver panel every 6 months - an induced ALP rise is expected and NOT hepatopathy by itself; judge true liver injury by ALT and bile acids. Cluster seizures/status loading: 12-24 mg/kg IV total, given in 2-4 mg/kg increments to effect.",
                "dosage_ja": "犬の維持抗てんかん薬の第一選択（ACVIMコンセンサス、Podell 2016 JVIM）: 2.5-3 mg/kg 経口 12時間毎で開始。【血中濃度モニタリング】開始・用量変更の2週間後に測定（定常状態は約10-14日。定常状態では採血タイミングの影響は小さい — Levitski & Trepanier 2000 JAVMA）。目標 15-35 μg/mL（>35 で肝毒性リスク増）。用量調整は比例計算: 新用量＝現用量×（目標濃度÷実測濃度）。安定後は6ヶ月毎に血中濃度＋CBC＋肝パネル — 酵素誘導によるALP上昇は予想内でそれ自体は肝障害ではない。真の肝障害はALT・胆汁酸で評価する。群発発作・てんかん重積のローディング: 合計12-24 mg/kg 静注を2-4 mg/kg刻みで効果をみながら分割投与。",
                "notes": "Sedation, ataxia, PU/PD and polyphagia in the first 1-2 weeks usually wane with tolerance. Idiosyncratic reactions (cytopenias, superficial necrolytic dermatitis) cluster in the first 3 months - CBC at 1 and 3 months after starting. Potent CYP450 inducer: lowers levels of co-administered drugs (cyclosporine, zonisamide, levetiracetam etc.). Never stop abruptly - taper over months to avoid withdrawal seizures.",
                "notes_ja": "開始1-2週の鎮静・失調・多飲多尿・多食は多くが耐性化して軽減する。特異体質反応（血球減少・表在性壊死性皮膚炎）は開始3ヶ月以内に集中 — 開始後1・3ヶ月にCBC推奨。強力なCYP450誘導薬で併用薬（シクロスポリン・ゾニサミド・レベチラセタム等）の血中濃度を低下させる。突然の中止は退薬発作を招く — 中止は数ヶ月かけて漸減。",
            },
            "cat": {
                "safe": True,
                "dosage": "FIRST-LINE anticonvulsant in cats (ISFM; Finnerty 2014 JFMS): 1.5-2.5 mg/kg PO q12h. Measure serum level 2 weeks after starting/dose change, target 15-30 ug/mL; then level + CBC + liver values every 6 months.",
                "dosage_ja": "猫の抗てんかん薬の第一選択（ISFM; Finnerty 2014 JFMS）: 1.5-2.5 mg/kg 経口 12時間毎。開始・用量変更の2週間後に血中濃度を測定し目標 15-30 μg/mL。以後は6ヶ月毎に血中濃度＋CBC＋肝機能値。",
                "notes": "Hepatotoxicity is rarer than in dogs, but watch for idiosyncratic facial pruritus, cytopenias and lymphadenopathy (pseudolymphoma) - resolve on discontinuation.",
                "notes_ja": "肝毒性は犬よりまれだが、特異体質の顔面掻痒・血球減少・リンパ節腫脹（偽リンパ腫）に注意 — いずれも休薬で回復する。",
            },
            "horse": {
                "safe": True,
    ...
    },
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "Maintenance: 20 mg/kg PO q8h (regular tablets) or extended-release (XR) 30 mg/kg PO q12h - never crush XR. Dogs ALSO ON PHENOBARBITAL clear levetiracetam roughly twice as fast (Moore 2011 AJVR) - if response is inadequate, increase the dose (keep q8h) rather than assuming drug failure. Cluster-seizure pulse protocol: 60 mg/kg PO after a seizure, then 20 mg/kg q8h until 48 h seizure-free (Packer 2015). Status/hospital bridge: 30-60 mg/kg IV. Renally excreted - reduce dose/extend interval in renal impairment.",
                "dosage_ja": "維持: 20 mg/kg 経口 8時間毎（通常錠）または徐放錠(XR) 30 mg/kg 12時間毎（XRは絶対に砕かない）。【フェノバルビタール併用犬】クリアランスが約2倍に増大し血中濃度が半減する（Moore 2011 AJVR）— 効果不十分なら「効かない」と判断する前に増量（間隔はq8hのまま）。群発発作のパルスプロトコル: 発作後に60 mg/kg 経口 → 20 mg/kg 8時間毎を発作なし48時間まで継続（Packer 2015）。てんかん重積・入院時のブリッジ: 30-60 mg/kg 静注。腎排泄 — 腎機能低下では減量・間隔延長。",
                "notes": "Wide therapeutic index - routine serum level monitoring is usually unnecessary (titrate clinically; a practical difference from phenobarbital/zonisamide). A 'honeymoon effect' (waning efficacy over months on monotherapy) is reported (Volk 2008) - if control fades, add/switch rather than escalating indefinitely. Side effects are usually limited to mild transient sedation.",
                "notes_ja": "治療域が広く、定期的な血中濃度測定は通常不要（臨床効果で調整 — 濃度管理が必須のフェノバルビタール/ゾニサミドとの実務上の違い）。単剤では数ヶ月で効果が減弱する「ハネムーン効果」の報告あり（Volk 2008）— 効果減弱時は無限に増量せず追加/切替を検討。副作用は軽度で一過性の鎮静程度が中心。",
            },
            "cat": {
                "safe": True,
                "dosage": "20 mg/kg PO q8h (Bailey 2008 JVIM) - increasingly used first-line in cats for its safety and lack of hepatic metabolism. Renally excreted - reduce the dose in CKD cats. XR tablets are impractical in cats (tablet size, cannot be split).",
                "dosage_ja": "20 mg/kg 経口 8時間毎（Bailey 2008 JVIM）— 安全性と肝代謝を受けない点から猫の第一選択として使用増加中。腎排泄のためCKD猫では減量。徐放錠は錠剤サイズが大きく分割不可のため猫では実用性が低い。",
                "notes": "Well tolerated; transient lethargy and inappetence are the main effects.",
                "notes_ja": "忍容性良好。主な副作用は一過性の傾眠と食欲低下。",
            },
        },
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "Atopic dermatitis: 5 mg/kg PO q24h (Atopica, microemulsion). ONSET IS SLOW - expect 4-8 weeks for full effect (Olivry, ICADA 2015); bridge the gap with oclacitinib or a short prednisolone course. Once controlled, TAPER: q24h -> q48h -> twice weekly (Steffan 2006 systematic review: about half of dogs maintain on q48h or less). GI upset in approx. 25% during the first 2 weeks - mitigate by freezing capsules, giving with a small meal, or a short antiemetic course (label says fasted, but feeding reduces absorption only approx. 20% and rarely matters clinically). Cost-sparing option: ketoconazole 2.5-5 mg/kg co-administration allows roughly halving the ciclosporin dose (deliberate CYP3A inhibition - manage as an interaction). Immune-mediated disease (IMHA, PLE etc.): 5 mg/kg q12h - consider blood level/pharmacodynamic monitoring at these doses.",
                "dosage_ja": "アトピー性皮膚炎: 5 mg/kg 経口 24時間毎（アトピカ、マイクロエマルジョン型）。【効果発現は4-8週と遅い】（Olivry, ICADA 2015）— 即効性のオクラシチニブや短期プレドニゾロンでブリッジする。効果達成後は【減量】: q24h → q48h → 週2回へ段階的に（Steffan 2006 システマティックレビュー: 約半数の犬はq48h以下で維持可能）。開始2週間の消化器症状（約25%）はカプセル冷凍・少量の食事との同時投与・短期制吐薬で軽減可（ラベルは空腹時だが、食事で吸収は約20%低下する程度で臨床的問題はまれ）。コスト対策: ケトコナゾール 2.5-5 mg/kg 併用でシクロスポリン用量を約半分に節約可（CYP3A阻害の治療的利用 — 相互作用として管理下で行う）。免疫介在性疾患（IMHA・PLE等）: 5 mg/kg 12時間毎 — この用量域では血中濃度/薬力学モニタリングを考慮。",
                "notes": "For atopy, blood level monitoring is NOT needed - titrate on clinical response. Gingival hyperplasia, papillomas and hirsutism are dose-related and reversible on reduction. Avoid raw-meat diets during treatment; use caution with live vaccines.",
                "notes_ja": "アトピー用途では血中濃度測定は不要 — 臨床効果で調整する。歯肉増殖・乳頭腫・多毛は用量依存性で減量により可逆。治療中の生肉給餌は避け、生ワクチンは慎重に。",
            },
            "cat": {
                "safe": True,
                "dosage": "Feline atopic skin syndrome: 7 mg/kg PO q24h (Atopica for Cats label); taper to q48h-twice weekly once controlled. Also used for stomatitis and IBD (adjunct).",
                "dosage_ja": "猫アトピー性皮膚症候群: 7 mg/kg 経口 24時間毎（Atopica for Cats ラベル）。効果達成後は q48h～週2回へ漸減。口内炎・IBDの補助にも使用。",
                "notes": "TOXOPLASMA risk is the defining feline caution: fatal disseminated toxoplasmosis is reported in cats starting ciclosporin (Lappin 2015) - keep the cat indoors, feed NO raw meat, and consider T. gondii serology before starting (seronegative cats are the highest-risk group for primary infection). Screen FeLV/FIV before long-term use.",
                "notes_ja": "猫で最重要の注意は【トキソプラズマ】: シクロスポリン開始後の致死的全身性トキソプラズマ症が報告されている（Lappin 2015）— 完全室内飼育・生肉給餌の禁止を徹底し、開始前のT. gondii抗体検査を考慮（抗体陰性猫は初感染リスクが最も高い群）。長期使用前にFeLV/FIV検査を。",
            },
            "horse": {
                "safe": True,
                "dosage": "5-10 mg/kg PO q24h",
        ...
        "contraindications": "Active neoplasia or history of malignancy. Unmanaged concurrent ketoconazole/itraconazole (markedly increases levels - combine only deliberately with ciclosporin dose reduction).",
        "contraindications_ja": "活動性腫瘍・悪性腫瘍の既往。管理外のケトコナゾール/イトラコナゾール併用（血中濃度が著明に上昇 — 併用はシクロスポリン減量を前提とした意図的な場合のみ）。",
    },
    {
        "id": "chlorambucil",
    ...
    # ══════════════════════════════════════════════════════════════════════
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "0.4-0.6 mg/kg PO q12h for a MAXIMUM of 14 days, then step down to q24h for maintenance - long-term q12h is off-label and raises infection/demodicosis risk (label). Rapid onset: pruritus falls within 4 h, significant by 24 h (Cosgrove 2013 Vet Dermatol). Dogs must be >=12 months old (field trials in younger dogs showed demodicosis and serious infections). Long-term use: CBC/chemistry once or twice yearly. Unlike steroids, oclacitinib does NOT interfere with intradermal or serum IgE allergy testing - it can be continued through allergy work-up and immunotherapy (Olivry, ICADA 2015).",
                "dosage_ja": "0.4-0.6 mg/kg 経口 12時間毎を【最長14日間】→ 以後は必ず 24時間毎の維持へステップダウン（q12hの長期継続はラベル外で感染症・毛包虫症リスクを上げる）。速効性: 投与4時間で掻痒が低下し24時間で有意（Cosgrove 2013 Vet Dermatol）。【12ヶ月齢以上】が条件（若齢犬の試験で毛包虫症・重篤感染が増加）。長期投与では年1-2回のCBC・生化学。ステロイドと異なり皮内試験・血清IgE検査に干渉しない — アレルゲン精密検査・減感作療法の期間中も継続できる（Olivry, ICADA 2015）。",
                "notes": "Pruritus rebounds quickly on discontinuation - it is symptomatic control, so run flea control, diet trial and allergen management in parallel. Evaluate for demodicosis, serious infection and neoplasia history before starting.",
                "notes_ja": "中止すると掻痒は速やかに再燃する（対症薬）— ノミ管理・除去食試験・アレルゲン管理を並行して進める。開始前に毛包虫症・重篤感染・腫瘍既往を評価。",
            },
            "cat": {
                "safe": True,
                "dosage": "Off-label: 0.4-1 mg/kg PO q12h. Cats metabolise oclacitinib faster than dogs and usually need CONTINUED twice-daily dosing - the canine step-down to q24h is often insufficient in cats (Lopes 2019 BMC Vet Res). Evidence remains limited; glucocorticoids and ciclosporin stay first-line for feline allergic dermatitis.",
                "dosage_ja": "適応外: 0.4-1 mg/kg 経口 12時間毎。猫は犬より代謝が速く、【12時間毎の継続】が必要なことが多い — 犬式のq24hへの減量では効果不十分になりやすい（Lopes 2019 BMC Vet Res）。エビデンスは限定的で、猫のアレルギー性皮膚炎ではステロイド・シクロスポリンが引き続き第一選択。",
                "notes": "Monitor CBC with long-term use; screen FeLV/FIV before chronic immunomodulation.",
                "notes_ja": "長期使用ではCBCをモニタリング。慢性の免疫調節前にFeLV/FIVを確認。",
            },
        },
        "side_effects": [
    ...
    },
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "0.6 mg/kg PO q24h (max 30 mg/day); practical banding: 1.88-3.75 mg/dog (<10 kg), 3.75-7.5 mg (10-25 kg), 15-30 mg (>25 kg). Adjunct appetite stimulant/antiemetic for anorexia of chemotherapy, CKD, hepatic and GI disease. Reduce to q48h in significant hepatic or renal impairment.",
                "dosage_ja": "0.6 mg/kg 経口 24時間毎（上限30 mg/日）。実用的な体重帯: 1.88-3.75 mg/頭（<10 kg）、3.75-7.5 mg（10-25 kg）、15-30 mg（>25 kg）。化学療法・CKD・肝疾患・消化器疾患に伴う食欲不振の補助（制吐作用も併有）。重度の肝・腎障害では48時間毎に減頻。",
                "notes": "Appetite stimulation is symptomatic support - always work up and treat the underlying disease in parallel. Higher doses increase serotonergic side effects (vocalization, agitation, tremor) without added benefit.",
                "notes_ja": "食欲刺激は対症的サポート — 基礎疾患の診断・治療と必ず並行する。増量しても効果は頭打ちで、セロトニン性副作用（発声・興奮・振戦）のみ増える。",
            },
            "cat": {
                "safe": True,
                "dosage": "Oral: 1.88 mg/cat PO q24h in cats with normal renal/hepatic function; CKD cats: 1.88 mg/cat PO q48h (half-life is markedly prolonged in CKD - Quimby 2011 AJVR PK; Quimby 2013 JVIM placebo-controlled crossover: 1.88 mg q48h significantly increased appetite and body weight in CKD cats); hepatic disease: extend to q72h. Doses >=3.75 mg add side effects (vocalization, agitation, tremor) without added appetite benefit. Transdermal (Mirataz 2% ointment, FDA NADA 141-481): 2 mg = 1.5-inch (approx. 3.8 cm) ribbon applied to the inner ear pinna q24h for 14 days for unintended weight loss (Poole 2019 J Vet Pharmacol Ther RCT: significant weight gain vs placebo) - useful when oral dosing is impossible (nausea, oral pain, fractious cat).",
                "dosage_ja": "経口: 腎・肝機能が正常な猫は 1.88 mg/頭 経口 24時間毎。CKD猫は半減期が著明に延長するため 1.88 mg/頭 48時間毎（Quimby 2011 AJVR 薬物動態; Quimby 2013 JVIM プラセボ対照クロスオーバー: CKD猫で1.88 mg q48hが食欲・体重を有意に改善）。肝疾患は72時間毎へ延長。3.75 mg以上は食欲効果が頭打ちで副作用（発声・興奮・振戦）のみ増加。経皮（ミラタズ2%軟膏、FDA承認 NADA 141-481）: 2 mg＝1.5インチ（約3.8 cm）のリボンを耳介内側に24時間毎×14日 — 意図しない体重減少に対しプラセボ対照RCTで有意な体重増加（Poole 2019 J Vet Pharmacol Ther）。悪心・口腔痛・投薬困難猫など経口不能例に有用。",
                "notes": "Symptomatic support - diagnose and treat the underlying cause (CKD, hepatic lipidosis, neoplasia) in parallel; in vomiting cats the 5-HT3 antagonism adds a useful antiemetic effect. Overdose causes serotonin syndrome (vocalization, agitation, tremor, tachycardia) - cyproheptadine is the antidote. Mirataz: wear gloves, alternate ears daily, wipe residue before the next dose, and prevent owners/other pets from contacting the treated pinna.",
                "notes_ja": "対症的サポート — 基礎疾患（CKD・肝リピドーシス・腫瘍等）の診断・治療と並行する。嘔吐併発猫では5-HT3拮抗による制吐作用も有用。過量でセロトニン症候群（発声・興奮・振戦・頻脈）— 拮抗薬はシプロヘプタジン。ミラタズは手袋着用・左右の耳を毎日交互・次回投与前に残薬を拭き取り・投与部位に飼い主や同居動物が触れないよう指導。",
            },
            "horse": {
                "safe": False,
                "dosage": "N/A",
    ...
    },
    {
        ...
        "species_info": {
            "dog": {
                "safe": True,
                "dosage": "Monotherapy or non-phenobarbital add-on: start 5 mg/kg PO q12h. Dogs ALSO ON PHENOBARBITAL: start 10 mg/kg q12h - PB induction roughly doubles zonisamide clearance (Orito 2008 J Vet Pharmacol Ther). SERUM MONITORING: measure 1-2 weeks after starting/dose change (steady state approx. 1 week), target 10-40 ug/mL. Veterinary products are approved for dogs in Japan (Consave/コンセーブ), making it a mainstay alongside phenobarbital in Japanese practice.",
                "dosage_ja": "単剤・非フェノバルビタール併用: 5 mg/kg 経口 12時間毎で開始。【フェノバルビタール併用犬は 10 mg/kg q12h で開始】— PBの酵素誘導でゾニサミドのクリアランスが約2倍になる（Orito 2008 J Vet Pharmacol Ther）。【血中濃度モニタリング】開始・用量変更の1-2週後に測定（定常状態は約1週間）、目標 10-40 μg/mL。日本では犬用製剤（コンセーブ）が承認されており、本邦の実臨床ではフェノバルビタールと並ぶ主力。",
                "notes": "Sulfonamide derivative - avoid with a history of sulfa hypersensitivity. Rare idiosyncratic reactions: hepatopathy, KCS, renal tubular acidosis - check liver enzymes and tear production after starting. Inappetence is the commonest side effect; giving with food helps.",
                "notes_ja": "サルファ剤誘導体 — サルファ過敏の既往では回避。まれな特異体質反応（肝障害・乾性角結膜炎・腎尿細管性アシドーシス）— 開始後に肝酵素と涙液量（シルマー値）を確認。最多の副作用は食欲低下で、食事と共に投与すると軽減しやすい。",
            },
            "cat": {
                "safe": True,
                "dosage": "5-10 mg/kg PO q24h - the feline half-life is long (approx. 33 h; Hasegawa 2008 J Feline Med Surg), so ONCE-daily dosing suffices, unlike dogs. Start at the low end and titrate up.",
                "dosage_ja": "5-10 mg/kg 経口 【24時間毎】— 猫は半減期が約33時間と長く（Hasegawa 2008 J Feline Med Surg）、犬と異なり1日1回で足りる。低用量から開始して漸増。",
                "notes": "About half of cats show inappetence, vomiting or somnolence at higher doses - dose-related; reduce rather than stop if control is good.",
                "notes_ja": "高用量では約半数の猫に食欲不振・嘔吐・傾眠がみられる — 用量依存性のため、発作が抑えられていれば中止でなく減量で対応。",
            },
            "horse": {
                "safe": True,
                "dosage": "Not well established",
