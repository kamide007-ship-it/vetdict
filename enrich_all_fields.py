    ...
    def enrich_dataset(self, input_file: str, output_file: str) -> Dict[str, int]:
        ...
        for i, record in enumerate(data):
            ...
            if self._is_content_inadequate(record.get('description'), 100):
                ...
                record['description_ja'] = f"{disease_name}は{species_name}に影響する疾患です。専門的な獣医学的診断と治療が必要です。"
            ...
            if self._is_content_inadequate(record.get('clinical_signs'), 100):
                signs = templates.get('clinical_signs', ['Variable clinical presentation depending on disease severity'])
                idx = hash(disease_name + 'signs') % len(signs)
                record['clinical_signs'] = signs[idx]
                record['clinical_signs_ja'] = f"{disease_name}の臨床兆候は多様で、疾患の重症度により異なります。専門的な診断が必要です。"
            ...
            if self._is_content_inadequate(record.get('treatment'), 100):
                ...
                record['treatment_ja'] = f"{disease_name}の治療は臨床兆候に基づいて個別に行われます。獣医師による専門的な医学的管理が必要です。"
            ...
            if self._is_content_inadequate(record.get('prognosis'), 100):
                ...
                record['prognosis_ja'] = f"{disease_name}の予後は疾患の重症度と治療への反応に依存します。早期診断と適切な治療が重要です。"
