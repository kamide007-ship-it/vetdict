# VetDict 品質検出レポート — degu

- 総レコード数: **178**

## T101 重複 / 過分割
- 完全一致重複クラスタ: **8** （冗長レコード 8 件）
- 疾患ファミリー候補（過分割の疑い, ≥3件）: **21**

### 完全一致重複（上位20）
- 熱中症（Heat Stroke）[degu_0004] / 熱中症（呼吸器型）（デグー）（Heat Stroke (Respiratory)）[degu_0181]
- バーバリング（過剰毛繕い）（Barbering (Over-grooming)）[degu_0017] / 毛引き（デグー）（Barbering）[degu_0142]
- 下痢（非特異的）（Diarrhea (Non-specific)）[degu_0029] / 下痢（デグー）（Diarrhea）[degu_0135]
- 肝リピドーシス（脂肪肝）（Hepatic Lipidosis (Fatty Liver Disease)）[degu_0034] / 肝リピドーシス（デグー）（Hepatic Lipidosis）[degu_0138]
- てんかん発作（Seizures (Epilepsy)）[degu_0045] / 痙攣発作（デグー）（Seizures）[degu_0147]
- 耳感染症（外耳炎）（Ear Infection (Otitis)）[degu_0051] / 耳感染症（デグー）（Ear Infection）[degu_0170]
- 蟯虫感染（シファシア）（Pinworm Infection (Syphacia)）[degu_0085] / 蟯虫症（デグー）（Pinworm Infection）[degu_0137]
- ケージ攻撃性（縄張り）（Cage Aggression (Territorial)）[degu_0118] / ケージ内攻撃行動（デグー）（Cage Aggression）[degu_0165]

### 疾患ファミリー候補（上位15）
- stem=`diabet` × 5件
    - 糖尿病（Diabetes Mellitus）[degu_0000]
    - 糖尿病性ケトアシドーシス（Diabetic Ketoacidosis）[degu_0001]
    - 糖尿病性白内障（Diabetic Cataracts）[degu_0006]
    - 糖尿病性神経障害（Diabetes-related Neuropathy）[degu_0056]
    - 糖尿病性腎症（Diabetic Nephropathy）[degu_0060]
- stem=`fractu` × 5件
    - 四肢骨折（Limb Fracture）[degu_0021]
    - 脊椎骨折（Spinal Fracture）[degu_0022]
    - 大腿骨骨折（Femoral Fracture）[degu_0108]
    - 脛骨骨折（Tibial Fracture）[degu_0109]
    - 骨折（デグー）（Fracture）[degu_0152]
- stem=`tumor` × 5件
    - 乳腺腫瘍（Mammary Tumor）[degu_0039]
    - 皮膚腫瘍（Skin Tumor）[degu_0040]
    - 腹腔内腫瘍（Abdominal Tumor）[degu_0041]
    - 精巣腫瘍（Testicular Tumor）[degu_0044]
    - 副腎腫瘍（デグー）（Adrenal Tumor）[degu_0189]
- stem=`absces` × 4件
    - 歯根膿瘍（Tooth Root Abscess）[degu_0013]
    - 咬傷・膿瘍（Bite Wounds / Abscess）[degu_0018]
    - 皮膚膿瘍（デグー）（Skin Abscess）[degu_0144]
    - 肝膿瘍（デグー）（Hepatic Abscess）[degu_0194]
- stem=`defici` × 4件
    - ビタミンC欠乏症（壊血病）（Vitamin C Deficiency (Scurvy)）[degu_0052]
    - カルシウム欠乏症（デグー）（Calcium Deficiency）[degu_0157]
    - ビタミンA欠乏症（デグー）（Vitamin A Deficiency）[degu_0158]
    - ビタミンE欠乏症（デグー）（Vitamin E Deficiency）[degu_0159]
- stem=`dental` × 4件
    - 不正咬合（Dental Malocclusion）[degu_0010]
    - 不正咬合 - 臼歯（後天性）（Dental Malocclusion - Molar (Acquired)）[degu_0062]
    - 不正咬合 - 切歯（先天性）（Dental Malocclusion - Incisor (Congenital)）[degu_0063]
    - 歯科疾患（デグー）（Dental Disease）[degu_0124]
- stem=`hepati` × 4件
    - 肝リピドーシス（脂肪肝）（Hepatic Lipidosis (Fatty Liver Disease)）[degu_0034]
    - 肝コクシジウム症（Hepatic Coccidiosis）[degu_0084]
    - 肝リピドーシス（デグー）（Hepatic Lipidosis）[degu_0138]
    - 肝膿瘍（デグー）（Hepatic Abscess）[degu_0194]
- stem=`pneumo` × 4件
    - 肺炎（Pneumonia）[degu_0025]
    - 誤嚥性肺炎（Aspiration Pneumonia）[degu_0026]
    - 細菌性肺炎（急性）（Bacterial Pneumonia (Acute)）[degu_0074]
    - ボルデテラ肺炎（Bordetella Pneumonia）[degu_0076]
- stem=`carcin` × 3件
    - 扁平上皮癌（Squamous Cell Carcinoma）[degu_0101]
    - 肝細胞癌（Hepatocellular Carcinoma）[degu_0102]
    - 腎癌（デグー）（Renal Carcinoma）[degu_0185]
- stem=`infest` × 3件
    - シラミ寄生症（Lice Infestation）[degu_0072]
    - ダニ症（デグー）（Mite Infestation）[degu_0140]
    - ノミ寄生（デグー）（Flea Infestation）[degu_0141]
- stem=`injury` × 3件
    - 尾部脱皮損傷（Tail Degloving Injury）[degu_0016]
    - 回し車による外傷（Exercise Wheel Injury）[degu_0111]
    - 脊髄損傷（デグー）（Spinal Injury）[degu_0172]
- stem=`intest` × 3件
    - 腸内寄生虫症（Intestinal Parasites）[degu_0031]
    - 腸閉塞（Intestinal Obstruction）[degu_0033]
    - 腸管寄生虫症（デグー）（Intestinal Parasitism）[degu_0177]
- stem=`malocc` × 3件
    - 不正咬合（Dental Malocclusion）[degu_0010]
    - 不正咬合 - 臼歯（後天性）（Dental Malocclusion - Molar (Acquired)）[degu_0062]
    - 不正咬合 - 切歯（先天性）（Dental Malocclusion - Incisor (Congenital)）[degu_0063]
- stem=`molar` × 3件
    - 臼歯棘（Molar Spurs）[degu_0012]
    - 不正咬合 - 臼歯（後天性）（Dental Malocclusion - Molar (Acquired)）[degu_0062]
    - 臼歯伸長（デグー）（Molar Elongation）[degu_0130]
- stem=`nephro` × 3件
    - 糖尿病性腎症（Diabetic Nephropathy）[degu_0060]
    - 腎結石症（Nephrolithiasis）[degu_0087]
    - 腎石灰化症（デグー）（Nephrocalcinosis）[degu_0187]

## T102 非臨床エントリ
- フラグ数: **0**

## T104 空 / 見出しのみ治療セクション
- 完全に空: **0**
- 投与量なし（見出しのみ）: **25**
- 投与量あり: **153**

### 投与量なし（上位30）
- インスリン抵抗性（Insulin Resistance）[degu_0002] — ja:549字 / en:1560字
- 肥満（Obesity）[degu_0003] — ja:483字 / en:597字
- 熱中症（Heat Stroke）[degu_0004] — ja:191字 / en:158字
- 切歯過長（Incisor Overgrowth）[degu_0011] — ja:71字 / en:178字
- 毛包虫症（Fur Mites (Demodex / Notoedres)）[degu_0015] — ja:163字 / en:488字
- 誤嚥性肺炎（Aspiration Pneumonia）[degu_0026] — ja:76字 / en:186字
- 腸内寄生虫症（Intestinal Parasites）[degu_0031] — ja:440字 / en:446字
- うっ血性心不全（Congestive Heart Failure）[degu_0049] — ja:355字 / en:437字
- 心筋症（Cardiomyopathy）[degu_0050] — ja:348字 / en:516字
- 耳感染症（外耳炎）（Ear Infection (Otitis)）[degu_0051] — ja:165字 / en:137字
- 敗血症（Septicemia）[degu_0053] — ja:569字 / en:592字
- 不正咬合 - 臼歯（後天性）（Dental Malocclusion - Molar (Acquired)）[degu_0062] — ja:185字 / en:158字
- 不正咬合 - 切歯（先天性）（Dental Malocclusion - Incisor (Congenital)）[degu_0063] — ja:185字 / en:158字
- 尾切断部感染（Tail Amputation Stump Infection）[degu_0069] — ja:194字 / en:204字
- マイコプラズマ感染症（Mycoplasma Infection）[degu_0075] — ja:166字 / en:137字
- 条虫感染症（Tapeworm Infection）[degu_0086] — ja:164字 / en:204字
- 中毒症（植物中毒）（Toxicosis (Plant Poisoning)）[degu_0112] — ja:494字 / en:614字
- 亜鉛中毒（Zinc Toxicosis）[degu_0113] — ja:781字 / en:1024字
- 常同行動（デグー）（Repetitive Behavior (Stereotypy)）[degu_0126] — ja:493字 / en:1004字
- 毛引き（デグー）（Barbering）[degu_0142] — ja:213字 / en:298字
- 開脚症（デグー）（Splay Leg）[degu_0154] — ja:188字 / en:228字
- ストレス関連疾患（デグー）（Stress-Related Illness）[degu_0164] — ja:224字 / en:363字
- 熱中症（呼吸器型）（デグー）（Heat Stroke (Respiratory)）[degu_0181] — ja:220字 / en:507字
- 動脈硬化症（デグー）（Arteriosclerosis）[degu_0183] — ja:204字 / en:225字
- 爪過成長（デグー）（Nail Overgrowth）[degu_0199] — ja:172字 / en:209字

## T108 出典 / 薬品リンク整合性（キーワード自動マッチの誤紐付け）
- 参照コーパス: 薬品 **610** 件 / 出典キー **30** 件（薬品ロードOK・出典ロードOK）
- 種別投与量なしの薬品リンク（旧・未ガード）: **53** 疾患 → ガード後残存: **0**
- 犬猫論文の他種への自動紐付け（旧・未ガード）: **10** 疾患 → ガード後残存: **0**

### 種に投与量データが無い薬品リンク（上位20）
- インスリン抵抗性（Insulin Resistance）[degu_0002] → グリピジド
- 結膜炎（Conjunctivitis）[degu_0008] → オフロキサシン点眼
- 角膜潰瘍（Corneal Ulcer）[degu_0009] → シプロフロキサシン
- 不正咬合（Dental Malocclusion）[degu_0010] → ロニダゾール
- 歯根膿瘍（Tooth Root Abscess）[degu_0013] → ロニダゾール
- 咬傷・膿瘍（Bite Wounds / Abscess）[degu_0018] → ロニダゾール
- 趾瘤症（バンブルフット）（Bumblefoot (Pododermatitis)）[degu_0020] → スルファジアジン, スルファジアジン銀
- 変形性関節症（Osteoarthritis）[degu_0023] → グルコサミン・コンドロイチン
- ジアルジア症（Giardiasis）[degu_0032] → ロニダゾール
- 子宮蓄膿症（Pyometra）[degu_0042] → メデトミジン, ロニダゾール
- 白癬菌感染 - 広範囲型（Trichophyton mentagrophytes (Ringworm) - Extensive）[degu_0073] → アムホテリシンB
- クロストリジウム腸炎（Clostridial Enteritis）[degu_0077] → ロニダゾール, バンコマイシン
- 盲腸内細菌叢異常（Cecal Dysbiosis）[degu_0080] → ロニダゾール
- 胃潰瘍（Gastric Ulceration）[degu_0081] → ロニダゾール
- 腸重積症（Intussusception）[degu_0083] → ロニダゾール
- 肝コクシジウム症（Hepatic Coccidiosis）[degu_0084] → スルファジメトキシン, シリマリン（マリアアザミ抽出物）
- 卵巣嚢胞（Ovarian Cysts）[degu_0090] → メデトミジン, デスロレリン
- 子宮内膜炎（Endometritis）[degu_0091] → ロニダゾール
- 乳腺炎（Mastitis）[degu_0093] → リネゾリド
- 胎児残留（Retained Fetus）[degu_0094] → メデトミジン, ロニダゾール

### 犬猫論文が他種に自動紐付け（上位20）
- 糖尿病（Diabetes Mellitus）[degu_0000] → key: diabetes mellitus
- 皮膚糸状菌症（Ringworm (Dermatophytosis)）[degu_0014] → key: dermatophytosis
- 慢性腎臓病（Chronic Kidney Disease）[degu_0036] → key: chronic kidney disease
- 尿路結石症（Urolithiasis (Bladder Stones)）[degu_0037] → key: urolithiasis
- 子宮蓄膿症（Pyometra）[degu_0042] → key: pyometra
- 心筋症（Cardiomyopathy）[degu_0050] → key: hypertrophic cardiomyopathy (hcm)
- リンパ腫（Lymphoma）[degu_0098] → key: lymphoma
- 貧血（デグー）（Anemia）[degu_0155] → key: immune-mediated hemolytic anemia
- 膀胱炎（デグー）（Cystitis）[degu_0186] → key: feline idiopathic cystitis (fic)
- 白血病（デグー）（Leukemia）[degu_0197] → key: feline leukemia virus

## T109 機械翻訳臭（置換辞書）
- ヒット総数: **0**（safe 0 / review 0）

### 用語別ヒット数

### 出現箇所（上位30）
