...
- 蟯虫感染（Pinworm (Oxyurid) Infection）[tortoise_0015] / 蟯虫感染（リクガメ）（Oxyurid Infection）[tortoise_0173]
- ビタミンA欠乏症（Vitamin A Deficiency (Hypovitaminosis A)）[tortoise_0023] / ビタミンA欠乏症（リクガメ）（Vitamin A Deficiency）[tortoise_0249]
- 肝リピドーシス（脂肪肝）（Hepatic Lipidosis (Fatty Liver Disease)）[tortoise_0032] / 肝リピドーシス（リクガメ）（Hepatic Lipidosis）[tortoise_0215]
- 腎不全（Renal Failure (Chronic Kidney Disease)）[tortoise_0039] / 腎不全（リクガメ）（Renal Failure）[tortoise_0207]
- 膀胱結石（Bladder Stones (Urolithiasis)）[tortoise_0040] / 膀胱結石（尿酸塩型）（Cystic Calculi (Bladder Stones) - Urate）[tortoise_0158] / 膀胱結石（尿酸塩）（リクガメ）（Bladder Stone (Urate)）[tortoise_0176] / 膀胱結石（リクガメ）（Bladder Stone）[tortoise_0209]
- 卵塞（難産）（Egg Binding (Dystocia)）[tortoise_0043] / 卵塞（卵詰まり）（Egg Binding (Dystocia)）[tortoise_0162]
- 熱傷（Thermal Burns）[tortoise_0046] / 熱傷（リクガメ）（Burns）[tortoise_0257]
- 皮下膿瘍（Abscess (Subcutaneous)）[tortoise_0050] / 膿瘍（リクガメ）（Abscess）[tortoise_0201]
- 耳膿瘍（Aural Abscess (Ear Abscess)）[tortoise_0051] / 耳膿瘍（リクガメ）（Ear Abscess (Aural Abscess)）[tortoise_0169]
...
- 四肢骨折（Bone Fracture (Limb)）[tortoise_0092] / 四肢骨折（リクガメ）（Fracture (Limb)）[tortoise_0232] / 甲羅骨折（リクガメ）（Fracture (Shell/Carapace)）[tortoise_0233]
...
- 卵巣腫瘍（Ovarian Neoplasia）[tortoise_0124] / 卵巣腫瘍（リクガメ）（Ovarian Tumor）[tortoise_0214]
- 鉛中毒（Lead Toxicosis）[tortoise_0126] / 鉛中毒（リクガメ）（Lead Poisoning）[tortoise_0254]

### 疾患ファミリー候補（上位15）
- stem=`respir` × 12件
    - 上部呼吸器感染症（Upper Respiratory Tract Infection (URTI)）[tortoise_0005]
    - 慢性呼吸器疾患複合（Chronic Respiratory Disease Complex）[tortoise_0145]
    - 呼吸器感染症（リクガメ）（Respiratory Infection）[tortoise_0179]
    - 上部気道疾患（リクガメ）（Upper Respiratory Tract Disease）[tortoise_0225]
    - 呼吸器細菌感染（リクガメ）（Respiratory Bacterial Infection）[tortoise_0276]
    - 呼吸器ウイルス感染（リクガメ）（Respiratory Viral Infection）[tortoise_0277]
    - 呼吸器真菌感染（リクガメ）（Respiratory Fungal Infection）[tortoise_0278]
    - 呼吸器寄生虫疾患（リクガメ）（Respiratory Parasitic Disease）[tortoise_0279]
    - 呼吸器腫瘍（リクガメ）（Respiratory Neoplasia）[tortoise_0280]
    - 呼吸器炎症性疾患（リクガメ）（Respiratory Inflammatory Disease）[tortoise_0281]
    - 下部気道感染（リクガメ）（Lower Respiratory Tract Infection）[tortoise_0282]
    - 慢性呼吸器疾患（リクガメ）（Chronic Respiratory Disease）[tortoise_0283]
- stem=`gastro` × 10件
    - 消化管閉塞（便秘）（Gastrointestinal Impaction (Constipation)）[tortoise_0029]
    - 胃腸炎（Gastroenteritis）[tortoise_0031]
    - 消化管細菌感染（リクガメ）（Gastrointestinal Bacterial Infection）[tortoise_0268]
    - 消化管ウイルス感染（リクガメ）（Gastrointestinal Viral Infection）[tortoise_0269]
    - 消化管真菌感染（リクガメ）（Gastrointestinal Fungal Infection）[tortoise_0270]
    - 消化管寄生虫疾患（リクガメ）（Gastrointestinal Parasitic Disease）[tortoise_0271]
    - 消化管腫瘍（リクガメ）（Gastrointestinal Neoplasia）[tortoise_0272]
...
- stem=`shell` × 10件
    ...
    - 甲羅の藻類付着（Algae Growth on Shell）[tortoise_0055]
    - フザリウム甲羅感染症（Fusarium Shell Infection）[tortoise_0104]
...
- stem=`defici` × 9件
    ...
    - ビタミンA欠乏症（リクガメ）（Vitamin A Deficiency）[tortoise_0249]
...
- stem=`dermat` × 8件
    ...
    - 皮膚糸状菌症（リクガメ）（Dermatophytosis）[tortoise_0187]
    - 潰瘍性皮膚炎（リクガメ）（Ulcerative Dermatitis）[tortoise_0203]
...
- stem=`absces` × 7件
    ...
    - 甲羅膿瘍（Shell Abscess）[tortoise_0112]
    - 耳膿瘍（リクガメ）（Ear Abscess (Aural Abscess)）[tortoise_0169]
...
- stem=`tortoi` × 6件
    ...
    - リクガメヘキサミタ症（Hexamita parva感染症）（Tortoise Hexamita parva Infection）[tortoise_0164]
...
- stem=`fractu` × 5件
    - 甲羅骨折・外傷（Shell Fracture / Trauma）[tortoise_0002]
    - 四肢骨折（Bone Fracture (Limb)）[tortoise_0092]
    - 脊椎外傷・椎体骨折（Spinal Trauma / Vertebral Fracture）[tortoise_0139]
    - 四肢骨折（リクガメ）（Fracture (Limb)）[tortoise_0232]
    - 甲羅骨折（リクガメ）（Fracture (Shell/Carapace)）[tortoise_0233]
- stem=`infest` × 5件
...
- 蟯虫感染（Pinworm (Oxyurid) Infection）[tortoise_0015] — ja:164字 / en:204字
...
- 異物誤飲（Foreign Body Ingestion）[tortoise_0030] — ja:68字 / en:479字
- 結膜炎（Conjunctivitis）[tortoise_0036] — ja:48字 / en:170字
- 眼周囲膿瘍（Periocular Abscess）[tortoise_0037] — ja:162字 / en:196字
- 角膜潰瘍（Corneal Ulcer）[tortoise_0038] — ja:67字 / en:193字
- 腎不全（Renal Failure (Chronic Kidney Disease)）[tortoise_0039] — ja:65字 / en:188字
- 熱傷（Thermal Burns）[tortoise_0046] — ja:79字 / en:193字
- 敗血症（Septicemia）[tortoise_0049] — ja:160字 / en:233字
- 耳膿瘍（Aural Abscess (Ear Abscess)）[tortoise_0051] — ja:165字 / en:196字
...
- 肝リピドーシス（脂肪肝）（Hepatic Lipidosis (Fatty Liver Disease)）[tortoise_0032] → ウルソデオキシコール酸
- 感染性口内炎（マウスロット）（Infectious Stomatitis (Mouth Rot)）[tortoise_0034] → アモキシシリン・クラブラン酸
- 腎不全（Renal Failure (Chronic Kidney Disease)）[tortoise_0039] → エリスロポエチン
- 卵塞（難産）（Egg Binding (Dystocia)）[tortoise_0043] → オキシトシン, プロポフォール
- 排卵前卵胞停滞（Pre-ovulatory Follicular Stasis）[tortoise_0044] → オキシトシン
...
- 四肢骨折（Bone Fracture (Limb)）[tortoise_0092] → モルヒネ

### 犬猫論文が他種に自動紐付け（上位20）
- 嘴の過成長・不正咬合（Beak Overgrowth / Malocclusion）[tortoise_0035] → key: malocclusion
- 腎不全（Renal Failure (Chronic Kidney Disease)）[tortoise_0039] → key: chronic kidney disease
- 膀胱結石（Bladder Stones (Urolithiasis)）[tortoise_0040] → key: urolithiasis
- パスツレラ症（Pasteurellosis）[tortoise_0099] → key: pasteurellosis
- リンパ腫（Lymphoma）[tortoise_0120] → key: lymphoma
- 溶血性貧血（Hemolytic Anemia）[tortoise_0148] → key: immune-mediated hemolytic anemia
- リクガメ尿石症（膀胱結石）（Tortoise Urolithiasis (Bladder Stone)）[tortoise_0163] → key: urolithiasis
- 皮膚糸状菌症（リクガメ）（Dermatophytosis）[tortoise_0187] → key: dermatophytosis
- 尿路結石症（リクガメ）（Urolithiasis）[tortoise_0208] → key: urolithiasis
- 膵炎（リクガメ）（Pancreatitis）[tortoise_0223] → key: feline pancreatitis
- 心筋症（リクガメ）（Cardiomyopathy）[tortoise_0244] → key: hypertrophic cardiomyopathy (hcm)
- 貧血（リクガメ）（Anemia）[tortoise_0247] → key: immune-mediated hemolytic anemia
