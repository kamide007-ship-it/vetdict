...
## T101 重複 / 過分割
- 完全一致重複クラスタ: **34** （冗長レコード 47 件）
- 疾患ファミリー候補（過分割の疑い, ≥3件）: **40**

### 完全一致重複（上位20）
- 不正咬合（Malocclusion）[chinchilla_0000] / 不正咬合（進行性）（Malocclusion - Progressive）[chinchilla_0163]
- 歯根膿瘍（Dental Abscess）[chinchilla_0002] / 歯根膿瘍（Tooth Root Abscess）[chinchilla_0075]
...
- 直腸脱（Rectal Prolapse）[chinchilla_0010] / 直腸脱（Prolapsed Rectum）[chinchilla_0132]
...
- 皮膚糸状菌症（Ringworm (Dermatophytosis)）[chinchilla_0014] / 皮膚糸状菌症（ミクロスポルム）（チンチラ）（Dermatophytosis (Microsporum)）[chinchilla_0226] / 皮膚糸状菌症（チンチラ）（Dermatophytosis (Ringworm)）[chinchilla_0251]
...
- 熱中症（Heat Stroke）[chinchilla_0020] / 熱中症（神経症状）（Heat Stroke (Neurological)）[chinchilla_0038] / 熱中症（Heat Stroke - Chinchilla）[chinchilla_0149] / 熱中症（高体温症）（Heat Stroke (Hyperthermia)）[chinchilla_0168]
- 尿路結石症（Urolithiasis (Bladder Stones)）[chinchilla_0021] / 尿路結石症（チンチラ）（Urolithiasis）[chinchilla_0273]
- 難産（Dystocia）[chinchilla_0025] / 難産（Dystocia - Chinchilla）[chinchilla_0156]
- 子宮蓄膿症（Pyometra）[chinchilla_0026] / 子宮蓄膿症（Pyometra - Chinchilla）[chinchilla_0157]
- 陰茎毛輪（Penile Fur Ring）[chinchilla_0027] / 陰茎毛輪（Penile Hair Ring - Chinchilla）[chinchilla_0148]
...
- リステリア症（Listeriosis）[chinchilla_0037] / リステリア症（Listeriosis - Chinchilla）[chinchilla_0146] / リステリア症（Listeriosis）[chinchilla_0169]
- 結膜炎（Conjunctivitis）[chinchilla_0039] / 結膜炎（Conjunctivitis - Chinchilla）[chinchilla_0153]

### 疾患ファミリー候補（上位15）
- stem=`chinch` × 13件
    - 腸重積（Intussusception - Chinchilla）[chinchilla_0145]
    - リステリア症（Listeriosis - Chinchilla）[chinchilla_0146]
    - 陰茎毛輪（Penile Hair Ring - Chinchilla）[chinchilla_0148]
    - 熱中症（Heat Stroke - Chinchilla）[chinchilla_0149]
    - 結膜炎（Conjunctivitis - Chinchilla）[chinchilla_0153]
    - 外耳炎（Otitis Externa - Chinchilla）[chinchilla_0154]
    - 糖尿病（Diabetes Mellitus - Chinchilla）[chinchilla_0155]
    - 難産（Dystocia - Chinchilla）[chinchilla_0156]
    - 子宮蓄膿症（Pyometra - Chinchilla）[chinchilla_0157]
    - 胃潰瘍（Gastric Ulceration - Chinchilla）[chinchilla_0158]
...
- stem=`dermat` × 7件
    ...
    - 皮膚糸状菌症（チンチラ）（Dermatophytosis (Ringworm)）[chinchilla_0251]
    - 接触性皮膚炎（チンチラ）（Contact Dermatitis）[chinchilla_0253]
- stem=`ring` × 7件
    - ファーリング（陰茎毛輪）（Fur Ring）[chinchilla_0013]
    - 陰茎毛輪（Penile Fur Ring）[chinchilla_0027]
...
- stem=`tumor` × 5件
    ...
    - 乳腺腫瘍（チンチラ）（Mammary Tumor）[chinchilla_0275]
- stem=`absces` × 4件
    - 歯根膿瘍（Dental Abscess）[chinchilla_0002]
    - 皮膚膿瘍（Skin Abscess）[chinchilla_0070]
    - 歯根膿瘍（Tooth Root Abscess）[chinchilla_0075]
    - 肝膿瘍（チンチラ）（Hepatic Abscess）[chinchilla_0227]
- stem=`cecal` × 4件
    - 盲腸捻転（Cecal Torsion）[chinchilla_0011]
    - 盲腸嵌頓（Cecal Impaction）[chinchilla_0087]
    - 盲腸内細菌叢異常（Cecal Dysbiosis）[chinchilla_0124]
    - 鼓脹症（急性盲腸鼓腸）（Bloat - Acute Cecal Tympany）[chinchilla_0144]
- stem=`hepati` × 4件
...
- 流涎症（Slobbers (Ptyalism)）[chinchilla_0003] → ムピロシン, リンコマイシン
- 消化管うっ滞（GI Stasis）[chinchilla_0004] → ドパミン, ドメペリドン, 活性炭
- 鼓腸症（Bloat (Gastric Tympany)）[chinchilla_0005] → ヒドロモルフォン, ロニダゾール
- 下痢（Diarrhea）[chinchilla_0007] → ロニダゾール
- 腸炎（Enteritis）[chinchilla_0008] → ロニダゾール
- 盲腸捻転（Cecal Torsion）[chinchilla_0011] → ロニダゾール
...
- 結膜炎（Conjunctivitis）[chinchilla_0039] → オフロキサシン点眼
- 角膜潰瘍（Corneal Ulcer）[chinchilla_0040] → シプロフロキサシン
- 白内障（Cataracts）[chinchilla_0041] → アトロピン1%点眼
- ビタミン欠乏症（Vitamin Deficiency）[chinchilla_0046] → リンコマイシン
- 緑膿菌感染症（Pseudomonas Infection）[chinchilla_0049] → シプロフロキサシン, セフタジジム, スルファジアジン, スルファジアジン銀
- ジアルジア症（Giardiasis）[chinchilla_0052] → ロニダゾール
