...
## T101 重複 / 過分割
- 完全一致重複クラスタ: **23** （冗長レコード 25 件）
- 疾患ファミリー候補（過分割の疑い, ≥3件）: **123**

### 完全一致重複（上位20）
- 喉頭片麻痺(喘鳴症)（Laryngeal Hemiplegia）[rp_laryngeal] / 喉頭片側麻痺(ロアリング)（Laryngeal Hemiplegia (Roaring)）[rp_laryngeal_hemiplegia]
- 軟口蓋背方変位 (DDSP)（DDSP）[rp_ddsp] / 軟口蓋背方変位（Dorsal Displacement of Soft Palate (DDSP)）[rp_ddsp2]
- 腺疫（Strangles）[rp_strangles] / 腺疫(ストラングルズ)（Strangles (Streptococcus equi)）[if_strangles]
- ピロプラズマ症（Piroplasmosis）[if_piroplamosis] / 馬ピロプラズマ症（Equine Piroplasmosis）[if_equine_piroplasmosis] / ピロプラズマ症(バベシア症)（Equine Piroplasmosis (Babesiosis / Theileriosis)）[if_piroplasmosis]
- 馬再発性ぶどう膜炎 (ERU)（Equine Recurrent Uveitis）[eye_uveitis_signs] / 馬回帰性ぶどう膜炎(月盲)（Equine Recurrent Uveitis (Moon Blindness)）[ey_recurrent_uveitis]
- 馬原虫性脊髄脳炎 (EPM)（Equine Protozoal Myeloencephalitis）[nr_epm] / 馬原虫性脊髄脳炎(詳細)（EPM (Extended)）[nr_epm_extended]
- 馬メタボリック症候群 (EMS)（Equine Metabolic Syndrome）[mt_ems] / 馬メタボリック症候群（Equine Metabolic Syndrome (EMS)）[mt_ems2]
- 下垂体中葉機能障害 (PPID)（Pituitary Pars Intermedia Dysfunction）[mt_ppid] / 下垂体中葉機能障害(クッシング病)（Pituitary Pars Intermedia Dysfunction (PPID / Cushing's)）[mt_ppid2]
- 横紋筋融解症(月曜病)（Exertional Rhabdomyolysis）[mt_rhabdomyolysis] / 横紋筋融解症(タイングアップ)（Exertional Rhabdomyolysis (Tying Up)）[mt_rhabdomyolysis2]
- バックドシン（Bucked Shin）[ms_bucked_shin] / 中手骨骨膜炎（Bucked Shins）[ms_bucked_shins]
- ロドコッカス肺炎(子馬)（Rhodococcus equi Pneumonia）[if_rhodococcus] / ロドコッカス肺炎（Rhodococcus equi Pneumonia (Foal)）[fl_rhodococcus_pneumonia]
- 馬ウイルス性動脈炎 (EVA)（Equine Viral Arteritis）[if_eve] / 馬ウイルス性動脈炎（Equine Viral Arteritis (EVA)）[if_equine_viral_arteritis]
- ドングリ中毒（Acorn Toxicity）[tx_acorn] / ドングリ中毒(タンニン中毒)（Acorn / Oak Toxicosis (Tannin Poisoning)）[tx_acorn2]
- フェスク中毒（Fescue Toxicity）[tx_fescue] / エンドファイトフェスク中毒（Fescue Toxicosis）[tx_endophyte_fescue] / フェスク中毒(繁殖)（Fescue Toxicosis (Reproductive)）[rp_fescue_toxicosis_repro]
- 肢軸異常(内反・外反)（Angular Limb Deformities）[ms_angular_limb] / 肢軸異常(ALD)（Angular Limb Deformity）[fl_angular_limb_deformity]
- 肘腫（Capped Elbow）[ms_capped_elbow] / 肘腫(シューブイル)（Elbow Hygroma (Shoe Boil / Capped Elbow)）[ms_elbow_hygroma]
- 浅指屈腱炎（Superficial Digital Flexor Tendinitis）[ms_sdft] / 浅趾屈腱炎（Superficial Digital Flexor Tendinitis (Bowed Tendon)）[mc_tendinitis_sdft]
- 潰瘍性リンパ管炎（Ulcerative Lymphangitis）[sk_ulcerative_lymph] / 潰瘍性リンパ管炎(コリネバクテリウム)（Ulcerative Lymphangitis (Corynebacterium pseudotuberculosis)）[if_corynebacterium2]
...
- 馬ヘルペス1型脊髄脳症（EHV-1 Myeloencephalopathy）[if_ehv1_myeloencephalopathy] / 馬ヘルペスウイルス1型神経型（EHV-1 Myeloencephalopathy (EHM)）[if_ehv1_neuro]
...
- stem=`equine` × 53件
    ...
    - 馬ピロプラズマ症（Equine Piroplasmosis）[if_equine_piroplasmosis]
    ...
    - 馬回帰性ぶどう膜炎(月盲)（Equine Recurrent Uveitis (Moon Blindness)）[ey_recurrent_uveitis]
    ...
    - ピロプラズマ症(バベシア症)（Equine Piroplasmosis (Babesiosis / Theileriosis)）[if_piroplasmosis]
    - 馬ウイルス性動脈炎（Equine Viral Arteritis (EVA)）[if_equine_viral_arteritis]
    - 馬ロタウイルス感染症（Equine Rotavirus Infection）[if_equine_rotavirus]
    - 馬メタボリック症候群（Equine Metabolic Syndrome (EMS)）[mt_ems2]
...
- stem=`toxico` × 12件
    ...
    - エンドファイトフェスク中毒（Fescue Toxicosis）[tx_endophyte_fescue]
    - フェスク中毒(繁殖)（Fescue Toxicosis (Reproductive)）[rp_fescue_toxicosis_repro]
    - イオノフォア中毒（Ionophore Toxicosis (Monensin / Lasalocid)）[tx_ionophore]
    - 有機リン中毒（Organophosphate Toxicosis）[tx_organophosphate2]
    - ドングリ中毒(タンニン中毒)（Acorn / Oak Toxicosis (Tannin Poisoning)）[tx_acorn2]
