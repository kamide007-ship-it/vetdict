...
## T101 重複 / 過分割
- 完全一致重複クラスタ: **33** （冗長レコード 42 件）
- 疾患ファミリー候補（過分割の疑い, ≥3件）: **57**

### 完全一致重複（上位20）
- 上部気道感染症（Upper Respiratory Infection (URI)）[guinea_pig_0000] / 上部気道感染症（モルモット）（Upper Respiratory Infection）[guinea_pig_0222]
- 細菌性肺炎（Bacterial Pneumonia）[guinea_pig_0001] / 細菌性肺炎（モルモット）（Pneumonia (Bacterial)）[guinea_pig_0195]
- 鼓脹症（胃拡張）（Bloat (Gastric Dilation)）[guinea_pig_0006] / 鼓脹症（胃鼓脹）（モルモット）（Bloat (Gastric Tympany)）[guinea_pig_0201]
- 胃潰瘍（Gastric Ulcers）[guinea_pig_0012] / 胃潰瘍（モルモット）（Gastric Ulcer）[guinea_pig_0213]
...
- 皮下膿瘍（Abscess (Subcutaneous)）[guinea_pig_0021] / 皮下膿瘍（モルモット）（Subcutaneous Abscess）[guinea_pig_0242]
- 尿路結石症（Urolithiasis (Bladder Stones)）[guinea_pig_0022] / 尿路結石症（モルモット）（Urolithiasis）[guinea_pig_0198]
- 尿路感染症（Urinary Tract Infection (UTI)）[guinea_pig_0023] / 尿路感染症（モルモット）（Urinary Tract Infection）[guinea_pig_0249]
...
- 斜頸（Head Tilt (Torticollis)）[guinea_pig_0043] / 斜頸（モルモット）（Torticollis）[guinea_pig_0304]
...
- 不正咬合（Dental Malocclusion）[guinea_pig_0055] / 不正咬合（モルモット）（Malocclusion）[guinea_pig_0196]
- 乳腺腫瘍（Mammary Tumors）[guinea_pig_0060] / 乳腺腫瘍（モルモット）（Mammary Tumor）[guinea_pig_0257]
...
- コクシジウム症（Coccidia）[guinea_pig_0072] / コクシジウム症（モルモット）（Coccidiosis）[guinea_pig_0207]
- ニキビダニ症（Demodex Mites）[guinea_pig_0073] / ニキビダニ症（モルモット）（Demodex Mange）[guinea_pig_0238]
- 毛ダニ症（Fur Mites (Chirodiscoides caviae)）[guinea_pig_0074] / 毛ダニ症（重度感染）（Fur Mites - Heavy Infestation）[guinea_pig_0172]

### 疾患ファミリー候補（上位15）
- stem=`pneumo` × 11件
    - 細菌性肺炎（Bacterial Pneumonia）[guinea_pig_0001]
    - 肺炎球菌感染症（Streptococcus pneumoniae Infection）[guinea_pig_0003]
    - 真菌性肺炎（アスペルギルス症）（Fungal Pneumonia (Aspergillosis)）[guinea_pig_0091]
    - ボルデテラ肺炎（Bordetella Pneumonia）[guinea_pig_0103]
    - 肺炎球菌性肺炎（Streptococcus pneumoniae Pneumonia）[guinea_pig_0104]
    - クレブシエラ肺炎（Klebsiella Pneumonia）[guinea_pig_0129]
    - 間質性肺炎（Interstitial Pneumonia）[guinea_pig_0147]
    - 細菌性肺炎（モルモット）（Pneumonia (Bacterial)）[guinea_pig_0195]
    - アデノウイルス肺炎（モルモット）（Adenovirus Pneumonia）[guinea_pig_0219]
    - マイコプラズマ肺炎（モルモット）（Mycoplasma Pneumonia）[guinea_pig_0221]
    - 誤嚥性肺炎（モルモット）（Aspiration Pneumonia）[guinea_pig_0225]
- stem=`tumor` × 9件
    - 子宮腫瘍（Uterine Tumors）[guinea_pig_0032]
    - 乳腺腫瘍（Mammary Tumors）[guinea_pig_0060]
    - 精巣腫瘍（Testicular Tumors）[guinea_pig_0093]
    - 膵島細胞腫瘍（Pancreatic Islet Cell Tumor）[guinea_pig_0140]
    - 肺腫瘍（モルモット）（Lung Tumor）[guinea_pig_0228]
    - 乳腺腫瘍（モルモット）（Mammary Tumor）[guinea_pig_0257]
    - 精巣腫瘍（モルモット）（Testicular Tumor）[guinea_pig_0258]
    - 甲状腺腫瘍（モルモット）（Thyroid Tumor）[guinea_pig_0280]
    - 副腎腫瘍（モルモット）（Adrenal Tumor）[guinea_pig_0281]
- stem=`cyst` × 8件
    - 皮脂嚢胞（Sebaceous Cyst）[guinea_pig_0018]
    - 卵巣嚢腫（Ovarian Cysts）[guinea_pig_0027]
    - 卵巣嚢胞 - 漿液性（Ovarian Cysts - Serous）[guinea_pig_0097]
    - 卵巣嚢胞 - 濾胞性（Ovarian Cysts - Follicular）[guinea_pig_0098]
    - 肝嚢胞（Hepatic Cyst）[guinea_pig_0139]
...
- stem=`absces` × 7件
    - 皮下膿瘍（Abscess (Subcutaneous)）[guinea_pig_0021]
    - 歯科膿瘍（Dental Abscess）[guinea_pig_0057]
    - 包皮腺膿瘍（Preputial Gland Abscess）[guinea_pig_0192]
    - 根尖膿瘍（モルモット）（Periapical Abscess）[guinea_pig_0230]
    - 皮下膿瘍（モルモット）（Subcutaneous Abscess）[guinea_pig_0242]
    - 肝膿瘍（モルモット）（Hepatic Abscess）[guinea_pig_0297]
    - 足底膿瘍（モルモット）（Pedal Abscess）[guinea_pig_0303]
- stem=`scurvy` × 7件
...
- stem=`impact` × 6件
    - 盲腸閉塞（Cecal Impaction）[guinea_pig_0010]
    - 肛門嚢閉塞（Impaction (Perineal Sac)）[guinea_pig_0079]
    - 直腸閉塞（Rectal Impaction）[guinea_pig_0090]
...
- stem=`lympha` × 5件
    - 連鎖球菌性リンパ節炎（Streptococcal Lymphadenitis (Lumps)）[guinea_pig_0071]
    - 頸部リンパ節炎（Cervical Lymphadenitis）[guinea_pig_0084]
    - 頸部リンパ節炎（ストレプトコッカス・ズーエピデミカス）（Cervical Lymphadenitis (Streptococcus zooepidemicus)）[guinea_pig_0102]
    - 頸部リンパ節炎（ストレプトバチルス）（Cervical Lymphadenitis (Streptobacillus)）[guinea_pig_0160]
    - 頸部リンパ節炎（モルモット）（Cervical Lymphadenitis (Lumps)）[guinea_pig_0246]
- stem=`ovaria` × 5件
    - 卵巣嚢腫（Ovarian Cysts）[guinea_pig_0027]
    - 卵巣嚢胞 - 漿液性（Ovarian Cysts - Serous）[guinea_pig_0097]
    - 卵巣嚢胞 - 濾胞性（Ovarian Cysts - Follicular）[guinea_pig_0098]
    - 卵巣奇形腫（Ovarian Teratoma）[guinea_pig_0118]
    - 卵巣嚢胞（機能性）（Ovarian Cysts - Functional）[guinea_pig_0167]
- stem=`adenoc` × 4件
    - 肺腺癌（Lung Adenocarcinoma）[guinea_pig_0116]
    - 子宮腺癌（Uterine Adenocarcinoma）[guinea_pig_0117]
    - 乳腺腺癌（Mammary Adenocarcinoma）[guinea_pig_0181]
    - 腸腺癌（モルモット）（Intestinal Adenocarcinoma）[guinea_pig_0216]
- stem=`carcin` × 4件
    - 皮膚扁平上皮癌（Skin Squamous Cell Carcinoma）[guinea_pig_0064]
    - 扁平上皮癌（モルモット）（Squamous Cell Carcinoma）[guinea_pig_0245]
    - 肝細胞癌（モルモット）（Hepatocellular Carcinoma）[guinea_pig_0275]
    - 腎癌（モルモット）（Renal Carcinoma）[guinea_pig_0276]
- stem=`caviae` × 4件
    - トリキサカルス疥癬 - 重症型（Trixacarus caviae (Sarcoptic Mange) - Severe）[guinea_pig_0101]
    - キロディスコイデス・カビエ寄生（Chirodiscoides caviae Infestation）[guinea_pig_0127]
...
- 下痢（Diarrhea）[guinea_pig_0007] → ロニダゾール
- 細菌性腸炎（Bacterial Enteritis）[guinea_pig_0008] → ロニダゾール
- 抗生物質関連腸毒素血症（Antibiotic-Associated Enterotoxemia）[guinea_pig_0009] → ロニダゾール, コレスチラミン, 活性炭
- 足底皮膚炎（バンブルフット）（Pododermatitis (Bumblefoot)）[guinea_pig_0017] → クロルヘキシジン, スルファジアジン
- 蝿蛆症（Flystrike (Myiasis)）[guinea_pig_0019] → クロルヘキシジン
- 接触性皮膚炎（Contact Dermatitis）[guinea_pig_0020] → クロルヘキシジン
...
- 壊血病関連関節炎（Scurvy-Related Arthritis）[guinea_pig_0034] → グルコサミン・コンドロイチン, クロルヘキシジン
- 変形性関節症（Osteoarthritis）[guinea_pig_0035] → グルコサミン・コンドロイチン
...
- 白血病（Leukemia）[guinea_pig_0066] → L-アスパラギナーゼ
