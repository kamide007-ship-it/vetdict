...
_PRODUCTS: list[tuple[frozenset[str], tuple[str, ...], str]] = [
    (
        ...
        "カルドメック®チュアブルP（イベルメクチン＋パモ酸ピランテル）: フィラリア予防と回虫・鉤虫の駆除を兼ねる月1回経口。／"
        "シンパリカ®トリオ（サロラネル＋モキシデクチン＋パモ酸ピランテル）: "
        "ノミ・マダニ＋フィラリア予防＋消化管内線虫を月1回経口でカバー。",
    ),
    (
        frozenset({"cat"}),
    ...
    (
        ...
        "プラジクアンテル）、レボリューション®（セラメクチン）、レボリューション®プラス（セラメクチン＋サロラネル、"
        "マダニもカバー）、フロントライン®プラス、ブラベクト®スポット（フルララネル）。"
    ...
    (
        frozenset({"cat"}),
        ("フィラリア", "heartworm", "dirofilar"),
        "予防: レボリューション®／レボリューション®プラス（セラメクチン）、ブロードライン®（エプリノメクチン配合）、ミルベマイシンオキシム製剤。"
        f"いずれも{_BAND}。"
    ...
    # --- Exotic companion species -------------------------------------------
    # Almost nothing is licensed for these in Japan, so the honest framing is
    # "off-label under veterinary direction" plus the contraindications that
    # actually kill them — not a product list implying an approved indication.
    (
        frozenset({"ferret"}),
        ("ノミ", "マダニ", "疥癬", "ミミヒゼンダニ", "耳ダニ", "flea", "tick", "sarcopt", "ear mite", "otodect"),
        "フェレットに国内承認された外部寄生虫薬はほとんどなく、犬猫用製剤の適応外使用を獣医師の判断で行うのが実際である。"
        "セラメクチン（レボリューション®）はノミ・ミミヒゼンダニ・疥癬に汎用され、フィラリア予防も兼ねる。"
        "フィプロニル製剤も用いられる。いずれも体重が小さいため過量投与に注意し、"
        "犬用の大型体重帯製剤を分割して使わない（正確な分割ができず過量のリスクがある）。"
        "⚠フェレットはフィラリアに感受性が高く、少数の虫体で致死的となるため、"
        "蚊の発生地域では通年のフィラリア予防を必ず併せて行う。",
    ),
    (
        frozenset({"rabbit", "guinea_pig", "chinchilla", "degu"}),
        (
            "ノミ",
            "マダニ",
            "疥癬",
        ...
        "セラメクチン（レボリューション®）またはイベルメクチンが外部寄生虫に広く用いられる（いずれも適応外使用）。"
        "ウサギのウサギキュウセンヒゼンダニ（Psoroptes cuniculi、耳疥癬）にはセラメクチンまたはイベルメクチンが有効で、"
        "痂皮は無理に剥がさず自然脱落を待つ（激痛と出血を伴うため）。"
        "⚠フィプロニル（フロントライン®）はウサギ・モルモット・チンチラ・デグーに禁忌——"
        "けいれん・食欲廃絶から死亡した報告があり、犬猫用スポットオンを流用してはならない。",
    ),
    (
        frozenset({"bird", "parakeet", "parrot"}),
        ("ダニ", "疥癬", "トリサシダニ", "ワクモ", "羽毛", "mite", "knemidokopt", "scaly"),
        "トリヒゼンダニ（Knemidokoptes、疥癬・鱗状嘴脚症）にはイベルメクチンが標準（適応外使用、"
        "経口・局所いずれも用いられ、2週間隔で反復）。"
        "体重が極小のため必ず希釈して正確に計量し、原液の滴下は行わない。"
        "ワクモ・トリサシダニは環境中に潜むため、鳥だけでなくケージ・止まり木・巣箱の徹底した清掃と交換を同時に行う。"
        "⚠フィプロニルは鳥に用いない（死亡例の報告がある）。"
        "⚠有機リン系・カーバメート系の殺虫剤や、鳥の近くでの燻煙・スプレー使用は避ける。",
    ),
    (
        frozenset({"dog", "cat"}),
        ("ジアルジア", "giardi"),
