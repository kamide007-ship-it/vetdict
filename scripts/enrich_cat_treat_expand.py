...
ENRICHMENTS["cat_feline_mitral_valve_disease"] = {
    "treatment_ja": (
        "猫の僧帽弁疾患は弁膜線維症や先天性異形成が多い。"
        "無症候期は心エコーで僧帽弁逆流の重症度、左房拡大（LA/Ao比）を定期評価。"
        "心不全顕性化時：フロセミド（1-2 mg/kg PO q12-24h、急性は2-4 mg/kg IV）。"
        "ピモベンダン（0.625-1.25 mg/cat PO q12h）はSAM非存在下で使用。"
        "ACE阻害薬（ベナゼプリル0.5 mg/kg PO q24h）で後負荷軽減。"
        "血栓塞栓症予防：クロピドグレル（18.75 mg/cat PO q24h）は左房拡大（>16 mm）で開始。"
        "不整脈にはアテノロール（6.25 mg/cat PO q12h）またはジルチアゼム。ストレス最小化が重要。"
    )
}
...
ENRICHMENTS["cat_feline_bile_duct_obstruction"] = {
    "treatment_ja": (
        "完全閉塞は外科的介入が必須。胆嚢十二指腸吻合術が標準術式で、総胆管探索＋ステント留置も選択肢。"
        "術前安定化：輸液療法、ビタミンK1（0.5-1.5 mg/kg SC/IM q12h×3回 — 凝固能回復に24-48h）、"
        "広域抗菌薬（アンピシリン/スルバクタム30 mg/kg IV q8h＋メトロニダゾール10 mg/kg IV q12h）。"
        "内科管理：UDCA（10-15 mg/kg PO q24h）で胆汁うっ滞改善、SAMe（90 mg/cat PO q24h空腹時）で肝保護。"
        "原因精査：胆管炎、膵炎、triaditis、胆石、腫瘍。エコー＋肝酵素（ALP/GGT/T-Bil）で評価。"
    )
}
...
ENRICHMENTS["cat_feline_congestive_heart_failure"] = {
    "treatment_ja": (
        ...
        "猫はストレスで急変するため最小限のハンドリングで処置。胸水にはエコーガイド下胸腔穿刺。"
        "維持療法：フロセミド（1-2 mg/kg PO q12-24h、最低有効量を探索）。"
        "ピモベンダン（0.625-1.25 mg/cat PO q12h — SAM非存在確認後）で収縮力増強＋血管拡張。"
        "ACE阻害薬（ベナゼプリル0.5 mg/kg PO q24h）。"
        "血栓予防：クロピドグレル（18.75 mg/cat PO q24h）は左房拡大時に開始。"
        "安静時RR>40回/分は再代償不全を示唆。腎機能（BUN/Cre）をq2-4週でモニタリング。低ナトリウム食。"
    )
}
...
ENRICHMENTS["cat_feline_chronic_progressive_polyarthritis"] = {
    "treatment_ja": (
        "免疫介在性多発性関節炎。プレドニゾロン（2 mg/kg PO q12h×2-4週→漸減して維持量0.5-1 mg/kg q48h）。"
        "ステロイド単独で不十分時はクロラムブシル（0.1-0.2 mg/kg PO q48-72h）を追加し、"
        "CBC q2-4週で骨髄抑制をモニタリング。シクロスポリン（5-7 mg/kg PO q24h）も代替。"
        "疼痛管理：ブプレノルフィン（0.02 mg/kg 頬粘膜 q8-12h）急性期。ガバペンチン（5-10 mg/kg PO q8-12h）長期鎮痛。"
        "フルネベトマブ（ソレンシア 1 mg/kg SC q月）は抗NGF抗体で補助的に有用。"
        "FeLV/FIV関連の場合があり検査必須。X線で関節びらん/骨増殖を経時的に評価。理学療法で関節可動域を維持。"
    )
}
