...
## 2026-09セッション（第34弾: グローバル検索2.0 — 「1ボックスで何でも」＋行き止まりゼロ）

### 背景（開発者要望「今まで以上に直感的に使いやすく」）
ヘッダーのグローバル検索を監査した結果、**最も目立つ入口が機能の一部にしか届いていなかった**:
- 薬品は `name/name_ja` の素の小文字一致のみ → 第10弾で薬品タブに入れた**商品名・かな正規化がオムニボックスには未適用**（「バイトリル」がヘッダー検索では0件）
- 薬品辞書のプリフェッチが**動物種タップ後にしか始まらない** → 初回訪問のヘッダー検索は薬品が黙って空
- 救急プロトコル・麻酔・症状チェック・相談チャットには検索から一切到達できない
- 0件時は「該当する疾患がありません」1行で行き止まり
- `/api/diseases?q=` はSQLite LIKE の byte 一致で **ひらがな/全角入力が0件**（「ぱるぼ」「ＦＩＰ」）

### 実装（`static/js/app.js` setupGlobalSearch 書き換え + `api/disease_store.py`）
- **薬品**: 薬品タブと同一の `normalizeDrugSearchText` + `_drugMatchesSearch`（name/alias/商品名）で照合、
  商品名ヒットは「商品名: バイトリル」タグ表示。**未ロード時は初回検索で `loadDrugDictionary()` を起動し
  到着後に自動再検索**（「薬品辞書を読み込み中…」行を表示）。アイドル・プリフェッチを setupGlobalSearch
  時に開始（種タップ非依存）。`loadDrugDictionary` に in-flight ガード（`_drugLoadInFlight`）を追加し
  二重フェッチを防止
- **疾患**: ローカル照合もかな/幅正規化。API側は `disease_store.search_diseases` が SQLite で0件のとき
  **かな/幅正規化した名前インデックスで rescue**（`_normalize_search_text` = NFKC+小文字+ひらがな→カタカナ、
  `_search_index_fallback` を両側正規化）。「ぱるぼ」→パルボ、「ＦＩＰ」→FIP（漢字の読みは対象外＝設計通り）
- **救急プロトコル・グループ（🚨）**: `ensureEmergencySearchData()`（オムニボックス専用 fetch-once キャッシュ —
  救急タブ自身の `emergencyData/emergencyLoaded` と意図的に分離し、タブのローダーと
  `navigateToEmergencyProtocol` の readiness poll のセマンティクスを不変に保つ）。タイトル・トリガー徴候で
  照合し、タップで完全一致着地。最近の検索にも種/プロトコルIDを保持
- **アクション行（常設、0件時は主導線）**: 「他の探し方」ブロックに
  🩺 症状「◯◯」で鑑別チェック（現在種のチェッカー語彙に一致した場合のみ — 種を推測しない、
  `_runCheckerWithSymptoms` で事前選択＋自動解析）／💉 麻酔プロトコルを「q」で検索（麻酔タブ＋検索欄プリフィル）／
  💬 「q」を相談チャットで相談する（自由入力モード＋入力欄プリフィル＋フォーカス）。
  530+口語エイリアスを持つチャットへの常設導線で、どんな入力も行き止まりにならない
- プレースホルダを「疾患・薬品・症状・救急を検索（例: バイトリル, 嘔吐）」に変更（商品名・症状で探せることを教示）
- CSS: `.search-action-item`（アンバー系で実体結果と視覚的に区別）/`.search-action-head`。SW v142→**v143**

### 回帰テスト（tests/test_omnibox_search.py 新規、8件）
- API: ひらがな/全角のかな折り畳み（ぱるぼ/ＦＩＰ）+ 完全一致経路の非回帰、正規化ユニット
...
## 2026-09セッション（第35弾: 臨床計算機スイート新設 — 競合アプリ比較に基づく機能拡充）

### 背景（開発者要望「競合のアプリケーションと比較し、より充実できる使い方に」）
競合の公開機能を調査した結果、**計算機能が最大のギャップ**と判明:
- **Plumb's**: BSA計算機・リーン体重計算機・単位換算・25,000件の相互作用チェッカー・飼い主ハンドアウト
- **Vetcalculators / Vet Easy**: 救急薬・CRI・輸液・チョコレート中毒・カロリー(RER/MER)計算が中核機能
- VetDict既存: 相互作用チェッカー✓・鑑別診断/問診チャット（競合に無い独自強み）✓、
  計算は「種フィルタ時の mg/kg×体重」のみで **CRI/輸液/カロリー/中毒/輸血/BSA は皆無** だった

### 臨床計算機スイート（薬品タブ新アコーディオン、7計算機・日英対応）
- **数式は `VD_CALC`（DOM非依存の純関数オブジェクト）に隔離** — pytest が app.js からブロックを抽出し
  **node で実測実行して教科書値と照合**（ソース存在チェックではなく本物の数値検証）
  1. **用量計算**: mg/kg(範囲)×体重 → mg/回 + 液剤mL(濃度) + 錠数(規格)。µg/kg対応
  2. **CRI**: µg/kg/min・µg/kg/hr・mg/kg/hr・mg/kg/day → mg/hr → **mL/hr**（ドブタミン例文付き）
  3. **輸液プラン**: 維持量（犬132×BW^0.75/猫80×BW^0.75 — AAHA/AAFP 2013、その他種は手入力）+
     脱水欠乏（BW×%×10）÷補正時間 + 継続損失 → 開始レートmL/hr。
     ショックボーラス対象外（救急タブ参照）・心疾患/乏尿腎の減量注意を明記
  4. **カロリー**: RER=70×BW^0.75（WSAVA 2011）× 種別MER係数（避妊去勢/未/減量/成長/入院）。
     入院はRER×1.0開始のrefeeding注意
  5. **チョコレート中毒**: テオブロミン表（ホワイト0.01/ミルク2.1/ダーク5.5/製菓用14/ココア26 mg/g —
     Merck/ASPCA APCC）→ mg/kg → リスク5段階（20 GI/40 心毒性/60 痙攣/100+ 致死域）を色分け表示 +
     「疾患エントリを開く」ピボット（dog Chocolate Toxicosis へ完全一致着地）+ 催吐2-4時間窓の注記
  6. **輸血量**: BW×血液量（犬90/猫60 mL/kg）×ΔPCV÷ドナーPCV + 目安則（2 mL/kg=PCV+1%）併記 +
     投与速度注記（Silverstein & Hopper 3rd）
  7. **BSA**: k×(BW g)^(2/3)×10⁻⁴（犬10.1/猫10.0）— m²ベース化学療法用
- 患者体重は既存の `vetdict-drug-weight` localStorage と**同一キーで共有**（薬品リストの体重計算と同期）
- 免責固定表示: 「投与前に必ず用量・濃度・計算を再確認。臨床判断を代替しません」

### 導線（クリックで計算機へ）
- **薬品カード→計算機プリフィル**: 種別用量行に「🧮 計算機で開く」ボタン（リスト用量ボックス +
  詳細パネルの全種カード）。`parseDoseRange` が**一意に解析できた safe 行のみ**表示（誤解析ゼロ設計）、
  計算機側に出典（薬品名+用量原文）を常時表示し原文確認を促す。委譲ハンドラでキャッシュ再描画後も動作
- **救急タブ→計算機**: 「🧮 臨床計算機を開く」ボタン（CRIタブで着地 — 救急現場の最頻計算）
- **チョコ計算→疾患DB**: リスク判定から疾患エントリへワンタップ
- GA4: `calc_from_drug` / `calc_from_emergency` / `calculator_tab` / `disease_from_calculator`
- 言語切替時は描画済み計算機を現在言語で再描画（applyLanguage フック）

### 回帰テスト（tests/test_clinical_calculators.py 新規、3件）
- `test_vd_calc_formulas_verified_by_node` — **実配布コードの数式を node で実行**し
  RER/BSA/維持輸液/欠乏/CRI(ドブタミン12 mL/hr)/チョコ(ダーク50g/10kg=27.5)/輸血(450mL)/
  リスク閾値境界(19.9=low, 20=gi, 40=cardiac, 60=seizure, 100=lethal)/テオブロミン表を全数照合
- `test_calculator_ui_is_wired` — アコーディオン/7パネル/プリフィル/救急リンク/言語再描画/CSS/共有体重キー
- `test_calculator_safety_rails_in_source` — 免責・出典エコー・ショック除外・催吐窓・
  safe行ゲート・疾患ピボットの完全一致着地

### 競合比較の残ロードマップ（次セッション候補）
- **飼い主向けハンドアウト**（Plumb's/Vetlexicon）: 疾患エントリから飼い主説明シートを印刷生成
  （既存の麻酔印刷チェックリスト機構を流用可能。トーン変換が必要なため獣医レビュー前提）
- **検査基準値リファレンス**（Vetcalculators）: 血液/生化学の種別基準値表 + IRIS CKDステージ
- **単位換算**（lb↔kg・mg/dL↔mmol/L等 — 日本市場では優先度低）
- リーン体重(BCS補正)計算（Plumb's 2024新機能）

### テスト・CI
- ServiceWorker: `CACHE_NAME` v144 → **v145**
- データ変更なし（配信DB・検索インデックスは不変）

## 2026-09セッション（第36弾: 臨床計算機のエラー監査・是正 + IRIS CKDステージング・単位換算・リーン体重・飼い主向けハンドアウト）

### 臨床計算機のエラー監査（利用者依頼「エラーがないか確認」— 5件の実害エッジケースを検出・修正）
マージ済み計算機スイート（第35弾）を、node実測・実データ突合・境界値で再監査:
1. **parseDoseRange が CRI/日量表記を1回量として誤プリフィル**（最重要）: `0.125 mg/kg/min IV CRI`
   （ジルチアゼム馬）や `2-5 mg/kg/day`（コリスチン）の持続/日量が「1回量」ボタンになっていた →
   `/kg` 直後にスラッシュが続く表記を `(?!\s*\/)` で除外（mg/kg/hr・day・min・dose）。除外により
   同一テキスト内の後続の真のボーラス（「CRI…; bolus 2 mg/kg」）には引き続き着地。
   実データ影響: CRI専用38行のボタンが正しく消え、6行が実際のボーラス/経口用量に是正
   （メトクロプラミド 1-2 mg/kg/day→ボーラス0.2-0.5、レミフェンタニルCRI→ロード量 等）
2. **退化パースのガード**: 桁区切りカンマ「1,000 µg/kg」→「000」= min 0、逆転レンジ（10-5）は
   null（ボタン非表示）に
3. **負数入力ガード**: `min="0"` でもタイプでは負数を入れられ、負の投与量・輸液量が表示され得た →
   `_calcNum` が負値を未入力（null）扱いに（計算機の入力は全て非負の量）
4. **上限<下限の逆転レンジ表示**（「5–2 mg」）→ `Math.max` で正規化
5. **言語切替の再描画で入力値・選択タブが全消失**（体重以外）→ 再描画前に input/select 値と
   アクティブタブを退避・復元（係数・換算方向selectは選択肢再構築後に復元、タブ復元は
   GA4イベントを発火しない silent モード）
- 副産物: `printAnesthesiaChecklist` がバイト単位同一で**3重定義**されていた（マージ痕跡の
  死コード124行、後勝ちで動作影響なし）→ 1定義に統合、回帰テストで再発防止

### 競合パリティ続弾: IRIS CKDステージング・単位換算・リーン体重（VD_CALC拡張、全てnode実測検証）
- **IRISタブ（新設）**: IRIS 2023 CKDステージング — クレアチニン（mg/dL・µmol/L両対応、
  犬 1.4/2.9/5.0・猫 1.6/2.9/5.0）+ SDMA（犬 18/36/>54・猫 18/26/>38）は高い方のステージを表示し
  乖離時は筋肉量低下例の注記、UPCサブステージ（犬 0.2-0.5・猫 0.2-0.4 境界）、
  血圧サブステージ（140/160/180）。**「安定したCKD専用 — AKI・脱水・輸液中の値では判定しない」の
  臨床ガードを常時表示**。検査基準値リファレンスの中で国際標準化されている部分を実装
  （汎用の血液基準値表は分析機依存のため意図的に非実装 — 各施設のRIを優先すべき）
- **単位換算タブ（新設）**: 血糖・クレアチニン・BUN・Ca・P・T-Bil・T4・コルチゾール・lb↔kg・°F↔°C
  の10項目を従来単位↔SI相互変換（海外文献・海外分析機対応）。項目変更で方向selectを自動再構築
- **リーン体重（BCS補正）**: カロリータブにBCS(9段階)入力を追加。BCS>5は1ポイント≈+10%過体重として
  推定理想体重を表示（Laflamme 1997; WSAVA 2011）、**減量係数選択時はRER/MERを理想体重基準で計算**
  （係数値に `_wl` マーカー、parseFloatは接尾辞無視）。BCS≤5は推定式適用外のため現体重（捏造しない）
- チョコ中毒タブに「リスク閾値は犬のデータ（猫はより感受性が高い）」の種注記を追加

### 飼い主向け説明シート印刷（Plumb's/Vetlexicon の owner handout 相当）
- 疾患DB詳細のcross-navに「🖨 飼い主向け説明シートを印刷」ボタンを新設（委譲ハンドラ、
  キャッシュ再描画後も動作）。麻酔チェックリストと同じ window.open→print 機構
- シート構成: 病名（日英）・動物種・日付・患者名/担当獣医師の記入欄、この病気について（description）、
  主な症状（symptoms_display の翻訳名）、ご家庭での予防・注意（prevention）、経過の見通し（prognosis）、
  緊急度が emergency/high の疾患には赤の緊急注意ボックス、再診の目安
- **臨床用の治療プロトコル（薬品名・用量）は意図的に非掲載**: 「治療とお薬について」は固定文
  （自己判断で中止・増減しない）+ 獣医師が個別指示を手書きするメモ欄。飼い主の用量自己解釈リスクを
  排除し、ECVNスポンサーブロックも同時に非掲載になる。フッターに一般情報免責
- 回帰テストがハンドアウト関数に treatment フィールドが混入しないことをソース検証

### 回帰テスト（tests/test_clinical_calculators.py 3→8件）
- `test_vd_calc_iris_conversion_lean_weight_verified_by_node` — IRIS境界値（犬猫クレアチニン/SDMA/
  UPC/BP 22ケース）・換算係数（血糖5.55・クレアチニン176.8・°F↔°C往復・lb）・理想体重をnode実測
- `test_parse_dose_range_rejects_cri_and_degenerate_doses` — 実配布コードのregexをnode実行
  （ジルチアゼム/コリスチン実データ・CRI先行+ボーラス後続・逆転・カンマ退化の10ケース）
- `test_calculator_input_hardening_in_source` — 負数ガード・レンジ正規化・言語切替退避/復元・
  silent タブ復元・print関数の単一定義
- `test_iris_and_conversion_panels_wired` / `test_owner_handout_is_wired_and_omits_dosing`

### テスト・CI
- 計算機テスト8件 + app.js参照の全9テストファイル126件合格、ruff check/format clean、
  index レンダリングスモーク合格
- ServiceWorker: `CACHE_NAME` v145 → **v146**
- データ変更なし（配信DB・検索インデックスは不変）

### 競合比較ロードマップの残り（次セッション候補）
- 汎用の検査基準値表: 分析機・施設依存のため意図的に見送り（IRISのような国際標準のみ実装する方針）
- 飼い主向けハンドアウトの疾患別カスタム文（現状は疾患フィールドの流用 — 獣医レビュー前提で
  トーン最適化の余地）

## 2026-09セッション（第37弾: 飼い主向けシートの疾患別ホームケア・ポイント + MGCS計算機 — 残ロードマップの実装）

### 背景（第36弾の残ロードマップ続行）
- 汎用の検査基準値表は方針どおり見送りを維持（分析機・施設依存 — IRISのような国際標準のみ実装）。
  その方針に沿う標準化スケールとして **MGCS（修正グラスゴー昏睡スケール）** を計算機に追加
- 飼い主向けシートの「疾患別カスタム文」を、教科書のクライアント指導レベルの内容に限定して実装
  （獣医レビューは従来のキュレート内容と同様にPRレビューで実施）

### 飼い主向け説明シート: 疾患別ホームケア・ポイント（`OWNER_HANDOUT_TIPS`、31エントリ）
- 主要フラグシップ疾患に「ご家庭でのケアのポイント」セクションを追加（該当疾患のみ表示 — 捏造しない）:
  - **犬**: IVDD（ケージレスト・抱き方・排尿確認）、膵炎（低脂肪厳守）、心臓病（**就寝時呼吸数<30/分の
    RSRR指導 — ACVIM 2019**）、アトピー、外耳炎、糖尿病（低血糖時の糖塗布→受診）、てんかん（動画記録・
    5分ルール）、クッシング、GDV予防（分食）、歯周病、OA（体重管理最優先）、CKD
  - **猫**: FIC/尿道閉塞（トイレ数+1・雄の無尿=救急）、甲状腺機能亢進、喘息（開口呼吸=救急）、
    肝リピドーシス（**24時間絶食で連絡**）、FCGS
  - **エキゾチック**: 草食4種GIうっ滞（**12時間ルール**）・不正咬合（牧草）、フェレット・インスリノーマ
    （低血糖対応）/副腎、鳥の卵詰まり・毛引き（フォージング・睡眠）、爬虫類の呼吸器（環境是正=治療の一部）・
    MBD（**UV-B球の定期交換**）、モルモット壊血病（ビタミンC毎日）、チンチラ熱中症（24°C）
  - **馬**: 疝痛ウォッチ（飼料撤去→連絡・**鎮痛剤の自己使用禁止**）、蹄葉炎（青草制限・指動脈チェック）、
    EGUS（粗飼料頻回）、PPID（蹄葉炎監視）
- **用量ゼロ設計**: 全エントリに薬品用量なし（回帰テストが mg/kg・IU・qNh 等の混入をregexガード）。
  各エントリに出典（ACVIM/ISFM/AAHA/Quesenberry & Carpenter/Mader/Sykes 等）を明記しシートにも表示
- 解決は括弧除去後の基底名部分一致＋動物種ゲート（`_ownerTipsFor`）。**全31パターンが配信DB実疾患名に
  解決することをミラーテストでCI固定**（リネーム/dedupeで孤児化したら検出）

### MGCS計算機タブ新設（Platt 2001 JVIM — 頭部外傷の標準化スケール）
- 運動機能・脳幹反射・意識レベル各1-6点（原典の記述選択肢を日英で全文収載）→ 合計3-18
- 予後帯: 3-8 予後不良域 / 9-14 慎重 / 15-18 良好域（48時間生存確率と直線的相関）— node実測で境界検証
- **臨床ガード常時表示**: 犬で検証・血圧/酸素化の安定後に評価・経時トレンドで判断・
  低血圧/低酸素/鎮静はスコアを見かけ上悪化・スコア単独で予後を断定しない
- VD_CALC に `mgcsTotal`（1-6範囲外/未選択はnull）/`mgcsBand` を追加（純関数、既存の抽出テスト機構で検証）

### 回帰テスト（tests/test_clinical_calculators.py 8→10件）
- `test_mgcs_verified_by_node_and_wired` — 境界6点+範囲外/NaN をnode実行、UI配線・ガード文言
- `test_owner_handout_tips_curated_and_resolvable` — nodeでレジストリ抽出→バイリンガル+出典必須・
  **用量語ゼロ**・全パターンの配信DB名解決（ミラーテスト）・ハンドアウト統合配線

### テスト・CI
- 計算機テスト10件 + app.js参照9ファイル128件合格、repo全体 ruff clean、index レンダリングスモーク合格
- ServiceWorker: `CACHE_NAME` v146 → **v147**
- データ変更なし（配信DB・検索インデックスは不変）
...
## 2026-09セッション（第39弾: 全21種コモンズ監査 — 「一般的な疾患が見落とされていないか」の体系的検証と是正）

### 背景（開発者指示「全ての動物種にて一般的な疾患が見落とされていないか、確認。」）
第38弾の可視化レイヤーに続き、全21種の common/very_common 疾患 約800件を対象に
**自己検索監査**（その疾患自身の教科書的症状セットで検索し、(a) 本体が top-3 に入るか、
(b) 未tier/稀なエントリが上位を奪っていないか）を実施。症状セットは Python set のため
サンプリングは実行毎に異なる — 複数回実行のユニオンで是正し、回帰テストは固定症状リストで固定。

### 結果: フラグ 88 → 0-3/run に収束（6ラウンドの是正）
| 是正クラス | 内容 |
|---|---|
| **supplementaryクローン症状セットのキュレート（6件）** | GP涙嚢炎が**泌尿器クローン**（blood_in_urine/straining_to_urinate等）を保持し膀胱炎主訴の1位を奪っていた → 眼徴候セット（eye_discharge/crusty_eyes/眼瞼腫脹+歯根疾患続発のfacial_swelling）に置換。第17弾同系統の残存: チンチラ涙嚢炎・ハムスター中耳炎・鳥卵管炎・ヘビ吻端擦過傷/水疱症 |
| **同一疾患の重複エントリのtier整列（7組）** | リクガメ Vitamin A Deficiency 正準/括弧変異、パラキート Egg Peritonitis/Egg Yolk Peritonitis、鳥 NSHP=MBD、両生類 FV3=Ranavirus、フェレット URI括弧変異・GI異物Linear変異、猫 Lily Nephrotoxicosis=ユリ中毒 — 未tier側が正準側より上に出る**自己重複逆転**を同tier付与で解消 |
| **病歴依存の鑑別の降格** | 胎子遺残（GP/ウサギ — 症状セットが子宮蓄膿症の**厳密なサブセット**で未tierのまま常勝）、避妊手術合併症、繁殖雌限定の妊娠中毒症変異（チンチラ/GP）→ uncommon |
| **異種混入・誤頻度の是正** | 猫: 胆嚢粘液嚢腫=rare（犬の疾患 — 猫は症例報告レベル）・胸腺腫=rare（猫前縦隔腫瘤はリンパ腫が主）・腸管MCT=uncommon・膝蓋骨脱臼=uncommon、鳥: Brachyspira=rare（主に家禽）・大動脈破裂=rare（ダチョウ/七面鳥）・マダニ=uncommon・甲状腺機能低下=rare、フェレット: COVID-19=uncommon・一次性細菌性肺炎=uncommon・白血病=uncommon（3大腫瘍はリンパ腫/インスリノーマ/副腎）、ウサギ: ロタ/クリプト/コロナ腸炎・腎結石・気管支炎、ハムスター/チンチラ: 誤嚥性肺炎=uncommon 等 |
| **真のコモンズへのtier付与** | 未tierだが実際に高頻度で正当な鑑別: 鳥ミコバクテリア様真菌性肺炎（アスペルギルス）・両生類ビタミンA欠乏(short tongue)/Ca欠乏・トカゲ アタデノ/慢性栄養失調/総排泄腔脱・爬虫類 卵性体腔炎/卵胞うっ滞・ハムスター心房血栓症・GP壊血病性関節炎・パラキート肺炎/卵管うっ滞・パロット Gram陰性感染/そ嚢停滞・チンチラ Pseudomonas・ウサギ細菌性肺炎 等 = common |
- 追加tierキー **約70件**（全て配信DBまたはチャットモジュール名に解決することを検証済み。
  dead-keyテストのメトリクスはモジュール名基準で 9/15 のまま — inertキーの新規追加ゼロ）
- 第38弾で cat 側に誤配置していた rabbit 用キー「Urinary Calculi (Ureteral)」を rabbit ブロックへ移動（真のinertキーを解消）

### 回帰テスト（tests/test_common_disease_visibility.py +6件 = 18件）
`TestAllSpeciesCommonsAudit` — 固定症状リストで決定論化:
- GP泌尿器主訴に涙嚢炎が出ない + 眼主訴で涙嚢炎rank1（クローン撲滅の両面）
- GP/ウサギ 陰部分泌物主訴で子宮蓄膿症 > 胎子遺残
- 猫黄疸主訴で肝リピドーシス/胆管炎が top、胆嚢粘液嚢腫は圏外+rare
- ハムスター斜頸+旋回で中耳炎 rank1
- リクガメVA重複ペアの同tier整列 + rank1
- パロットPDD+重金属がtier付きで上位

### 残存フラグの性質（設計上許容）
- 実行毎に0-3件の新規サンプル衝突が出るが、全て「tier付きcommon同士の正当な競合」または
  単発の未tierニッチエントリ。6ラウンドのマルチラン・ユニオンで持続的な逆転は全て解消済み
- MISS（common本体がtop-3外）約50件は同等以上の頻度の正当な鑑別に負けたもの（監査で個別確認）
...
## 2026-09セッション（第41弾: 破傷風抗毒素/フルシトシン補完 + チャット精度第25弾 + 相互作用レジストリ接続）

### エラーチェック（結果: ベースライン健全）
- repo全体 ruff check clean、フルテスト **4,257件合格**（34 skip、ベースライン）
- 配信SQLiteクリーンビルド: **6,893疾患**、treatment/prevention/prognosis **100%**、
  主要臨床フィールド（治療/病因/予後/予防/説明/病態/臨床徴候）の空欄 **0**、キリル文字混入 **0**
- 薬用量: safe薬品の dosage 欠落 **0**（624薬品時点、全species_info検証）、文字列型相互作用スキーマ **0**
- 麻酔: 全21種×全8カテゴリ完備（188プロトコル）、薬剤行の dose 欠落 **0**、全種 references あり
- prevalence dead key: **9**（当該種DBに疾患自体が無い既知残、上限15ガード内。配信名照合で17に見えるが
  8件はチャット経路のモジュール名キーとして意図的に併設されたもの — 両方に不達のみが真の dead）
- 薬品マッチャー飽和度: 片仮名トークン上位候補（ノルモソル/リンゲル/ウルソデオキシコール/クラブラン等）は
  全て実フレーズで解決済み（bare フラグメントの見かけ上の未解決はスニペット突合で否定）

### referenced-but-absent 薬品2剤の補完（`drug_batch_58.py` 新規、624→626薬品）
片仮名+英語薬品様トークン監査（第24回スイープ、実フレーズ突合）で検出:
- **破傷風抗毒素（TAT）** — 自辞書の犬破傷風「TAT 100-500 IU/kg IV/IM…試験量投与後」・馬破傷風
  「TAT 1,500-10,000 IU IV/IM」が用量付きで名指しするのに本体未収載だった自己参照ギャップ
  （ネオスチグミン/アスピリン/プロタミン/プレドニゾンと同型）。馬破傷風は開発者専門種のフラグシップ救急。
  馬: 治療 10,000-50,000単位 IV/IM + 創傷時予防 1,500-3,000単位（Reed & Bayly 4th; Plumb's 10th）、
  **タイラー病（血清肝炎 — 投与4-10週間後の劇症肝壊死、equine parvovirus-H関連）** という製剤固有の
  医原性リスクを明記。犬猫: ウマ由来血清のため皮内試験量先行（Greene 4th）。受動免疫のみ＝トキソイド
  別部位同時接種の定義的事実、混合によるトキソイド不活化を相互作用に収載。bare「TAT」は非索引
  （ブルセラ血清学 2ME-TAT への誤爆をテストで固定）
- **フルシトシン（5-FC）** — 猫/鳥クリプトコッカス併用療法エントリが「25-50 mg/kg PO q6-8h」等で
  用量付き参照するのに未収載。**犬は safe:False**（重度皮膚薬物反応→TEN様反応 — Malik; Sykes。
  「フルシトシンが猫・鳥の薬であって犬の薬ではない」決定的理由）。単剤耐性化→アムホテリシンB±アゾール
  併用必須を禁忌に明記。アムホテリシン腎毒性によるクリアランス低下・シタラビン拮抗を相互作用に収載
- **英語表記エイリアス3件**: Cobalamin（31 EN参照）→vitamin_b12、Silibinin→milk_thistle、
  Calcium glubionate / グルビオン酸カルシウム（36参照 — EN先頭語 calcium がストップリスト、
  JA名はシロップ付きで索引不達だった）→calcium_glubionate
- 動線検証: 逆引き「この薬品を使う疾患」= tetanus_antitoxin **23疾患**（馬/猫破傷風・クロストリジウム症等）/
  flucytosine 5 / calcium_glubionate 41。薬品タブ検索・相互作用チェッカー自然言語解決も確認

### 相互作用チェッカーへのレジストリ接続（新規薬品の実効化）
- per-monograph の `drug_interactions` フィールドは**表示専用**で、併用チェッカーはキュレート済み
  `INTERACTIONS` レジストリ（drug_interactions.py）のみ参照する設計 — フルシトシン×アムホテリシンB
  （クリプトコッカス標準併用の監視事項）とフルシトシン×シタラビン（拮抗）をレジストリにも登録し、
  チェッカーで1タップ検出可能に。回帰テストでレジストリ到達を固定

### 診断チャット精度 第25弾（22症例フレッシュスイープ 6 MISS → 22/22 合格）
- **チンチラ「耳が垂れて元気がない 耳から臭い」が中耳炎不達**: 耳介下垂の語彙が皆無 →
  耳が垂れて/片耳が垂れ→ear_drooping 新設 + _ID_SYNONYMS チェーン（head_tilt/head_shaking/
  ear_discharge）→ 中耳炎・内耳炎 rank 1（従来は筋萎縮/貧血のクローンセットが上位）
- **猫「毛玉を何度も吐く 便が出ていない」**: 進行形「便が出ていない/出てない」が「便が出ない」キーに
  不一致 → エイリアス3種追加 → 便秘・宿便/巨大結腸症/猫毛球閉塞症が top3
- **猫「お尻を舐めてばかりいる 便に血」**: 肛門部舐めの語彙が皆無 → お尻を舐め/おしりを舐め/肛門を舐め
  →scooting → 瓜実条虫症 rank 1・肛門嚢ddx接続
- **ヘビ「口の中に白いチーズ状のもの」単独で細菌性肺炎が1位**: 乾酪様滲出物エイリアス8種が
  非特異ID mucus_in_mouth（肺炎の泡沫とも共有）に解決していた → **stomatitis（マウスロットの実体）へ
  是正**（爬虫類4種すべての語彙に実在、他種は _ID_SYNONYMS 末尾追加の mouth_lesions/mucus_in_mouth で
  安全フォールバック、ハムスター等は抽出段階で脱落）→ 感染性口内炎 rank 1
- **オウム・シックバード三徴「羽をふくらませて…便が緑色」が抽出・ランクとも不達**:
  (a) かな表記「羽をふくらませて」欠落 → エイリアス追加、(b) **_SYN に緑色便ブリッジが無く**
  parrot オウム病の lime_green_droppings 表記に届かなかった → diarrhea_green↔lime_green_droppings↔
  green_droppings 相互ブリッジ新設、(c) 未tierクローン（低体温症/心臓弁膜症/殺鼠剤中毒）を
  uncommon/rare に → オウム病 rank 1（parrot）・top3（parakeet — 合成生殖器クローンペア
  Reproductive Congenital Anomaly=rare / Inflammatory Disease=uncommon も是正）
- **鳥「多飲+水様便」**: bird の E. coli Infection 括弧なし重複エントリを (Colibacillosis)=very_common に
  整列、Citrobacter=uncommon（日和見）、Avian Coronavirus=rare（家禽IBV系 — 伴侶鳥では実質不在）
- **マッチャーエンジンのシノニムcoverage二重計上は今回は見送り**: coverage側のソースグループ化+
  曖昧主訴の重み継承是正を試作したが、意図的に調整済みの19ランキング回帰（blocked cat/インスリノーマ/
  MBD等）を崩すためリバート。既存の的を絞った手法（エイリアス/tier/クローンガード）で全MISSを解消。
  エンジン側の原理的改善は「全ランク回帰テストの期待値を獣医レビュー付きで再調整するセッション」を
  前提とする次回以降の候補として記録
- 回帰テスト: `TestChatClinicalAccuracyAuditRound25`（7件）+ 既存2テストの期待値を stomatitis 移行に更新 +
  `_KNOWN_ALIAS_MISMATCHES` に ear_drooping 追加

### UX: クイック入力の拡充（検証済み新主訴の1タップ導線）
- チンチラ「耳が垂れて耳から臭い」（→中耳炎・内耳炎 rank 1）・猫「お尻を舐めてばかりいる」
  （→条虫/肛門嚢ddx）。ミラーテスト JA_QUICK 同期（全フレーズ抽出保証をCIで維持）
- 新規薬品2剤は既存の治療チップ/逆引き/相互作用チェッカー機構で鑑別診断・チャット結果カード・
  疾患DBの3ビューから自動到達（破傷風エントリ→TATチップ、クリプトコッカス→フルシトシンチップを検証済み）

### 表示数値の同期・キャッシュ
- `setDefaultStats()`: dog 568/cat 546/horse 361/鳥系 238薬品、pendingStats drugs 624→**626**
...
- 薬用量: safe薬品の dosage 欠落 **0**（626薬品時点、全species_info検証）、文字列型相互作用スキーマ **0**
- 麻酔: 全21種×全8カテゴリ完備（188プロトコル）、薬剤行の dose 欠落 **0**、全種 references あり
- prevalence: 配信名非一致キー17（うち8はチャット経路のモジュール名キーとして意図的併設、真の残9 — 既知）
- 薬品マッチャー飽和度（第25回スイープ）: 片仮名+英語トークン×用量文脈の上位候補を実スニペットで
  find_drugs_in_text 突合 — クロトリマゾール/ボリコナゾール/水酸化アルミニウム/エリスロポエチン/
...
- **メタゾラミド** — 犬緑内障フラグシップ・ハリネズミ緑内障・小型哺乳類緑内障ガイダンスが
  ...
  （アセタゾラミドのみ収載）。犬の全身性CAI第一選択（Gelatt's Veterinary Ophthalmology 6th ed;
  Plumb's 10th ed）。クラス定義的安全事実を収載: **点眼CAI（ドルゾラミド）との併用は眼圧下降の
  追加効果なく毒性のみ相加**（経路はどちらか一方）、サリチル酸併用=重度代謝性アシドーシス（major）、
  肝疾患禁忌（アンモニア排泄低下→肝性脳症）。猫は低用量+点眼優先、ハリネズミは自サイト参照用量
- 動線検証: テキストマッチャー解決（メタゾラミド/methazolamide）・逆引き「この薬品を使う疾患」5疾患・
  相互作用チェッカー自然言語解決（メタゾールアミド表記ゆれ含む）

### レガシー犬DBに食餌性中毒2疾患を新設（83→85疾患、79→81症状）
- **レガシーチャットDBに中毒エントリがゼロ**だったため、「玉ねぎ入りハンバーグを食べてしまった
  元気がない 尿が赤い」が尿路結石症1位になっていた（日本の犬の最頻食餌性中毒相談なのに）。
  犬モジュール（チェックボックス経路）には両疾患とも収載済みで、チャット経路だけの欠落
- **摂取文脈フラグ2症状を新設**（postpartum_lactating と同型）: `onion_ingestion`
  （玉ねぎを食べ/ネギを食べ/ニラを食べ/ニンニクを食べ/玉ねぎ入り 等9エイリアス）、
  `chocolate_ingestion`（チョコレートを食べ/チョコを食べ/チョコを盗み食い）
- **タマネギ・ニンニク中毒**（common）: N-プロピルジスルフィド酸化障害→ハインツ小体性溶血性貧血、
  加熱無効・摂取後1-5日遅発、15-30 g/kg目安（Cope 2005; Plumb's 10th ed）
- **チョコレート中毒**（common）: メチルキサンチン中毒、犬半減期17.5h、20/40-60/100+ mg/kg の
  リスク帯、催吐2-4時間窓+活性炭反復（腸肝循環）。**臨床計算機（チョコ中毒タブ）への誘導を本文に明記**
- **摂取フラグ・ゲート型設計**（pyometra ガードと同型）: 全身徴候（痙攣・振戦・嘔吐・蒼白粘膜・
  色素尿）は全て非特異的で、セットに持たせると素の痙攣主訴でてんかんを、溶血主訴でIMHAを
  乗っ取ることを検証で確認 → 症状セットは摂取フラグのみに限定し、パトグノモニック・クラスタ
  {onion_ingestion}×1.8 / {onion_ingestion, dark_urine}×2.0 / {chocolate_ingestion}×1.8 /
  {chocolate_ingestion, tremors}×2.0 で摂取申告時の rank 1 を保証
- 検証: 玉ねぎ+溶血徴候→中毒1位・溶血性貧血2位、チョコ+振戦→中毒1位・てんかん2位、
  素の痙攣→てんかん1位維持、素の溶血徴候→溶血性貧血/IMHA上位維持（回帰テストで固定）
- 疾患名は犬モジュールと完全一致（Onion/Garlic Toxicosis・Chocolate Toxicosis）— チャット候補カードの
  「疾患DBで詳細を開く」ピボットが base-name 完全一致で着地

### 診断チャット精度 第26弾（24症例フレッシュスイープ 5 MISS → 23/24 + 順位品質1件是正）
- **猫の蹠行姿勢（かかと歩行）が抽出ゼロ**: 糖尿病性神経障害のpathognomonicで cat語彙に
  plantigrade_stance が実在するのに飼い主表現が皆無 → かかとをつけて歩/かかとを地面につけ/
  かかとで歩/ぺたぺた歩 エイリアス + _ID_SYNONYMS フォールバック（他種は後肢虚弱へ）
  → 「後ろ足のかかとをつけて歩く ぺたぺた歩き」で糖尿病 rank 1
- **猫異所性尿管が排尿主訴の1位を奪取**: 先天奇形（若齢雌の持続性尿失禁、猫では犬よりさらに稀 —
  Holt & Gibbs）が未tier小セット（3症状）で「トイレに何度も行くけど出ていない」の1位だった
  → rare 付与 → 尿路感染症/尿道閉塞系が上位
- **猫傍腫瘍性脱毛症が高齢猫主訴の1位を奪取**: 膵癌/胆管癌マーカー（真に稀 — Turek 2003
  Vet Dermatol）が未tierで「毛づくろいしなくなって痩せた」（CKD/甲状腺機能亢進の古典像）の
  1位だった → rare 付与 + 毛づくろいをしなくな/毛がボサボサ→poor_coat エイリアス → CKDがtop6に
- **両生類レッドレッグ症候群が自身の主訴で引けなかった**: 「皮膚が赤くなって足に出血斑」が
  骨折/ヒル寄生上位だった → 出血斑/皮膚に赤い斑点→petechiae エイリアス新設（両生類はネイティブ
  petechiae=レッドレッグの定義的所見、他種は skin_hemorrhage/hemorrhage へフォールバック。
  フェレットはネイティブ petechiae=エストロジェン性骨髄抑制に正しく接続）+ _SYN 出血系相互ブリッジ
...
- チョコ計算機→疾患エントリ（第35弾）の**逆方向が dead end** だった: チャットでチョコレート中毒が
  1位に出た瞬間の次のアクションは「量からのリスク計算」そのものなのに、計算機への直接導線が無かった
- `DISEASE_CALC_MAP`（種ゲート付き）新設: チョコレート中毒→チョコ中毒タブ（犬）、
  慢性腎臓病(CKD)→IRISステージングタブ（犬猫 — IRIS 2023はイヌネコで検証されたスケールのため種ゲート）。
  マップが解決しない疾患にはボタン自体を出さない（dead end ゼロ設計）
- **4面に配線**: チェッカー結果カード（renderCalculatorCrossLink）・自由チャット候補カード・
  問診最終結果カード（_chatCalcLink）・疾患DB詳細 cross-nav（🧮 臨床計算機ボタン）。
  3つの委譲ハンドラ全てに `.calc-nav-link`→`openClinicalCalculators({tab})` ルーティング
  （キャッシュ再描画後も動作）。GA4 `calculator_from_disease`
- クイック入力に「チョコレートを食べてしまった」（犬）を追加 — タップ→チョコレート中毒 rank 1 →
  計算機ピボット＋緊急対応プロトコルピボット（既存マップに解決）の完全動線
- 回帰テスト: `test_app_js_disease_to_calculator_pivot_wired`（マップ・4面レンダラー・3ハンドラ配線）
...
## 2026-09セッション（第45弾: 猫ガバペンチンの詳細化 — 開発者リクエスト対応）

### 背景（開発者からの直接リクエスト「猫のガバペンチンの使い方なども詳しく」）
猫のガバペンチンは実臨床で最も使われる薬の一つなのに、収載内容が「3-10 mg/kg PO q8-12h ＋
来院前ストレスに有効」の2行のみで、用途別のプロトコル・エビデンス・安全上の実用知識が欠けていた。

### 猫行の全面拡充（エビデンスベース・3用途を構造化）
1. **来院前・移動ストレス軽減**（ISFM/AAFP 2022 Cat Friendly ガイドラインの第一選択）:
   50-100 mg/頭 の**体重非依存固定用量**を移動の1.5-3時間前（健康成猫100 mg・小柄/高齢/虚弱50 mg、
   前夜＋当日朝の2回投与でより安定 — van Haaften 2017 JAVMA: 来院90分前100 mg/頭でストレススコア
   低下・ハンドリング協力性が有意に改善）。カプセル開封→ウェットフード混和の実投与法も記載
2. **慢性疼痛（OA・神経障害性）**: 5 mg/kg（25 mg/頭）q12h から開始し最大10 mg/kg q8-12hへ漸増
   （Guedes 2018 JAVMA RCT: 高齢OA猫で10 mg/kg q12hが飼い主評価の活動性を改善）。
   FHS（知覚過敏症候群）・断脚後神経障害性疼痛の補助も明記
3. **CKD猫の減量**: 肝代謝なし・腎排泄のため高窒素血症で血中濃度上昇 — **約50%に減量**
   （来院前≤50 mg/頭・慢性2.5-5 mg/kg）し鎮静の程度で調整（Quimby, JFMS薬物動態）
- **安全・実用ノート**: 効果ピーク1-2時間・鎮静/ふらつき/流涎は約8時間で消失、来院前投与は
  白衣高血圧を減らす一方で測定血圧をやや下げうる（血圧評価来院での解釈注意）、
  **ヒト用キシリトール含有経口液剤の使用禁止**、単回投与は漸減不要・慢性投与は1-2週漸減

### 犬行も同時補強
- 状況性不安（雷・花火・来院）: 10-20 mg/kg をトリガーの1.5-2時間前（トラゾドン併用可）を追記
- キシリトール含有ヒト用液剤の犬毒性（低血糖・肝壊死）警告、腎機能低下時の約50%減量を明記

### 配信経路の確認
- 薬品API（/api/drugs・/api/drugs/<id>）は Python の DRUGS リストから直接配信 — SQLite の drugs
  テーブルは件数統計にのみ使用のため、**内容変更はデプロイで即反映**（migrate 不要）。
  batch_3/batch_5 のエキゾチック種パッチ（GP/ハムスター/ハリネズミ/チンチラ）は不変を検証

### 回帰テスト（tests/test_drug_dictionary.py TestFelineGabapentinDetail、+4件）
- 猫行が3用途＋3引用（van Haaften/Guedes/Quimby）を日英で保持、固定用量50-100 mg/頭・CKD 50%減量
- 安全ノート（キシリトール警告・血圧解釈・漸減指針）の維持
- 犬行の状況性不安＋キシリトール警告、エキゾチックパッチ行の生存

### テスト・CI
- 薬品テスト群 263件合格＋新規4件、ruff check/format clean
- 静的アセット変更なし（サーバーサイドのみ）— ServiceWorker 据え置き

## 2026-09セッション（第46弾: 頻用薬6剤のガバペンチン水準拡充 — 「その他にも充実化」対応）

### 背景（開発者リクエスト「その他にも充実化を図る」）
第45弾（猫ガバペンチン）に続き、日本の小動物臨床で使用頻度が最も高く「使い方のニュアンス」が
臨床価値に直結する6剤を監査 — いずれも用量欄が20-60字の骨格のみだった — を同水準に拡充。

### 拡充した6剤（全て日英対応・エビデンス引用付き）
1. **ミルタザピン**: 猫=健常 1.88 mg q24h／**CKDは半減期延長で q48h**（Quimby 2011 AJVR PK・
   Quimby 2013 JVIM クロスオーバー）・肝疾患 q72h・≥3.75 mgは副作用のみ増加。
   **ミラタズのラベル誤記を是正**（旧「2インチ」→ 正 **2 mg＝1.5インチ（約3.8 cm）**、
   FDA NADA 141-481、Poole 2019 JVPT RCT）＋手袋・耳交互・拭き取りの投与指導。
   セロトニン症候群の拮抗=シプロヘプタジン。犬=0.6 mg/kg q24h（上限30 mg/日）＋体重帯
2. **トラゾドン**: 犬=状況性 5-7 mg/kg をトリガー1.5-2時間前（自宅試験投与→不足時8-10）、
   術後ケージレスト 3.5→7 mg/kg（Gruen 2014 JAVMA）、入院ストレス（Gilbert-Gregory 2016）。
   猫=**来院前 50 mg/頭 単回・出発1-1.5時間前**（Stevens 2016 JAVMA RCT）。チル・プロトコル
   （＋ガバペンチン）とセロトニン症候群カウンセリング
3. **ブプレノルフィン**: 猫=OTM 0.02-0.04 mg/kg（アルカリ性唾液で注射並みBA — Robertson 2005、
   自宅術後鎮痛に最適）＋ **Simbadol 0.24 mg/kg SC q24h×最長3日（6倍濃度 — 液量流用禁止）**＋
   猫オピオイド高体温監視。犬=**OTM吸収不良（Ko 2011: 0.12 mg/kgで漸く有効域）→注射優先**。
   天井効果→重度疼痛はメサドン/フェンタニルへ
4. **メチマゾール**: 低用量開始（AAFP 2016）→2-3週毎にT4+CBC+肝腎再検→T4基準下半分目標。
   経皮PLOゲル 2.5-5 mg q12h（Sartor 2004）。**重篤副作用（顔面掻爬・肝障害・血球減少）は
   減量でなく中止**（I-131/手術/食事へ）。**腎マスキング**（正甲状腺化後15-40%で高窒素血症
   顕在化 — 高窒素血症のみでは中止せず、医原性甲状腺機能低下を回避: Williams 2010 JVIM）
5. **アムロジピン**: 猫=第一選択（ACVIM 2018/Acierno 2018）0.625 mg開始（SBP≥200/TODは1.25）
   →1-2週毎に SBP<160（理想<140）へ最大2.5 mg。**白衣効果の除外**とTOD裏付け、急性網膜剥離=
   即日降圧の緊急。犬=**単剤はRAAS活性化→ACEi/テルミサルタン併用**
6. **ピモベンダン**: 犬=**EPIC基準のB2開始**（雑音≥3/6＋LA/Ao≥1.6・LVIDDN≥1.7・VHS>10.5 —
   前臨床期間を約15ヶ月延長: Boswood 2016）・**B1は適応なし**・食前1時間・PROTECT（ドーベルマン
   DCM）。猫=**LVOTO/SAMを心エコーで除外してから**の適応外使用（Reina-Doreste 2014 JAVMA）、
   HOCMは禁忌側

### 配信・整合
- 薬品APIはPython DRUGSリストから直接配信 — デプロイで即反映（migrate不要）
- batch 3/5 のエキゾチックパッチ行（GP/チンチラ/ハリネズミ/フェレット等）は全剤で不変を検証
- トラゾドンの配信エントリは `drug_batch_5.py`（重複するbatch_18側は非配信）

### 回帰テスト（tests/test_drug_dictionary.py TestHighFrequencyDrugDetailBatch2、+7件）
- 各剤の引用・プロトコル数値・安全パール（ミラタズ1.5インチ＋旧2インチの死守、Simbadol 6倍濃度、
  メチマゾール中止規則＋腎マスキング、EPIC B2基準＋B1除外、LVOTOゲート、RAAS併用則）を日英で固定
- パッチ行生存＋dog/cat行の完全バイリンガル検証

### テスト・CI
- フルテストスイート **4,351件合格**（34 skip）。初回ランで `test_no_safe_true_with_contraindicated_notes`
  が新規ピモベンダン猫notesの「Contraindicated/禁忌」文言を検出（safe=True行のnotesにこの語は不可 —
  HOCM禁忌はエントリレベル contraindications が既に保持）→「使用を避ける/Avoid」に修正して全合格
- repo全体 ruff clean、静的アセット変更なし（サーバーサイドのみ）— ServiceWorker 据え置き

## 2026-09セッション（第47弾: 頻用薬拡充・第2陣 — 抗てんかん薬の血中濃度モニタリング＋トリロスタンACTH刺激プロトコル等6剤）

### 背景（開発者承認済みの候補リスト続行）
第46弾の提示候補（フェノバルビタール/レベチラセタムの血中濃度モニタリング・トリロスタンの
ACTH刺激プロトコル・シクロスポリン・オクラシチニブ等）を開発者が採択 — いずれも用量欄が
20-50字の骨格のみだった6剤（＋日本の実臨床で主力のゾニサミド）をガバペンチン水準に拡充。

### 拡充した6剤（全て日英対応・エビデンス引用付き）
1. **フェノバルビタール**: 犬=2.5-3 mg/kg開始（ACVIM Podell 2016）→**開始/変更2週後に血中濃度**
   （定常状態では採血タイミング影響小 — Levitski & Trepanier 2000）目標15-35 μg/mL・
   **比例計算での用量調整式**・安定後6ヶ月毎＋CBC/肝パネル（**誘導性ALP上昇≠肝障害 —
   ALT/胆汁酸で評価**）・ローディング12-24 mg/kg分割IV・漸減中止規則・CYP450誘導。
   猫=第一選択（ISFM/Finnerty 2014）1.5-2.5 mg/kg・目標15-30・偽リンパ腫パール
2. **レベチラセタム**: 犬=20 mg/kg q8h／XR 30 q12h・**PB併用でクリアランス約2倍→効果不十分は
   増量**（Moore 2011）・群発パルス60→20 mg/kg（Packer 2015）・IVブリッジ・腎排泄減量。
   **治療域が広く定期血中濃度測定は通常不要**（PB/ZNSとの実務上の対比）・ハネムーン効果（Volk 2008）。
   猫=20 mg/kg q8h（Bailey 2008）CKD減量
3. **トリロスタン**: 犬=1-2 mg/kg q24h**食事と共に**→夕方再燃/糖尿病併発はq12h分割（Vaughan 2008）。
   **ACTH刺激: 開始/変更10-14日後・投薬後4-6時間・目標刺激後1.45-5.4 μg/dL**→30日/90日/3ヶ月毎＋
   電解質。代替=pre-pillコルチゾール（Macfarlane 2016）。**中止規則**（<1.45 or 消化器症状→中止して
   受診・回復後減量再開）＋「嘔吐/食欲不振の日はスキップ」の飼い主指導。
   猫=**q12hに是正**（代謝が速い — Mellett Keith 2013、旧q24h）＋インスリン減量予期
4. **シクロスポリン**: 犬=5 mg/kg q24h・**効果発現4-8週→オクラシチニブ/短期ステロイドでブリッジ**
   （Olivry ICADA 2015）→q48h→週2回へ漸減（Steffan 2006: 約半数がq48h以下で維持）・初期GI対策
   （カプセル冷凍/食事同時）・**ケトコナゾール併用の用量節約**（意図的CYP3A阻害）・アトピーでは
   血中濃度不要。禁忌欄の「ケトコナゾール併用」を「管理外の併用」に是正（節約プロトコルとの矛盾解消）。
   猫=7 mg/kg（ラベル）＋**トキソプラズマ**（致死的全身性の報告 — Lappin 2015。室内・生肉禁止・
   開始前血清検査）＋FeLV/FIVスクリーニング
5. **オクラシチニブ**: 犬=q12h**最長14日→必ずq24h維持へ**（長期q12hは感染/毛包虫症リスク）・
   4時間で効果（Cosgrove 2013）・**12ヶ月齢以上**・**皮内/IgE検査に非干渉**（減感作精密検査中も
   継続可 — Olivry）・対症薬（中止で再燃→基礎管理並行）。
   猫=**代謝が速くq12h継続が必要**（犬式q24h減量は不十分 — Lopes 2019）・ステロイド/CsAが第一選択
6. **ゾニサミド**: 犬=単剤5 mg/kg q12h／**PB併用は10 mg/kg q12h開始**（クリアランス約2倍 —
   Orito 2008）・**血中濃度: 1-2週後・目標10-40 μg/mL**・日本は犬用コンセーブ承認（本邦主力）・
   サルファ過敏回避＋KCS/シルマー値パール。猫=**q24h**（半減期約33時間 — Hasegawa 2008、旧q12-24h）

### 配信・整合
- 配信エントリは6剤とも `drug_dictionary.py` 本体（oclacitinib の batch_19 側は非配信）。
  トリロスタンの猫/馬行のみ `drug_batch_5.py` の SPECIES_INFO_PATCH_5 — 猫行をそこで是正
- エキゾチックパッチ行（phenobarbital 馬/ウサギ/チンチラ、levetiracetam 鳥系、cyclosporine 馬等）
  は全剤で生存を検証

### 回帰テスト（tests/test_drug_dictionary.py TestHighFrequencyDrugDetailBatch3、+7件）
- 各剤のモニタリング数値（15-35/15-30・4-6時間・1.45-5.4・10-40）・引用・安全パール
  （比例計算式・ALP≠肝障害・PB併用倍量・14日ステップダウン・トキソ・スキップ指導・猫q12h/q24h是正）
  を日英で固定＋パッチ行生存＋完全バイリンガル検証

### テスト・CI
- 薬品+安全テスト 277件合格、repo全体 ruff clean、フルテストスイート合格
- 静的アセット変更なし（サーバーサイドのみ）— ServiceWorker 据え置き。薬品APIはPython直配信のため
  デプロイで即反映（migrate不要）

## 2026-09セッション（第47弾・並行セッション分: フィゾスチグミン/チルミコシン/アトルバスタチン補完 + チルミコシン誤用法是正 + 犬レプトスピラ症新設 + チャット精度第29弾）

### エラーチェック（結果: ベースライン健全）
- repo全体 ruff check clean、フルテスト **4,352件合格**（34 skip、ベースライン）
- 配信SQLiteクリーンビルド: **6,893疾患・634薬品**、treatment/prevention/prognosis **100%**、
  主要臨床フィールドの空欄 **0**、キリル文字混入 **0**
- 薬用量: safe薬品の dosage 欠落 **0**、文字列型相互作用スキーマ **0**
- 麻酔: 全21種×全8カテゴリ完備（188プロトコル）、薬剤行の dose 欠落 **0**、全種 references あり
- prevalence 真の dead key: **9**（当該種DBに疾患自体が無い既知残、上限15ガード内。
  配信名非一致20のうち11はチャット経路のモジュール名キーとして意図的併設）
- 薬品マッチャー飽和度（第28回スイープ、カタカナ+英語×用量文脈の実フレーズ突合）:
  ノルモソルR/乳酸リンゲル/ウルソデオキシコール酸/バイオチン/ティルドロネート/プロジンク等
  上位候補は全て解決済みを確認

### referenced-but-absent 薬品3剤の補完（`drug_batch_63.py` 新規、634→637薬品）
- **フィゾスチグミン** — 自サイトの中毒プロトコル3件（鳥の抗コリン性痙攣 0.02-0.06 mg/kg IV slowly・
  犬イベルメクチン昏睡 0.06 mg/kg IV・ウサギナス科植物中毒）が用量付きで名指しする解毒薬なのに
  未収載だった自己参照ギャップ（2-PAM/プロタミン/デクスラゾキサンと同型）。**第三級アミンで
  血液脳関門を通過**（第四級のネオスチグミンとの定義的差異=中枢抗コリン症状を拮抗できる根拠）。
  有機リン・カーバメート中毒への使用禁忌（AChE阻害の相加→コリン作動性クリーゼ）・アトロピンの
  レスキュー準備・TCA過量時の心静止リスクを明記（Plumb's 10th; Peterson & Talcott 3rd）
- **チルミコシン（ミコチル/プルモチル）** — 山羊CCPP/豚マイコプラズマ肺炎エントリが用量付き参照。
  獣医薬で最も安全ゲートが重要な部類: **静注は全種（牛含む）で致死的・豚馬霊長類は注射経路自体が
  致死的・ヒト誤自己注射の死亡例（米国ラベルに医師ホットライン）**。馬/犬/猫 safe:False。
  ブタではアドレナリンが致死性を増強（レスキューにならない）を相互作用に収載
- **アトルバスタチン（リピトール）** — 鳥の大動脈破裂/血栓塞栓症エントリが「0.3-1 mg/kg PO q24h
  （外挿）」で参照する、オウム類アテローム性動脈硬化症（高齢アマゾン・ヨウム剖検最多クラス —
  Beaufrère 2013）のスタチン療法。オウムPK研究（Beaufrère 2015）の血中濃度限定的という
  正直なエビデンス併記＋食事転換が最重要介入。**イトラコナゾール（アスペルギルス症治療）との
  CYP3A4相互作用=major**（鳥臨床でまさに現実的な併用）を収載
- 動線検証: 逆引き「この薬品を使う疾患」= physostigmine 4/tilmicosin 2/atorvastatin 6疾患、
  テキストマッチャー・相互作用チェッカー自然言語解決（ミコチル/リピトール/エゼリン）全て接続

### 臨床的に危険な用法記述の是正（チルミコシン）
- 山羊CCPPエントリの「チルミコシン 10 mg/kg SC q24h（注射は心毒性→緩徐投与）」は
  **反復投与＋緩徐静注を示唆する二重に危険な記述**だった（静注は全種で致死的、SCも単回が原則）
  → 「10 mg/kg SC 単回（再投与は最短72時間後。静注は全種で致死的—絶対禁忌。ヤギは10 mg/kg超で
  死亡例。ヒト誤自己注射も致死リスク）」に日英とも修正（Micotil label; Plumb's 10th）

### 診断チャット精度 第29弾（44症例フレッシュスイープ 7 MISS → 全症例合格）
- **犬レガシーDBにレプトスピラ症を新設**（86→87疾患、84→85症状）: 人獣共通の届出感染症なのに
  エントリが無く、「川遊びのあと発熱して黄疸」が肝臓病/IMHAのみで**水系曝露歴が一切使われて
  いなかった** → `stagnant_water_exposure` 曝露文脈フラグ（川遊び/川の水を飲/水たまりの水/
  池の水/田んぼの水/用水路 — Sykes 2011 ACVIM consensus）+ 発熱・黄疸・多飲・嘔吐・褐色尿の
  教科書的症状セット（tier=uncommon）+ クラスタ {曝露}×1.5/{曝露+発熱}×1.8/{曝露+黄疸}×2.0/
  {発熱+黄疸+多飲}×1.5 → 曝露申告時 rank 1。**ゲート検証**: 素の黄疸+嘔吐+食欲不振→肝臓病1位・
  溶血三徴→溶血性貧血1位は不変（回帰テストで固定）。名前は dog モジュール「Leptospirosis」と
  完全一致（疾患DBピボット着地）
- **逆くしゃみ「みたいな」形**: 「しゃっくりみたいな呼吸」（のような形のみ収載）→ BOAS rank 1
- **挙上肢位**: 「足を上げたまま」が抽出ゼロで pain 単独→緑内障1位だった → limping 解決で
  膝蓋骨脱臼/肘形成不全/CCL が top-3
- **黒色耳垢**: 「耳の中が黒い垢だらけ」（黒い耳垢キーに不一致）→ ear_discharge で耳ダニ rank 1
- **ヘビの弛緩性脱力**: 「とぐろを巻かず」「だらんとして」が抽出ゼロ → loss_of_righting_reflex
  新設エイリアス（IBD/スターゲイジングの hallmark — Mader 3rd ed）→ 封入体病/スターゲイジング
  症候群が top-3。他種は ID_SYNONYMS チェーン（muscle_weakness/weakness/ataxia）で安全
- **馬の前肢 hot-hoof**: 「前足をかばって」（足/脚表記差）「蹄が熱くて」（連用形）「指動脈の拍動」
  が全て欠落 + **未tierの3所見蹄エントリ**（蹄関節疾患/蹄異物/低蹄関節リングボーン/蹄冠損傷/
  蹄踵狭窄・剪断）がカバレッジで蹄葉炎・蹄膿瘍を抑圧 → エイリアス6種追加 + 6エントリを
  uncommon tier 化 + Acute Laminitis=common + fore ペアの膿瘍ブースト 1.4→1.5（hind と同値）
  → 急性蹄葉炎 rank 1・蹄膿瘍が top クラスタ内。後肢ペアの蹄膿瘍 rank 1 は不変

### UX: クリック動線
- `DISEASE_EMERGENCY_MAP` に レプトスピラ症→aki（犬）を追加 — 乏尿性AKIが本疾患の救急実体。
  チャット候補カード/チェッカー結果/疾患DBから「🚨 緊急対応プロトコル」ワンタップ
- クイック入力に犬「川や水たまりの水を飲んだ後に発熱」を追加（タップ→レプトスピラ症 rank 1、
  ミラーテスト JA_QUICK 同期）
- 新規薬品3剤は既存の治療チップ/逆引き/相互作用チェッカー機構で鑑別診断・チャット結果カード・
  疾患DBの3ビューから自動到達（検証済み）

### 回帰テスト（+16件）
- 薬品: TestBatch63（6件 — 3剤の存在・完全バイリンガル用量・定義的安全事実（BBB通過・OP禁忌・
  静注致死ゲート・ヒト誤注射警告・アドレナリン増悪・アゾールmajor）・危険用法テキストの再発防止
  JSON走査・マッチャー/チェッカー解決）
- チャット: TestChatClinicalAccuracyAuditRound29（10件 — レプトスピラ rank1×2+非ハイジャック
  ガード、逆くしゃみ変化形、挙上肢位→整形、猫耳ダニ、ヘビIBD+他種フォールバック、
  馬 fore/hind/連用形+指動脈の4ケース）

### 表示数値の同期・キャッシュ
- `setDefaultStats()` 10種の diseases/drugs を実測同期（dog 579/cat 556/horse 363/rabbit 261/
  鳥系 242/exotic_other 3 等）、pendingStats drugs 634→**637**・symptoms 84→**85**
- ServiceWorker: `CACHE_NAME` v154 → **v155**
- 再現手順: `migrate_to_sqlite.py`（クリーンビルド 6,893疾患・637薬品）— 疾患名不変のため
  検索インデックス no-op

## 2026-09セッション（第48弾: 丁寧語正規化 — チャット診断精度の最大の系統的欠陥の是正）

### 背景（開発者フィードバック「まだまだチャットや類症鑑別相談の診断精度が低い」）
過去29ラウンドの個別修正（エイリアス・tier・クラスタ）を重ねてもなお低いという指摘を受け、
個別対応ではなく**体系的測定→系統的故障クラスの特定→根本修正**のアプローチで対応。
83症例の現実的な飼い主主訴コーパス（丁寧語・話し言葉・変化形を意図的に混在、12種+犬・馬経路）を
新規作成して測定した結果、**丁寧語（ます体）の主訴がほぼ全滅**という最大級の系統的欠陥を発見:
「食べません」「吐いています」「飲みます」「痩せてきました」「元気がありません」「出ません」が
全経路で抽出ゼロ（プローブ17件中12件が全滅）。飼い主が最も自然に打つ形が全く通じていなかった。

### 丁寧語正規化層 `normalize_chat_text()`（symptom_extractor.py 新設）
- **形態論的に常に正しい置換のみ**をテーブル化（60+ペア）: 〜ています→〜ている・
  〜ていました→〜ている・〜ていません→〜ていない・ありません→ない・〜まりません→〜まらない
  （〜まる動詞で常に正しい）・します/しました/しません、および高頻度症状動詞の個別活用
  （食べません→食べない、飲みます→飲む、吐きました→吐いた、立たなくなりました 等30+動詞）
- **〜ません→〜ない の一括置換は意図的に不採用**（五段動詞で誤活用: 飲みません→飲みない ✗）。
  テーブルに無い動詞は変換しない（誤生成ゼロの保守設計、回帰テストで固定）
- 3経路すべてに適用: 汎用種（_extract_species_symptoms）・レガシー犬（extract_symptoms_from_text
  手動パス）・馬（_extract_equine_symptoms）。否定形（〜ていない等）は正規化後も保存され
  否定ガードと共存

### キー自動展開3種（symptom_aliases.py モジュールロード時、既存のてる⇄ている展開と同機構）
1. **い形容詞→く形**: 「体が冷たい」→「体が冷たく」（冷たくて動かない が抽出可能に）。
   否定内容エイリアスも「食欲がなくなった」形を獲得
2. **ている→て語幹**: 「吐いている」→「吐いて」（吐いていて/吐いており/吐いてから を捕捉）。
   安全ガード: 語幹3文字以上+漢字必須+危険語幹ブロックリスト（「張って」=頑張って誤爆 等13語）
3. **が→も助詞バリアント**: 「糞が小さく」→「糞も小さく」（2つ目以降の症状は も で導入される）
- **キュレート接頭辞シャドウガード** `_shadows_curated_prefix()`: 生成キーがより短いキュレート済み
  キー（別ID）を最長一致で影にする場合は生成しない（「甲羅が柔らかい」→soft_bones のく形が
  キュレート語幹「甲羅が柔らか」→soft_shell を上書きするバグを検出・防止）
- てる⇄ている補完を全生成後に最終パスで再適用（生成順序に依存しない不変条件）
- 実効エイリアス: 2,584 → **3,705キー**（+1,121自動生成）

### 否定ガードの Phase 3 穴を修正（両経路）
- Phase 1/2 が全て否定でゼロ件になると **Phase 3（フラグメント分割）が否定ガードなしで再マッチ**し、
  「嘔吐はしていません。下痢もありません」が vomiting+diarrhea を抽出していた → 汎用種・レガシー犬
  両方の Phase 3 に is_negated_mention を適用（陽性コントロール不変を検証）

### ランキング是正（エビデンスベース）
- **猫UTI tier very_common→common**: 猫の細菌性UTIは若齢猫で稀（LUTSの主因は FIC 55-65%、
  UTI 8-15%で高齢・CKD併発に偏る — ISFM/AAFP; Sykes; Lund）。排尿主訴で常にFIC・閉塞より
  上位に出る clinically wrong なランキングだった
- **無尿ペアのブースト1.35→1.5**: 「おしっこが出ない」申告は閉塞 until proven otherwise
  （FICは定義上尿産生がある）→「何度もトイレに行くのにおしっこが出ません」で尿道閉塞 rank 1
- **新パトグノモニック・フロア**: 猫{頻尿+無尿}→閉塞、爬虫類4種{stomatitis}→感染性口内炎
  （マウスロット — Mader 3rd ed）、ウサギ{maggots_visible}→ハエウジ症（時間単位の救急）1.4、
  ウサギ{くしゃみ+鼻汁}→パスツレラ（スナッフル複合 — Harcourt-Brown）1.25
- **tier整列**: ウサギ 鼻腔ポリープ=rare（猫の疾患）・アスペルギルス=rare・鼻腔内異物=uncommon・
  副鼻腔炎=common・Demodex=rare（Quesenberry: ウサギ毛包虫症は真に稀）、馬 白筋症=uncommon
  （子馬のSe/VitE欠乏疾患）・悪性高熱=rare（麻酔関連）・セレン欠乏=uncommon —
  成馬のタイイングアップ主訴で未tierの3所見エントリがER/PSSMを抑圧していた
- **新規エイリアス**: 足を引きずっ（促音便語幹）、吐きそうなのに吐けな（GDV）、立たなくなっ、
  チーズのような（マウスロット）、お尻に虫/ウジ虫/蛆（フライストライク）+ maggots_visible
  ID_SYNONYMS チェーン、卵が出かかっ（卵塞）、高齢/年寄り→年齢抽出（最頻の高齢表現が未収載だった）

### 効果（83症例コーパス実測）
| 経路 | Before | After |
|---|---|---|
| 全体（top-N合格率） | 64/83（77%） | **83/83（100%）** |
- 丁寧語プローブ 17件: 12 MISS → **0 MISS**
- 否定文の誤抽出: 2系統 → **0**（陽性コントロール不変）

### 回帰テスト（TestChatClinicalAccuracyAuditRound30、+20件）
- 正規化ユニット（誤活用を生成しない保守性含む）・丁寧語3経路・否定Phase3両経路・
  く形/て語幹/が→も展開・シャドウガード・閉塞rank1+非無尿FICガード・フライストライク・
  スナッフル・卵塞・マウスロット×2・タイイングアップ・GDV・高齢年齢抽出

### テスト・CI
- フルテストスイート: **4,343件合格**（86 skip → 配信DB再構築後の served-db 系 200件も合格）、
  カバレッジ 82.56%
- ruff check / format: repo全体 clean
- 変更はPythonのみ（静的アセット不変のため ServiceWorker 据え置き。疾患データ・配信DBも不変 —
  正規化・展開・tierは実行時参照）

## 2026-09セッション（第49弾: チャット精度第31弾 — 可能形正規化 + レガシー犬DBに常同障害・乳頭腫症 + 馬・跛行フロア）

### エラーチェック（結果: ベースライン健全）
- repo全体 ruff check clean、フルテスト **4,395件合格**（34 skip、ベースライン）
- 配信SQLiteクリーンビルド: **6,893疾患・637薬品**、主要JAフィールド（治療/病因/予後/予防/説明/病態）の空欄 **0**
- 薬用量: safe薬品の dosage 欠落 **0**、文字列型相互作用スキーマ **0**
- 麻酔: 全21種×全8カテゴリ完備（188プロトコル）、薬剤行の dose 欠落 **0**、全種 references あり
- 薬品マッチャー飽和度（第29回スイープ、カタカナ+英語×用量文脈の実スニペット突合 + モノグラフ自己参照監査）:
  真の欠落 **0**（残存トークンは疾患名・薬剤クラス名・手技名のみ — 辞書は637剤で飽和を再確認）

### 診断チャット精度 第31弾（52症例・2波スイープ 15 MISS → 52/52 合格）
- **丁寧語正規化の残穴: 可能形**（第48弾テーブルの続き）: 「食べられません」「走れません」が未変換で
  猫の口腔痛主訴が抽出不能だった → `（ら）れません→（ら）れない`・`れました→れた` を追加
  （ます語幹が「れ」で終わる全動詞で形態論的に常に正しい — かもしれません→かもしれない も正変換）。
  「落ちます/落ちました」も追加（鳥の止まり木落下が polite 形で不達だった）
- **否定ガードとの衝突バグ**: 新キー「吐きたそうなのに吐け」が入力「〜吐けない」の末尾「ない」を
  否定と誤判定して自己遮断 → 既存慣行（吐きそうなのに吐けな）に合わせ「な」までをキーに含めて解決
- **レガシー犬DBに常同障害・乳頭腫症を新設**（87→89疾患、85→87症状）:
  - **常同障害・強迫性障害**: 尾追い・肢端舐性は頻出の行動主訴なのに CDS しか無く
    「自分のしっぽを追いかけてかじっています」が抽出ゼロだった → tail_chasing 症状 +
    compulsive_disorder エントリ（ブルテリア回転/GSD尾追い/ドーベルマン脇腹吸引の好発、
    Overall 2013; Moon-Fanelli 2011 JAVMA）+ 単独クラスタ×1.6 → rank 1。
    症状セットは反復行動シグナルのみに限定（pyometra ゲートと同型 — 汎用掻痒を乗っ取らない）
  - **犬乳頭腫症（イボ・脂腺腫）**: 「いぼがたくさん」主訴（若齢=パピローマウイルス/高齢=脂腺腫、
    Muller & Kirk 7th ed）が抽出ゼロだった → wart_like_growths 症状 + papillomatosis エントリ +
    単独クラスタ×1.6 → rank 1。ひらがな「いぼのような」キー新設（カタカナは round-24 収載済み）
  - 両エントリとも EN base-name が dog モジュール（Compulsive Disorder (Canine OCD) /
    Canine Papillomatosis）と一致 — チャット候補カード「疾患DBで詳細を開く」ピボットが完全一致着地
- **犬チェックボックス/問診経路のパリティ**: dog モジュールに tail_chasing 語彙
  （behavioral カテゴリ）を追加し Compulsive Disorder 2エントリの症状セットに付与 →
  チェックボックスでも常同障害が rank 1（scooting/oral_mass/cyanosis パリティと同型）
- **新規エイリアス・チェーン**: 痛がって鳴い（て形 — 猫ATE啼鳴）、頭を振っ（語幹 — 外耳炎 rank 1）、
  がぶがぶ飲→excessive_thirst（クッシング top5）、夜中に走り回→hyperactivity
  （高齢猫の夜間活動亢進+啼鳴→甲状腺機能亢進症 rank 1 — Carney 2016 AAFP）、
  おしっこが白っぽ/ドロドロ→thick_white_urine（ウサギ尿路スラッジ/高Ca尿症 top3 —
  語彙に実在するのに飼い主表現が皆無だった）、脚の力が抜け→leg_weakness、
  点々が動い/赤い点々が動→visible_mites（ヘビダニ rank 1）、口で呼吸→open_mouth_breathing
  （リクガメ肺炎 rank 1）、しっぽを追いかけ→tail_chasing。
  ID_SYNONYMS 新設5系統: tail_chasing/thick_white_urine/leg_weakness/visible_mites の
  種フォールバックチェーン（非保有種は安全に脱落・代替IDへ）
- **鳥・卵詰まりのパトグノモニック・フロア**: 「お尻から卵が出かかっています」（時間単位の救急）で
  卵管炎・慢性産卵が卵詰まり（難産）をカバレッジで上回っていた → bird {egg_binding}→
  Egg Binding (Dystocia) ×1.45（Ritchie & Harrison; Bowles 2002）→ rank 1
- **馬**: 喘息の飼い主表現（ゼーゼー/ヒューヒュー→resp_abnormal_lung_sounds、
  運動すると咳→resp_cough_at_exercise）新設 → 馬喘息 (IAD/RAO) top3。
  非負重跛行「急に片足を全く着けなくなりました」が抽出ゼロ → 足を着けな/片足を着け 系エイリアス +
  **跛行所見→蹄膿瘍の症候群フロア（減額値0.42）**: 蹄膿瘍は急性重度跛行の最多原因
  （Adams & Stashak 7th ed）なのに未tierの2所見エントリ（中手骨骨膜炎/手根骨炎/橈骨神経麻痺）が
  カバレッジで独占していた → フロア機構に per-finding 減額オーバーライドを新設
  （パトグノモニックではなく「最多原因」のため、発熱+咳+跛行の呼吸器像では1位を奪わない —
  ガードテストで固定）+ 5エントリを uncommon tier 化（Bucked Shins/Carpitis/橈骨神経麻痺/
  後天性屈腱拘縮/二頭筋滑液包炎 — 競走馬・外傷ニッチ）→ 蹄膿瘍 rank 1。
  hot-hoof ペア既存テストは「膿瘍か蹄葉炎 until proven otherwise」の top2 セット検証に更新

### UX: クイック入力の拡充（検証済み新主訴の1タップ導線）
- 犬「自分のしっぽを追いかけてかじる」（→常同障害 rank 1）、ウサギ「おしっこが白っぽくてドロドロ
  している」（→尿路スラッジ/高Ca尿症）、鳥「お尻から卵が出かかっている」（→卵詰まり救急 rank 1）
- ミラーテスト JA_QUICK 同期（全フレーズ抽出保証をCIで維持）

### 回帰テスト（+18件: TestChatClinicalAccuracyAuditRound31）
- 可能形正規化（誤活用を生成しない保守性含む）、なのに形GDV+否定ガード共存、外耳炎・クッシング・
  猫甲状腺亢進、常同障害/乳頭腫症 rank 1 + 他種フォールバック + チェックボックスパリティ、
  ウサギスラッジ、鳥卵詰まりフロア + 脚力低下 polite、ヘビダニ、リクガメ口呼吸、
  馬喘息・非負重跛行→蹄膿瘍 rank 1 + 呼吸器像非ハイジャックガード
...
## 2026-09セッション（第51弾: しか〜ない否定ガード修正 + 丁寧語正規化の残穴3系統 + 馬チョーク両所見解決 + レガシー犬歯科・喉頭語彙 + レギュラーインスリン破損修正 + ランディングチャットのチップ誤配線修正）

### エラーチェック（結果: ベースライン健全）
- repo全体 ruff check clean、フルテスト **4,434件合格**（34 skip、ベースライン）
- 配信SQLiteクリーンビルド: **6,893疾患・639薬品**、treatment/prevention/prognosis **100%**、
  主要臨床フィールド（治療/病因/予後/予防/説明/病態）の空欄 **0**
- 薬用量: safe薬品の dosage 欠落 **0**、文字列型相互作用スキーマ **0**、ID重複 **0**
- 麻酔: 全21種×全8カテゴリ完備（188プロトコル）、薬剤行の dose 欠落 **0**、全種 references あり
- 薬品マッチャー飽和度（第31回スイープ、カタカナ×用量文脈 + 実マッチャー突合）: 真の欠落 **0**
  （ニスタチン/ダーベポエチン/ロイプロリド等の見かけ上の未解決は全て正準名差のエイリアス解決を確認 —
  辞書は639剤で飽和を再確認）

### 疾患コンテンツの文字破損修正（レギュラーインスリン）
- 猫糖尿病エントリのDKAプロトコルが「**規ュラーインスリン**CRI」（レギュラーインスリンの文字化け —
  漢字+小書きカナは日本語として存在しない並び）になっていた → JSON 2箇所 + cat_diseases.py 1箇所を修正。
  修正により insulin_regular（batch 35）の関連薬品チップも当該エントリで解決
- 同型破損（漢字直後の小書きカナ）の全域スキャンで他の混入 **0** を確認。回帰テスト: 禁止トークンに追加

### 診断チャット精度 第33弾（40症例・2波スイープ 17 MISS → 全症例合格）
- **否定ガードの「しか〜ない」誤遮断を修正**（原理的バグ）: 「ポタポタと**しか**出ない」（滴下排尿=陽性症状）が
  否定と誤判定され抽出ゼロだった → is_negated_mention がマッチ直前8文字に「しか」を検出した場合は
  発火しない（しか は必ず否定述語を要求し、構文全体は限定の肯定表現）。素の否定（咳はない）は不変
- **丁寧語正規化の残穴3系統**: 〜ておらず→〜ていない（おらず=おる の文語否定でて形後は全動詞正しい）、
  〜がります/がりました/がりません→がる活用（がります で終わる動詞は全て五段〜がる: 痛がる・嫌がる・
  転がる・上がる）、歩きます/歩きました → 「うんちが出ておらず」「後ろ足を痛がっています」
  「ペタペタ歩きます」が全経路で抽出可能に
- **馬チョークの両所見解決**: 「鼻から餌」（飼料の鼻腔逆流）は嚥下不能の発現そのものなので、
  EQUINE_SYMPTOM_ALIASES に**tuple値サポート**を追加し (resp_bilateral_discharge, dig_salivation) の
  両所見に解決 → 既存のチョーク症候群ペアが発火。さらにチョーク自身の clinical_signs_detail が記載する
  嚥下時咳嗽（Feige EVJ 2000）を associated_findings に反映（従来は咳併記の主訴で呼吸器感染に
  カバレッジ負けし top5 圏外だった）→ 「餌の後に咳き込み鼻から餌」でチョーク rank 1
- **馬の急性呼吸器三徴の是正**: アスペルギルス症の最小3所見セット {発熱,咳,鼻汁} が coverage 1.0 で
  rare tier でも常勝する artifact だった → 自身の詳細が記載する肺型の慢性経過（体重減少・呼吸困難）を
  所見に反映 + 未tierの競合8件を整合（EHV-4/細菌性肺炎=common、誤嚥性肺炎/肺膿瘍=uncommon、
  真菌性肺炎/成馬ロドコッカス/類鼻疽（東南アジア限定）/コクシジオイデス（Americas endemic）=rare）
  → インフルエンザ/腺疫が上位に
- **発熱時の跛行フロア抑制**: 蹄膿瘍フロア（0.42）は「単純な足部像」専用の設計意図なのに、
  Aspergillosis の過剰スコアが偶然蓋をしていたため発熱+咳+跛行の全身像でも rank 1 を取ってしまう
  ことが顕在化 → gen_fever 併存時は limb_lameness フロアを発火させない（素の跛行→膿瘍 rank 1 は不変）
- **レガシー犬DBに difficulty_eating 症状を新設**（87→88症状）: 「片側だけで噛む」「口を触ると嫌がる」の
  歯科疼痛主訴が bare pain 経由で**緑内障 rank 1** になっていた → 歯周病の症状セットに追加
  （AAHA Dental 2019。口腔内腫瘍への追加は乳腺ブリッジとの coverage 希釈で口腔腫瘤主訴を
  乱すことが判明し撤回 — 回帰テストが検出）→ 歯周病 rank 1
- **ウサギ後肢麻痺フロア**: 「後ろ足で立てなくなり引きずる」で未tierの2所見・爪過成長エントリが
  カバレッジ勝ちしていた → {hind_limb_paralysis}→後肢不全麻痺/脊椎骨折・脱臼 ×1.35
  （ウサギの急性後肢麻痺は保定不良によるL7骨折 until proven otherwise — Harcourt-Brown）。
  素の跛行主訴は対象外（爪過成長/ソアホックの日常ddx維持をガードテストで固定）
- **トカゲ・ラバージョーフロア**: {jaw_softening}→MBD ×1.5（ラバージョーはNSHPのpathognomonic —
  Mader 3rd ed）+ Nephrocalcinosis=uncommon（未tier4所見エントリが後肢虚弱併記で MBD を抑圧していた）
- **新規エイリアス（約35キー）**: 耳から茶色い液/耳から液（外耳炎 rank 1）、水がすぐなくな・
  おしっこの量が増え（PU/PD → 糖尿病/CKD top）、片側だけで噛/片方の歯で噛、後ろ足を痛が/前足を痛が/
  足を痛が→lameness_or_limping（膝蓋骨/CCL/OA top-3）、ペタペタ歩（カタカナ形 → 猫糖尿病性神経障害
  rank 1）、うんちが出ていない系3キー、便のまわりに水/便の周りが水っぽ→polyuria（鳥の尿部分拡大 →
  腎不全/糖尿病）、むせて/声がガラガラ/ガラガラ声（喉頭麻痺 rank 1）、おしっこに時間がかか/
  ポタポタとしか出（膀胱炎/尿石症）、吐こうとして吐けな/吐けず・えずいて/えずく→unproductive_retching
  （えずく は旧 coughing 誤マッピングを是正 — えずく=空嘔吐。ケンネルコフの咳+えずき主訴は
  rank 1 不変をガードで固定）、歯茎が赤/歯ぐきが赤→bleeding_gums（猫歯周病群）、砂のようなものが混ざ、
  耳が真っ赤（チンチラ熱中症 rank 1）、横になったまま/横たわったまま→lethargy、お腹が大きく膨れ/
  お腹がふくれ、鼻の穴のまわりが汚れ（鳥副鼻腔炎）、前肢をかば/後肢をかば（解剖学用語形 —
  hot-hoof ペア発火で蹄膿瘍/急性蹄葉炎 top-2）
- 回帰テスト: `TestChatClinicalAccuracyAuditRound33`（20件）

### UX: ランディングチャットのクイック入力チップ誤配線修正
- **バグ**: ヒーロー（ランディング）チャットと本体チャットは両方常にDOMに存在するため、
  ランディング側のチップをタップすると `getElementById("chatInput")` 先勝ちで**画面外の本体チャットに
  送信され、返答はユーザーが見ていない場所に描画**されていた（ランディングパネルは無反応に見える）
- チップが属するコンテナ（`closest("#landingChatMessages")`）を判定して自パネルの input/send に
  ルーティング。回帰テスト: `test_app_js_landing_quick_tap_routes_to_landing_chat`
- クイック入力に検証済み新主訴3件を追加: 犬「水を飲むとむせて声がガラガラ」（喉頭麻痺）、
  猫「かかとをつけてペタペタ歩く」（糖尿病性神経障害）、トカゲ「あごが柔らかくてぶよぶよ」（MBD）。
  ミラーテスト JA_QUICK 同期（全フレーズ抽出保証をCIで維持）

### 表示数値の同期・キャッシュ
- pendingStats symptoms 87→**88**（difficulty_eating追加）
- ServiceWorker: `CACHE_NAME` v157 → **v158**
- 再現手順: `migrate_to_sqlite.py`（クリーンビルド — cat DKA修正・馬チョーク/アスペルギルス所見反映。
  疾患名不変のため検索インデックス no-op）

## 2026-09セッション（第52弾: 次亜塩素酸/フェリウェイ/アダプティル補完 + チャット精度第34弾 + オムニボックス→計算機動線 + SGストレス症候群の病態テンプレート是正）

### エラーチェック（結果: ベースライン健全）
- repo全体 ruff check clean、フルテスト **4,455件合格**（34 skip、ベースライン）
- 配信SQLiteクリーンビルド: **6,893疾患・639薬品**、treatment/prevention/prognosis **100%**、
  主要臨床フィールド（治療/病因/予後/予防/説明/病態/臨床徴候）の空欄 **0**、キリル文字・破損カナ混入 **0**
- 薬用量: safe薬品の dosage 欠落 **0**（全species_info検証）、文字列型相互作用スキーマ **0**、ID重複 **0**
- 麻酔: 全21種×全8カテゴリ完備（188プロトコル）、薬剤行の dose 欠落 **0**、全種 references あり
- 薬品マッチャー飽和度（第32回スイープ、片仮名+英語×用量文脈の実スニペット突合）: 既存参照は
  ほぼ全解決（ノルモソルR/乳酸リンゲル/クラブラン酸/ウルソデオキシコール酸/バイオチン/
  ティルドロネート/イソクスプリン等を実フレーズで確認）

### referenced-but-absent 薬品3剤の補完（`drug_batch_65.py` 新規、639→642薬品）
実フレーズ突合で残っていた真のモノグラフ欠落は全て製品クラス:
- **次亜塩素酸ナトリウム（希釈漂白剤）** — **1,200+参照でコーパス最多の未解決**（「次亜塩素酸1:32」の
  パルボ/FeLV/カリシのケージ消毒・ウサギロタの「漂白剤1:32」バリアナーシング・皮膚糸状菌1:10）。
  クロルヘキシジン/ポビドンヨードは収載済みなのに、サイト自身が処方する環境消毒薬の本体が無かった。
  クラス定義的事実を文書化: 希釈表（1:32≈0.16%・接触10分・皮膚糸状菌1:10）、有機物失活（先に清掃）、
  **アンモニア混合絶対禁止（塩素ガス=major）**、無効病原体（ティザー芽胞・コクシジウム/
  クリプトスポリジウムのオーシスト — サイト自身のティザー病エントリと整合）、濃厚液誤飲=腐食性救急で
  **催吐禁忌**、魚は器具のみ（safe:False・脱塩素必須）、鳥の塩素ガス感受性（Greene 4th; Moriello 2017）
- **フェリウェイ（F3フェイシャルフェロモン）** — FIC/FLUTDフラグシップのMEMOプロトコルが名指し
  （53参照）。ディフューザー連続稼働・スプレーは15分前・多頭対立はマルチキャット製剤という使い分けと、
  **エビデンスの限定性（Frank 2010 JAVMA systematic review）を正直に併記** — MEMO補助であり
  単独治療にしない（Mills & Mills 2001; Gunn-Moore & Cameron 2004; Buffington 2006）
- **アダプティル（DAP犬アピージングフェロモン）** — 犬行動疾患群（音響恐怖・分離不安・CDS）が
  42参照。カラー=持続ストレッサー/ディフューザー=24-48時間前開始/スプレー=15分前の使い分けと
  行動修正併用の原則（Mills 2006; Gaultier 2005; Landsberg 2015; Frank 2010）
- 動線検証: 逆引き「この薬品を使う疾患」= sodium_hypochlorite **273疾患** / feliway_f3 30 /
  adaptil_dap 20。治療チップ・相互作用チェッカー自然言語解決（ひらがな含む）も全接続

### 診断チャット精度 第34弾（61症例・2波スイープ 11 MISS → 全実用症例合格）
- **丁寧語スイープの残穴**: 「おしっこがほとんど出ていません」（ブロック猫の代表的主訴 —
  ほとんど 割り込み+否定キー）→ decreased_urination 新キー4種 → 尿道閉塞 rank 1
- **ず形**: 「後ろ足に力が入らず」（フェレット・インスリノーマ主訴）→ hind_limb_weakness →
  インスリノーマ/低血糖 top-2。足/脚のず形も同時収載
- **レガシー犬DBに歯根膿瘍（眼下膿瘍）を新設**（91→92疾患）: 犬の眼窩下腫脹+排膿の最多原因
  （上顎第4前臼歯根尖膿瘍 — Niemiec, BSAVA Dental Manual）なのにエントリが無く、
  「顔の片側が腫れて目の下から膿」が眼瞼疾患/蕁麻疹上位だった → パトグノモニック・ペア
  {facial_swelling, eye_discharge}×1.5 で rank 1（名前は dog モジュール「Tooth Abscess」と
  完全一致 — 疾患DBピボット着地。結膜炎主訴の非ハイジャックをガードテストで固定）
- **SGうつ・ストレス症候群の病態テンプレート是正（臨床誤り）**: pathophysiology_ja が
  「栄養障害である」という誤カテゴリテンプレートだった → HPA軸活性化→常同行動（旋回・ペーシング）→
  自咬症の行動学的連続体に日英とも是正 + 症状セットに circling を追加
  （Johnson-Delaney; Quesenberry & Carpenter 4th ed）→ 夜間反復旋回の主訴でストレス症候群が
  鑑別に出るように（前庭ddxと共存）
- **甲羅損傷の_SYNブリッジ**: 「甲羅に傷」→shell_lesions がフラグシップ甲羅腐敗症
  （shell_pitting/discoloration/soft_spots表記）に届かず、未tierの病原体名変異
  （Pseudomonas/Beneckea/Geotrichum）が上位独占 → shell_lesions→[pitting, discoloration,
  soft_spots, shell_rot, skin_ulceration] ブリッジ + 3変異=uncommon + 「臭い液が出」→foul_odor
  エイリアス → 甲羅腐敗症 rank 1
- **ウサギ盲腸便付着（sticky bottom）**: 最頻の飼い主相談なのに付着表現が皆無 →
  「お尻の周りに軟らかい便」等7キー→abnormal_cecotropes（新設ID_SYNONYMSチェーンで非保有種は
  soft_stool/diarrheaへ）→ 盲腸便摂取障害/盲腸内細菌叢異常 top-2
- **変化形ギャップ**: うずくまっ（促音便語幹 — 自動て語幹展開の漢字必須ガードでかな語幹は
  生成されないため手動）、水も飲まず→loss_of_appetite（adipsia プロキシ）、耳の付け根を掻/
  黒いカスが出（猫耳ダニ rank 1）、尻尾の皮がむけ（しまいました形 — デグー尾剥脱）、
  脱皮した皮が目（位置修飾形 — ヘビ眼鏡鱗停滞 rank 1）、白目のあたりが白く/白目が濁っ
  （デグー白内障 top-3）
- ガード検証: 猫急性腹症（水も飲まず+うずくまり+腹痛）は消化管閉塞スペクトラム+胆管炎が上位
  （猫膵炎の腹痛は~25%のみ — Steiner。強制ブーストは行わない）

### UX: オムニボックス→臨床計算機のワンタップ動線（新規） + クイック入力3件
- **計算機がグローバル検索から到達不能だった**: 「輸液」「CRI」「チョコ中毒」「換算」と検索しても
  計算機への動線ゼロ（チェッカー/麻酔/チャットのアクション行は既存） → `_CALC_SEARCH_TABS`
  （キーワード正規表現→タブID、かな折り畳み済みクエリで判定）を新設し、マッチ時に
  「🧮 臨床計算機（タブ名）を開く」アクション行を表示 → `openClinicalCalculators({tab})` で
  該当タブに直接着地。チョコ/輸液/CRI/カロリー/輸血/BSA/IRIS/MGCS/換算/用量 の10インテント。
  GA4 `calculator_from_search`
- クイック入力に検証済み新主訴3件: 犬「顔の片側が腫れて目の下から膿が出る」（歯根膿瘍）、
  猫「耳の付け根を掻いて黒いカスが出る」（耳ダニ）、ウサギ「お尻の周りに軟らかい便がつく」
  （盲腸便） — ミラーテスト JA_QUICK 同期（全フレーズ抽出保証をCIで維持）
- 新規薬品3剤は既存の治療チップ/逆引き/相互作用チェッカー機構で鑑別診断・チャット結果カード・
  疾患DBの3ビューから自動到達（検証済み）

### 回帰テスト（+17件）
- 薬品: TestBatch65HypochloriteAndPheromones（4件 — 3剤の存在・完全バイリンガル用量・
  クラス定義的安全事実（1:32+10分・アンモニア=major・ティザー/オーシスト無効・魚safe:False・
  催吐禁忌）・フェロモンの正直なエビデンス枠組み（Frank 2010・MEMO補助・単独治療にしない）・
  マッチャー/チェッカー解決8ケース）
- チャット: TestChatClinicalAccuracyAuditRound34（12件 — ブロック猫丁寧形/フェレットず形/
  歯根膿瘍rank1+結膜炎ガード/猫耳基部/ウサギ盲腸便/甲羅腐敗rank1/SGストレス症候群top5+
  病態が行動学的でありcirclingを保持/デグー白内障・尾剥脱/ヘビ・スペクタクル位置変異）
- UX: test_omnibox_calculator_action_routes_to_matching_tab（キーワード表→タブ実在の整合検証付き）

### 表示数値の同期・キャッシュ
- `setDefaultStats()`: dog 582/cat 559/rabbit 262/鳥系 244/reptile 103/tortoise 110/snake 109/
  lizard 105/fish 29薬品、pendingStats drugs 639→**642**
- ServiceWorker: `CACHE_NAME` v158 → **v159**
- 再現手順: `migrate_to_sqlite.py`（クリーンビルド 6,893疾患・642薬品）— 疾患名不変のため
  検索インデックス no-op

## 2026-09セッション（第53弾: 捏造薬品エントリの是正 + 新規腫瘍薬2剤の公開 + 副詞挿入の系統的正規化 + 腫瘍→BSA計算機動線）

### エラーチェック（結果: ベースライン健全）
- repo全体 ruff check clean、フルテスト **4,472件合格**（34 skip、ベースライン）
- 配信SQLiteクリーンビルド: **6,893疾患・642→644薬品**、treatment/prevention/prognosis **100%**
- 薬用量: safe薬品の dosage 欠落 **0**、文字列型相互作用スキーマ **0**、ID重複 **0**
- 麻酔: 全21種×全8カテゴリ完備（188プロトコル）、薬剤行の dose 欠落 **0**、全種 references あり
- 薬品マッチャー飽和度（第33回スイープ、カタカナ×用量文脈 + モノグラフ自己参照監査）:
  実フレーズ突合で真の欠落 **0**（ノルモソルR/乳酸リンゲル/MMF/鉄デキストラン/
  カルシウムグルコン酸/αインターフェロン等の上位候補は全て解決済みを再確認）

### 捏造薬品エントリの発見・是正（臨床的に危険なデータ破損）
- **「Epofolaner (Zenrelia)」は実在しない薬品だった**: epofolaner という外部寄生虫薬は存在せず、
  ブランド Zenrelia の実体は**イルノシチニブ（Elanco の犬アトピー用JAK阻害薬、2024年FDA承認）**。
  実在ブランドを架空の薬品名+誤った薬効クラス（イソオキサゾリン系・月1回ノミダニ予防）に
  結び付けた過去エンリッチメントの捏造で、獣医師が Zenrelia を検索すると誤クラスの用量が出ていた
- batch_15 のエントリを正しい **イルノシチニブ（ゼンレリア）** モノグラフに全置換:
  0.6-0.8 mg/kg PO q24h（導入期から1日1回 — オクラシチニブとの差別化点）、
  **枠組み警告**（投与中はワクチン免疫応答が不十分になりうる — 接種前後のラベル休薬、
  2026年ラベル改訂で「致死的ワクチン誘発性疾患」文言は削除・免疫応答警告は維持）、
  12ヶ月齢未満不可・JAK阻害薬同士を重ねない。旧 id は `_DRUG_ALIAS_TO_ID` で新 id に解決
  （URL・ブックマーク維持）、旧名エポフォラネルは search_aliases で検索・マッチャーとも到達可能

### エビデンスベースの新規腫瘍薬2剤の公開（`drug_batch_66.py` 新規、642→644薬品）
- **ラバクフォサジン（タノベア）** — 犬リンパ腫に対して**史上初めてFDAが正式承認した薬剤**
  （2016条件付き→2021正式承認）なのに、自サイトのリンパ腫レスキューテキストが
  「ラバクフォサジン（L-SARN）」と garbled 注記付きで名指しするだけで未収載だった。
  1 mg/kg 30分点滴 q21日×5（ボーラス不可）、**ウエスト・ハイランド・ホワイト・テリア禁忌**
  （致死的肺線維症の品種素因）、特徴的皮膚障害・遅発性肺線維症の監視（Thamm 2017; Saba 2018）。
  garbled「（L-SARN）」注記も正しいタノベア表記+用量に是正
- **チギラノールチグラート（ステルフォンタ）** — 非転移性皮膚型MCTの腫瘍内注射薬
  （De Ridder 2021 JVIM RCT: 単回注射28日後の完全奏効75%）。1 mg/mL溶液を腫瘍体積1 cm³あたり
  0.5 mL・上限0.25 mL/kg かつ総量5 mL・腫瘍≤10 cm³（USラベル/DailyMed）。
  **必須併用プロトコル**（プレドニゾロン＋H1＋H2 — 脱顆粒死の予防）と、
  腫瘍壊死→脱落→二次治癒が作用機序そのものである旨の飼い主カウンセリングを明記
- **鑑別診断・チャットへの動線**: 犬MCTエントリに Stelfonta 行・犬アトピーエントリに
  Zenrelia 行を追記（日英、エビデンス引用付き）→ 関連薬品チップ・逆引き
  「この薬品を使う疾患」（rabacfosadine→Lymphoma、tigilanol→MCT、ilunocitinib→アトピー）・
  相互作用チェッカー自然言語解決（タノベア/ステルフォンタ/ゼンレリア）の3動線を検証済み

### 診断チャット精度 第35弾（66症例・2波スイープ → 全実用症例合格）
- **系統的故障クラス: 助詞直後の副詞挿入** — 「後ろ足が**急に**動かなくなり」（猫ATE!）
  「後ろ足を**急に**痛がって」「顔が**パンパンに**腫れて」「毛を**自分で**かじって」が
  既存キュレートキーに不一致で抽出ゼロ/劣化していた → `normalize_chat_text` に
  `_ADVERB_STRIP`（が/を/も/は 直後の 急に・突然・パンパンに・自分で を除去 —
  助詞直後のこれらは常に述語修飾で、除去しても症状の意味は不変）を追加。
  副詞を含む既存キー12件は全てベース形キーの併存を検証（目が急に赤→目が赤 等）。
  過去セッションが個別エイリアスで対処してきた故障クラスの根本対応
- **丁寧語正規化の残穴**: 治りません→治らない（五段・治る）を追加 —
  「かさぶたができて治りません」で non_healing_wound キー群が発火（猫SCC/ボーエン top-2）
- **新規エイリアス**: ケンケンしながら/ケンケン歩き（カタカナ形）→lameness_or_limping、
  ブツブツが出/ぶつぶつが出→skin_rashes
- **クラスタ追加**: レガシー犬 {facial_swelling, skin_rashes}→急性蕁麻疹・血管性浮腫 ×1.5
  （ブツブツ=膨疹の飼い主表現。急性顔面腫脹+発疹はアレルギー反応 until proven otherwise —
  毛包虫症・膿皮症は急性顔面腫脹を主訴にしない。顔面腫脹なしの掻痒・発疹は
  皮膚科ddx firstを維持 — ガードテストで固定）
- 修正後の実測: 猫ATE副詞形 rank 1、犬急性跛行→パテラ/肘形成不全/CCL top-3、
  チンチラ毛噛み rank 1、顔面腫脹+発疹→蕁麻疹 top-2

### UX: 腫瘍疾患→BSA計算機のワンタップ動線
- `DISEASE_CALC_MAP` に **リンパ腫/肥満細胞腫/骨肉腫/血管肉腫（犬猫）→ BSA計算機タブ** を追加 —
  CHOP・ビンブラスチン・ドキソルビシン・カルボプラチン等の mg/m² 換算がそのまま次のアクションになる
  腫瘍群。チェッカー結果・自由チャット・問診最終結果・疾患DB詳細の既存4面レンダラーで自動発火
  （本セッションの新規腫瘍薬2剤とも接続: MCT閲覧→Stelfontaチップ+BSA計算機）

### 回帰テスト（+14件）
- `TestChatClinicalAccuracyAuditRound35`（8件 — 副詞ストリップ単体+助詞なし文脈の不変ガード、
  猫ATE/犬跛行/顔面腫脹アレルギー/皮膚科ガード/チンチラ毛噛み/オウム毛引きガード/治りません）
- `TestBatch66OncologyAndZenreliaFix`（5件 — epofolaner不在+alias解決+JAK正クラス+枠組み警告、
  タノベア用量+ウェスティ禁忌、ステルフォンタ用量上限+必須併用、マッチャー/チェッカー解決、
  疾患テキスト参照+L-SARN再発防止）
- `test_app_js_disease_to_calculator_pivot_wired` に BSA マップ検証を追加

### 表示数値の同期・キャッシュ
- `setDefaultStats()`: dog 584/cat 561薬品、pendingStats drugs 642→**644**
- ServiceWorker: `CACHE_NAME` v159 → **v160**
- 再現手順: `migrate_to_sqlite.py`（クリーンビルド 6,893疾患・644薬品）— 疾患名不変のため
  検索インデックス no-op

## 2026-09セッション（第54弾: シロリムス（Felycin-CA1）公開 — 猫心疾患初のFDA承認薬 + 犬・趾間皮膚炎の新設 + チャット精度第36弾）

### エラーチェック（結果: ベースライン健全）
- repo全体 ruff check clean、フルテスト **4,485件合格**（34 skip、ベースライン）
- 配信SQLiteクリーンビルド: **6,893疾患・644薬品**、treatment/prevention/prognosis **100%**、
  主要臨床フィールド（治療/病因/予後/予防/説明/病態）の空欄 **0**
- 薬用量: safe薬品の dosage 欠落 **0**、文字列型相互作用スキーマ **0**、ID重複 **0**
- 麻酔: 全21種×全8カテゴリ完備（188プロトコル）、薬剤行の dose/name_ja 欠落 **0**、全種 references あり
- 薬品マッチャー飽和度（第34回スイープ、片仮名+英語×用量文脈の実スニペット突合）:
  referenced-but-absent の真の欠落 **0**（テアニン/カソゼピン/メグルミン/モフェチル/デキストラン/
  MgSO4/IVIG/アグレプリストン等の上位候補は全て解決済み。カモミール/パームオイルは用量なしの
  ハーブ補助言及で対象外）

### エビデンスベースの新規薬品公開: シロリムス徐放錠（Felycin-CA1）（`drug_batch_67.py` 新規、644→645薬品）
- **猫の心疾患に対して史上初めてFDAが承認（2025年条件付き承認）した薬剤**なのに、辞書には
  mTOR阻害薬が皆無で、猫HCMフラグシップにも疾患修飾の選択肢が無かった（アテノロール/
  ジルチアゼム=対症、クロピドグレル=血栓予防のみ）。DailyMedの米国ラベルで用量・禁忌を検証の上収載:
  - 目標 0.3 mg/kg PO **週1回**・食事と共に。徐放錠（0.4/1.2/2.4 mg）は**丸ごと嚥下必須**
    （分割・粉砕で徐放性が失われる）、体重帯別の錠数テーブル、**2.5 kg未満は投与不可**
  - 適応: 無症候性HCM（拡張末期左室壁厚≥6 mm）で CHF/全身性高血圧/重度LVOT閉塞の無い症例
  - **禁忌: 糖尿病（治験中1例が致死的DKAへ進行 — 発症時は直ちに中止）・既存の肝疾患**。
    ALT/AST を開始後1-2ヶ月毎→安定後6-12ヶ月毎に監視
  - **条件付き承認の正直なエビデンス枠組み**: 有効性はRAPACAT無作為化試験（週1回投与で6ヶ月時点の
    左室壁厚進行を抑制 — Kaplan 2023）に基づく「合理的期待」段階と明記（過大評価しない）
  - CYP3A4/P-gp相互作用面: アゾール・**ジルチアゼム（HCM併用薬の定番 — 併用前に再評価）**・
    アムロジピン/アミオダロン・シクロスポリン=major、エプリノメクチン/エモデプシド（P-gp基質）・
    生ワクチン=moderate。犬は safe:False（承認適応・検証済み用量なし）
- **鑑別診断・相談チャットへの動線**: 猫HCMエントリ（JSONオーバーレイ）の治療文に無症候性期の
  Felycin-CA1 行を追記（日英）→ 治療チップ（find_drugs_in_text解決を検証）・逆引き
  「この薬品を使う疾患」（sirolimus→猫HCM）・相互作用チェッカー自然言語解決
  （シロリムス/ラパマイシン/フェリシン/felycin）+ `drug_interactions.py` レジストリに
  sirolimus×ジルチアゼム/ケトコナゾール/シクロスポリン=major を登録（併用チェッカーで1タップ検出）

### 新規疾患: 犬・趾間皮膚炎（レガシーチャットDB、93疾患・89症状に）
- 犬皮膚科の頻出主訴（趾間フルンクローシス＝深在性膿皮症 — Muller & Kirk 7th ed）なのに
  レガシーDBに肢端部位のエントリが無く、「肉球の間が赤く腫れて」が汎用 itching に解決され
  毛包虫症/外耳炎が上位だった → **部位特異ID `paw_redness` を新設**（肉球の間が赤/指の間が赤い/
  指の間が腫れ 等のエイリアスを itching/skin_redness から移行）+ interdigital_furunculosis エントリ
  （common、英ブル2.0×/仏ブル1.8×/ラブ1.6×、毛包破裂→角化物異物反応の病態と培養に基づく4-6週+
  全身抗菌薬・基礎アレルギー管理を記載）+ 単独クラスタ {paw_redness}×1.5（部位限局発赤は部位診断的）
  → rank 1。汎用掻痒主訴の非ハイジャックをガードテストで固定
- **名称は dog モジュール（Interdigital Cyst (Furuncle)）と base-name 整合** — チャット候補カードの
  「疾患DBで詳細を開く」ピボットが完全一致着地。SPECIES_PREVALENCE dog に
  'Interdigital Cyst (Furuncle)': common を追加（チェックボックス経路の prior + チップ導線を活性化）
- ID_SYNONYMS: paw_redness→[pododermatitis_signs, foot_lesions, foot_sores, skin_redness, itching]
  （非保有種は従来挙動へ安全フォールバック）

### 診断チャット精度 第36弾（35症例フレッシュスイープ 10 MISS → 35/35 合格）
- **程度・時間副詞の guarded 除去**（第35弾の副詞ストリップを拡張）: すごく/とても/かなり/いつも/ずっと
  を助詞直後で除去。**副詞自体を含むキュレートキー3系統は lookaround で温存**
  （食欲がすごく→increased_appetite・いつもと違う→lethargy・口をずっと開け→open_mouth_breathing —
  回帰テストで固定）→「水を飲む量がすごく増えて」「あごの下がいつも濡れて」が抽出可能に
- **丁寧語正規化の残穴**: 入りません→入らない/入ります→入る（五段・入る）→
  「片足に力が入りません」（鳥の止まり木落下+片肢不全）が全経路で解決
- **新規エイリアス**: 吐こうとするのに→unproductive_retching（GDVの のに接続形 → rank 1）、
  白目が黄色っぽ/目が黄色っぽ→jaundice、**トイレでいきん/長い時間いきん→straining_to_urinate**
  （飼い主が便秘と誤認するブロック猫の教科書的シナリオ — トイレ内怒責+鳴き続け（新規→
  vocalization_changes）で尿道閉塞 rank 1。便秘明記主訴は便秘系維持をガードで固定）、
  口を開けると痛/口を開けるときに痛/ご飯を途中でやめ→difficulty_eating（猫FCGS/歯周病 top3）、
  あごの下が濡れ→wet_chin（ウサギ不正咬合群）、止まり木から落ち→falling_off_perch（て形）、
  舐め続け→excessive_licking（連用形）
- **馬・指動脈拍動の飼い主表現**: 「指で触ると脈を感じます」が抽出ゼロだった → 触ると脈/脈を感じ/
  脈が触れ/脈がドクドク→limb_digital_pulse（EQUINE_SYMPTOM_ALIASES）→ hot-hoofペア発火で
  蹄葉炎/蹄膿瘍 top-2（bounding digital pulse — Adams & Stashak 7th ed）
- 回帰テスト: `TestChatClinicalAccuracyAuditRound36`（14件）+ `TestBatch67Sirolimus`（4件）

### UX: クイック入力の拡充
- 犬「肉球の間が赤く腫れて舐め続ける」を追加（→趾間皮膚炎 rank 1、ミラーテスト JA_QUICK 同期）

### 表示数値の同期・キャッシュ
- `setDefaultStats()`: dog 585/cat 562薬品、pendingStats drugs 644→**645**・symptoms 88→**89**
